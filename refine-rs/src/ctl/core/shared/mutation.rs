...
impl AddMutation {
    ...
    pub fn with_attrs(mut self, attrs: impl IntoIterator<Item = (AttrId, AttrMutation)>) -> Self {
...
impl ChangeMutation {
    ...
    pub fn with_attrs(mut self, attrs: impl IntoIterator<Item = (AttrId, Option<AttrMutation>)>) -> Self {
        self.attrs.extend(attrs);
        self
    }
}
...
fn apply_roll(core_mutation: &mut rc::MutationMut, attr_id: AttrId, roll: UnitInterval) {
    ...
    match core_mutation.get_raw_mattr_mut(attr_id) {
        ...
        Err(rc::err::RawMAttrGetError::MutationNotFound(..)) => {
