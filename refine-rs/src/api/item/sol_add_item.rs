use crate::{Item, ItemAddCmd, ItemInfo, ItemInfoCmd, SolarSystem, err::ItemAddError};

impl<'r, 's> SolarSystem<'r> {
    #[tracing::instrument(name = "itm-add", level = "trace", skip_all)]
    pub async fn add_item(&'s mut self, ctl_cmd: ItemAddCmd) -> Result<Item<'r, 's>, ItemAddError> {
        let item_id = self
    ...
    pub async fn add_item_and_get_info(
    ...
    ) -> Result<(Item<'r, 's>, ItemInfo), ItemAddError> {
        let (item_id, item_info) = self
            .exec_standard_fallible(move |core_sol| {
                ...
                Ok::<_, ItemAddError>((item_id, item_info))
            })
            .await?;
        let item = Item::new(self, item_id);
        Ok((item, item_info))
