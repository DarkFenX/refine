...
impl ItemInfo {
    ...
    pub(crate) fn from_core_autocharge(core_autocharge: &mut rc::AutochargeMut, item_mode: ItemInfoMode) -> Self {
        Self::Autocharge(AutochargeInfo::from_core(core_autocharge, item_mode))
    }
    pub(crate) fn from_core_booster(core_booster: &mut rc::BoosterMut, item_mode: ItemInfoMode) -> Self {
        Self::Booster(BoosterInfo::from_core(core_booster, item_mode))
    }
    pub(crate) fn from_core_character(core_character: &mut rc::CharacterMut, item_mode: ItemInfoMode) -> Self {
        Self::Character(CharacterInfo::from_core(core_character, item_mode))
    }
    pub(crate) fn from_core_charge(core_charge: &mut rc::ChargeMut, item_mode: ItemInfoMode) -> Self {
        Self::Charge(ChargeInfo::from_core(core_charge, item_mode))
    }
    pub(crate) fn from_core_drone(core_drone: &mut rc::DroneMut, item_mode: ItemInfoMode) -> Self {
        Self::Drone(DroneInfo::from_core(core_drone, item_mode))
    }
    pub(crate) fn from_core_fighter(core_fighter: &mut rc::FighterMut, item_mode: ItemInfoMode) -> Self {
        Self::Fighter(FighterInfo::from_core(core_fighter, item_mode))
    }
    pub(crate) fn from_core_fw_effect(core_fw_effect: &mut rc::FwEffectMut, item_mode: ItemInfoMode) -> Self {
        Self::FwEffect(FwEffectInfo::from_core(core_fw_effect, item_mode))
    }
    pub(crate) fn from_core_implant(core_implant: &mut rc::ImplantMut, item_mode: ItemInfoMode) -> Self {
        Self::Implant(ImplantInfo::from_core(core_implant, item_mode))
    }
    pub(crate) fn from_core_module(core_module: &mut rc::ModuleMut, item_mode: ItemInfoMode) -> Self {
        Self::Module(ModuleInfo::from_core(core_module, item_mode))
    }
    pub(crate) fn from_core_proj_effect(core_proj_effect: &mut rc::ProjEffectMut, item_mode: ItemInfoMode) -> Self {
    ...
    pub(crate) fn from_core_service(core_service: &mut rc::ServiceMut, item_mode: ItemInfoMode) -> Self {
        Self::Service(ServiceInfo::from_core(core_service, item_mode))
    }
    pub(crate) fn from_core_ship(core_ship: &mut rc::ShipMut, item_mode: ItemInfoMode) -> Self {
        Self::Ship(ShipInfo::from_core(core_ship, item_mode))
    }
    pub(crate) fn from_core_skill(core_skill: &mut rc::SkillMut, item_mode: ItemInfoMode) -> Self {
        Self::Skill(SkillInfo::from_core(core_skill, item_mode))
    }
    pub(crate) fn from_core_stance(core_stance: &mut rc::StanceMut, item_mode: ItemInfoMode) -> Self {
        Self::Stance(StanceInfo::from_core(core_stance, item_mode))
    }
    pub(crate) fn from_core_subsystem(core_subsystem: &mut rc::SubsystemMut, item_mode: ItemInfoMode) -> Self {
        Self::Subsystem(SubsystemInfo::from_core(core_subsystem, item_mode))
    }
    pub(crate) fn from_core_sw_effect(core_sw_effect: &mut rc::SwEffectMut, item_mode: ItemInfoMode) -> Self {
