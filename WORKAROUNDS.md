...
- `serde_with` has `KeyValueMap` which allows to turn a sequence of structs into a map. However, it does not provide a way to transform value (pick one of struct fields as map value). Once [issue #981](https://github.com/jonasbb/serde_with/issues/981) is resolved, go through all uses of `custom_serde.*::as_map`, and replace custom serialization by `KeyValueMap` with value tag where applicable. 
