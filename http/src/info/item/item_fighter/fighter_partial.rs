...
use super::rearm_minion::HItemRearmMinionInfo;
use crate::{
    info::{
        HItemInfoMode,
        item::{
            count_info::HItemCountInfo, item_autocharge::HAutochargeInfo, item_fighter::ability::HAbilityInfo,
...
pub(crate) struct HFighterInfoPartial {
    ...
    count: Option<HItemCountInfo>,
    #[serde_as(as = "Map<DisplayFromStr, _>")]
    #[serde(skip_serializing_if = "Vec::is_empty")]
    abilities: Vec<(i32, HAbilityInfo)>,
    rearm_minion: HItemRearmMinionInfo,
    #[serde_as(as = "Map<DisplayFromStr, _>")]
    #[serde(skip_serializing_if = "Vec::is_empty")]
...
impl HFighterInfoPartial {
    pub(super) fn from_core(core_fighter: &mut rc::FighterMut, item_mode: HItemInfoMode) -> Self {
        Self {
            ...
            count: core_fighter.get_count().map(HItemCountInfo::from_core_fighter_count),
            abilities: core_fighter
                .iter_abilities()
                .map(|v| (v.get_id().into_i32(), HAbilityInfo::from_core(v)))
                .collect(),
            rearm_minion: HItemRearmMinionInfo::from_core(core_fighter.get_rearm_minion()),
