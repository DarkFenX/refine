...
impl HValChargeSizeFail {
    pub(in crate::info::validation) fn from_core(core_val_fail: rc::val::ValChargeSizeFail) -> Self {
        Self {
            charges: core_val_fail
                .charges
                .into_iter()
                .map(|(charge_item_id, core_charge_info)| {
                    (charge_item_id, HValChargeSizeItemInfo::from_core(core_charge_info))
                })
                .collect(),
        }
    }
...
impl HValChargeSizeItemInfo {
    fn from_core(core_val_charge_info: rc::val::ValChargeSizeChargeInfo) -> Self {
        Self {
