...
def test_affected_child(client, consts):
    # Make sure child owner-modifiable items are affected by projected buffs
    eve_buff_type_attr = client.mk_eve_attr(id_=consts.EveAttr.warfare_buff_1_id)
    eve_buff_val_attr = client.mk_eve_attr(id_=consts.EveAttr.warfare_buff_1_value)
    eve_affectee_attr = client.mk_eve_attr()
    eve_buff = client.mk_eve_buff(
...
def test_unaffected_root_via_child(client, consts):
    ...
    eve_proj_effect = client.mk_eve_item(
        ...
        eff_ids=[eve_effect.id], defeff_id=eve_effect.id)
    eve_ship = client.mk_eve_ship(attrs={eve_affectee_attr.id: 7.5})
    eve_drone = client.mk_eve_item(attrs={eve_affectee_attr.id: 15})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship.id)
    ...
    api_proj_effect = api_sol.add_proj_effect(type_id=eve_proj_effect.id)
    api_proj_effect.change_proj_effect(add_projs=[api_drone.id])
    assert api_drone.update().attrs[eve_affectee_attr.id].dogma == approx(75)
...
def test_unaffected_child_via_root(client, consts):
    ...
    api_proj_effect.change_proj_effect(add_projs=[api_ship.id])
    assert api_ship.update().attrs[eve_affectee_attr.id].dogma == approx(37.5)
    assert api_drone.update().attrs[eve_affectee_attr.id].dogma == approx(15)
