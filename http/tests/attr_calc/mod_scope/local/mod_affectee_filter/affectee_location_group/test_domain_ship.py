...
def test_affected_charge(client, consts):
    ...
    eve_charge = client.mk_eve_item(grp_id=eve_grp.id, attrs={eve_affectee_attr.id: 3200})
    eve_ship = client.mk_eve_ship(attrs={eve_affector_attr.id: 50}, eff_ids=[eve_effect.id])
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_fit.set_ship(type_id=eve_ship.id)
    api_module = api_fit.add_mod(type_id=eve_module.id, charge_type_id=eve_charge.id)
    assert api_module.update().charge.attrs[eve_affectee_attr.id].dogma == approx(4800)
