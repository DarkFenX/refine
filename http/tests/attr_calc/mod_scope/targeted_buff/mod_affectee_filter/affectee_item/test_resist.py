...
def test_resisted_value_change_root_struct(client, consts):
    ...
    eve_rig_effect = client.mk_eve_effect(cat_id=consts.EveEffCat.passive, mod_info=[eve_rig_mod])
    eve_affectee_rig = client.mk_eve_item(attrs={eve_boost_attr.id: -25}, eff_ids=[eve_rig_effect.id])
    eve_affectee_struct = client.mk_eve_struct(attrs={eve_affectee_attr.id: 200, eve_resist_attr.id: 0.4})
    client.create_sources()
    api_sol = client.create_sol()
    api_affector_fit = api_sol.create_fit()
    api_affectee_fit = api_sol.create_fit()
    ...
    api_affectee_rig = api_affectee_fit.add_rig(type_id=eve_affectee_rig.id)
    assert api_affectee_struct.update().attrs[eve_affectee_attr.id].dogma == approx(167)
    api_affectee_rig.remove()
    assert api_affectee_struct.update().attrs[eve_affectee_attr.id].dogma == approx(156)
