...
def test_self_buff_switch_no_default(client, consts):
    ...
    # Verification
    api_ship.update()
    assert api_ship.attrs[eve_affectee_attr1.id].dogma == approx(20)
    assert api_ship.attrs[eve_affectee_attr2.id].dogma == approx(50)
    # Action
    api_module.change_mod(charge=eve_charge1.id)
    # Verification
    api_ship.update()
    assert api_ship.attrs[eve_affectee_attr1.id].dogma == approx(100)
    assert api_ship.attrs[eve_affectee_attr2.id].dogma == approx(50)
    # Action
    ...
    api_ship.update()
    assert api_ship.attrs[eve_affectee_attr1.id].dogma == approx(20)
    assert api_ship.attrs[eve_affectee_attr2.id].dogma == approx(500)
    # Action
...
def test_self_buff_switch_with_default(client, consts):
    ...
    eve_buff3 = client.mk_eve_buff(
        aggr_mode=consts.EveBuffAggrMode.max,
        op=consts.EveBuffOp.post_mul,
        item_mods=[client.mk_eve_buff_mod(attr_id=eve_affectee_attr3.id)])
    ...
    assert api_ship.attrs[eve_affectee_attr1.id].dogma == approx(20)
    assert api_ship.attrs[eve_affectee_attr2.id].dogma == approx(50)
    assert api_ship.attrs[eve_affectee_attr3.id].dogma == approx(125)
    # Action
    api_module.change_mod(charge=eve_charge1.id)
    # Verification
    api_ship.update()
    assert api_ship.attrs[eve_affectee_attr1.id].dogma == approx(100)
    assert api_ship.attrs[eve_affectee_attr2.id].dogma == approx(50)
    assert api_ship.attrs[eve_affectee_attr3.id].dogma == approx(100)
    # Action
    api_module.change_mod(charge=eve_charge2.id)
    # Verification
    api_ship.update()
...
def test_unaffected_on_fleet_add(client, consts):
    ...
    assert api_ship2.update().attrs[eve_affectee_attr.id].dogma == approx(7.5)
