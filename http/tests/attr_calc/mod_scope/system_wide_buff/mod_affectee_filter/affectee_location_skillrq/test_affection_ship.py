...
def test_affected_child_ship_multiple(client, consts):
    ...
    assert api_module1.update().attrs[eve_affectee_attr.id].dogma == approx(260)
    assert api_module2.update().attrs[eve_affectee_attr.id].dogma == approx(260)
...
def test_unaffected_other_skillrq(client, consts):
    ...
    eve_sw_effect = client.mk_eve_item(
        ...
        eff_ids=[eve_effect.id],
        defeff_id=eve_effect.id)
    eve_module = client.mk_eve_item(attrs={eve_affectee_attr.id: 200}, srqs={eve_skill2.id: 1})
    eve_ship = client.mk_eve_ship()
    client.create_sources()
    api_sol = client.create_sol()
    api_sol.add_sw_effect(type_id=eve_sw_effect.id)
...
def test_unaffected_root_char(client, consts):
    ...
    eve_sw_effect = client.mk_eve_item(
        ...
        eff_ids=[eve_effect.id],
        defeff_id=eve_effect.id)
    eve_char = client.mk_eve_item(attrs={eve_affectee_attr.id: 200}, srqs={eve_skill.id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_sol.add_sw_effect(type_id=eve_sw_effect.id)
    api_fit = api_sol.create_fit()
