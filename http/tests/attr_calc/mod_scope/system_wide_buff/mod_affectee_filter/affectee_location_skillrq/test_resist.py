...
def test_resisted_value_change(client, consts):
    ...
    eve_sw_effect_buff = client.mk_eve_buff(
        aggr_mode=consts.EveBuffAggrMode.max,
        op=consts.EveBuffOp.post_mul,
        loc_srq_mods=[client.mk_eve_buff_mod(attr_id=eve_affectee_attr.id, skill_id=eve_skill.id)])
    eve_sw_effect_effect = client.mk_eve_effect(
        id_=consts.EveEffect.weather_darkness,
        cat_id=consts.EveEffCat.active,
        resist_attr_id=eve_resist_attr.id)
    eve_sw_effect = client.mk_eve_item(
        attrs={eve_buff_type_attr.id: eve_sw_effect_buff.id, eve_buff_val_attr.id: 5},
        eff_ids=[eve_sw_effect_effect.id], defeff_id=eve_sw_effect_effect.id)
    ...
    eve_ship = client.mk_eve_ship(attrs={eve_resist_attr.id: 0.4})
