...
def test_unaffected_other_fit(client, consts):
    ...
    api_struct2 = api_fit2.set_ship(type_id=eve_struct.id)
    api_proj_effect = api_sol.add_proj_effect(type_id=eve_proj_effect.id)
    api_proj_effect.change_proj_effect(add_projs=[api_struct1.id])
    assert api_struct2.update().attrs[eve_affectee_attr.id].dogma == approx(100)
...
def test_replace_target(client, consts):
    ...
    eve_effect = client.mk_eve_effect(cat_id=consts.EveEffCat.system, mod_info=[eve_mod])
    eve_proj_effect = client.mk_eve_item(attrs={eve_affector_attr.id: 20}, eff_ids=[eve_effect.id])
    eve_struct1 = client.mk_eve_struct(attrs={eve_affectee_attr.id: 100})
    eve_struct2 = client.mk_eve_struct(attrs={eve_affectee_attr.id: 50})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_struct1 = api_fit.set_ship(type_id=eve_struct1.id)
    api_proj_effect = api_sol.add_proj_effect(type_id=eve_proj_effect.id)
    api_proj_effect.change_proj_effect(add_projs=[api_struct1.id])
    assert api_struct1.update().attrs[eve_affectee_attr.id].dogma == approx(120)
    api_struct2 = api_fit.set_ship(type_id=eve_struct2.id)
    assert api_struct2.update().attrs[eve_affectee_attr.id].dogma == approx(50)
    api_proj_effect.change_proj_effect(add_projs=[api_struct2.id])
    assert api_struct2.update().attrs[eve_affectee_attr.id].dogma == approx(60)
