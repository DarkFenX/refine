...
def test_affected_state_change(client, consts):
    ...
    eve_struct = client.mk_eve_item(attrs={eve_affectee_attr.id: 100})
...
def test_replace_target(client, consts):
    ...
    eve_effect = client.mk_eve_effect(cat_id=consts.EveEffCat.system, mod_info=[eve_mod])
    eve_proj_effect = client.mk_eve_item(attrs={eve_affector_attr.id: 20}, eff_ids=[eve_effect.id])
    eve_struct1 = client.mk_eve_item(attrs={eve_affectee_attr.id: 100})
    eve_struct2 = client.mk_eve_item(attrs={eve_affectee_attr.id: 50})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
