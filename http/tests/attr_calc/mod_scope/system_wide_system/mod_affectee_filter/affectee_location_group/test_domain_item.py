...
def test_unaffected(client, consts):
    ...
    eve_parent_tgt_item = client.mk_eve_item(grp_id=eve_grp.id, attrs={eve_tgt_attr.id: 100})
    eve_child_tgt_item = client.mk_eve_item(grp_id=eve_grp.id, attrs={eve_tgt_attr.id: 100})
    client.create_sources()
    api_sol = client.create_sol()
    api_sol.add_sw_effect(type_id=eve_src_item.id)
    api_fit = api_sol.create_fit()
    api_parent_tgt_item = api_fit.set_ship(type_id=eve_parent_tgt_item.id)
    api_child_tgt_item = api_fit.add_rig(type_id=eve_child_tgt_item.id)
    assert api_parent_tgt_item.update().attrs[eve_tgt_attr.id].dogma == approx(100)
