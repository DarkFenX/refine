...
def test_value_change_root_ship(client, consts):
    ...
    eve_affectee_ship = client.mk_eve_ship(attrs={eve_affectee_attr.id: 500, eve_resist_attr.id: 0.3})
    ...
    assert api_affectee_ship.update().attrs[eve_affectee_attr.id].dogma == approx(410)
    api_affectee_rig = api_affectee_fit.add_rig(type_id=eve_affectee_rig.id)
    assert api_affectee_ship.update().attrs[eve_affectee_attr.id].dogma == approx(365)
...
def test_value_change_root_struct(client, consts):
    ...
    eve_rig_effect = client.mk_eve_effect(cat_id=consts.EveEffCat.passive, mod_info=[eve_rig_mod])
    eve_affectee_rig = client.mk_eve_item(attrs={eve_boost_attr.id: 50}, eff_ids=[eve_rig_effect.id])
    eve_affectee_struct = client.mk_eve_struct(attrs={eve_affectee_attr.id: 500, eve_resist_attr.id: 0.3})
    client.create_sources()
    api_sol = client.create_sol()
    api_affector_fit = api_sol.create_fit()
    api_affectee_fit = api_sol.create_fit()
    ...
    assert api_affectee_struct.update().attrs[eve_affectee_attr.id].dogma == approx(410)
    api_affectee_rig = api_affectee_fit.add_rig(type_id=eve_affectee_rig.id)
    assert api_affectee_struct.update().attrs[eve_affectee_attr.id].dogma == approx(365)
    api_affectee_rig.remove()
    assert api_affectee_struct.update().attrs[eve_affectee_attr.id].dogma == approx(410)
...
def test_value_change_child(client, consts):
    ...
    eve_affectee_drone = client.mk_eve_item(
        attrs={eve_affectee_attr.id: 500, eve_resist_attr.id: 0.3},
        srqs={eve_skill.id: 1})
    ...
    api_affector_fit = api_sol.create_fit()
    api_affectee_fit = api_sol.create_fit()
    api_affectee_fit.set_ship(type_id=eve_affectee_ship.id)
