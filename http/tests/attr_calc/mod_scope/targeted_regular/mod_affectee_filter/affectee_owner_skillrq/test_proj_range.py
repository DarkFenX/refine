...
def test_add_with_change_without_remove(client, consts):
    ...
    assert api_affectee_drone.update().attrs[eve_affectee_attr.id].dogma == approx(500)
    api_affector_module.change_mod(add_projs=[(api_affectee_ship.id, 15000)])
    assert api_affectee_drone.update().attrs[eve_affectee_attr.id].dogma == approx(350)
...
def test_add_without_change_with_remove(client, consts):
    ...
    eve_affector_module = client.mk_eve_item(
        ...
        defeff_id=eve_effect.id)
    eve_affectee_ship = client.mk_eve_ship()
    eve_affectee_fighter = client.mk_eve_item(attrs={eve_affectee_attr.id: 500}, srqs={eve_skill.id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_affector_fit = api_sol.create_fit()
    api_affectee_fit = api_sol.create_fit()
    api_affectee_ship = api_affectee_fit.set_ship(type_id=eve_affectee_ship.id)
    api_affectee_fighter = api_affectee_fit.add_drone(type_id=eve_affectee_fighter.id)
    api_affector_module = api_affector_fit.add_mod(type_id=eve_affector_module.id, state=consts.ApiState.active)
    assert api_affectee_fighter.update().attrs[eve_affectee_attr.id].dogma == approx(500)
    ...
    assert api_affectee_fighter.update().attrs[eve_affectee_attr.id].dogma == approx(350)
    api_affector_module.change_mod(rm_projs=[api_affectee_ship.id])
    assert api_affectee_fighter.update().attrs[eve_affectee_attr.id].dogma == approx(500)
...
def test_add_with_change_with_remove(client, consts):
    ...
    assert api_affectee_drone.update().attrs[eve_affectee_attr.id].dogma == approx(481.25)
...
def test_value_change_optimal(client, consts):
    ...
    api_sol = client.create_sol()
    api_affector_fit = api_sol.create_fit()
    api_affector_fit.set_ship(type_id=eve_ship.id)
    api_affectee_fit = api_sol.create_fit()
    ...
    assert api_affectee_drone.update().attrs[eve_affectee_attr.id].dogma == approx(247.731075)
...
def test_value_change_falloff(client, consts):
    ...
    assert api_affectee_fighter.update().attrs[eve_affectee_attr.id].dogma == approx(350)
    api_affector_rig = api_affector_fit.add_rig(type_id=eve_affector_rig.id)
    assert api_affectee_fighter.update().attrs[eve_affectee_attr.id].dogma == approx(279.539826)
    api_affector_rig.remove()
    assert api_affectee_fighter.update().attrs[eve_affectee_attr.id].dogma == approx(350)
