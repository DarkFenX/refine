...
def test_add_with_change_without_remove(client, consts):
    ...
    eve_affectee_rig = client.mk_eve_item(attrs={eve_affectee_attr.id: 500}, srqs={eve_skill.id: 1})
    ...
    assert api_affectee_rig.update().attrs[eve_affectee_attr.id].dogma == approx(500)
    api_affector_module.change_mod(add_projs=[(api_affectee_ship.id, 15000)])
    assert api_affectee_rig.update().attrs[eve_affectee_attr.id].dogma == approx(350)
    api_affector_module.change_mod(change_projs=[(api_affectee_ship.id, None)])
    assert api_affectee_rig.update().attrs[eve_affectee_attr.id].dogma == approx(200)
...
def test_add_with_change_with_remove(client, consts):
    ...
    assert api_affectee_rig.update().attrs[eve_affectee_attr.id].dogma == approx(481.25)
...
def test_value_change_optimal(client, consts):
    ...
    eve_affectee_struct = client.mk_eve_struct()
    eve_affectee_module = client.mk_eve_item(attrs={eve_affectee_attr.id: 500}, srqs={eve_skill.id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_affector_fit = api_sol.create_fit()
