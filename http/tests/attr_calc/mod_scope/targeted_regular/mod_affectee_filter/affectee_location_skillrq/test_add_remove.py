...
def test_add_afor_afee_proj_remove_state_proj_fit(client, consts):
    ...
    eve_affectee_module = client.mk_eve_item(attrs={eve_affectee_attr.id: 80}, srqs={eve_skill.id: 1})
    ...
    assert api_affectee_module.update().attrs[eve_affectee_attr.id].dogma == approx(80)
    api_affector_module.change_mod(add_projs=[api_affectee_ship.id])
    assert api_affectee_module.update().attrs[eve_affectee_attr.id].dogma == approx(96)
