"""
This module contains a few general tests. You can find more specific tests in per-scope folders.
"""

from pytest import approx


def test_attr_defined_on_effect(client, consts):
...
def test_attr_defined_on_affector_item(client, consts):
...
def test_invalid_reference_on_effect(client, consts):
    ...
    eve_affector_module = client.mk_eve_item(
        ...
        eff_ids=[eve_module_effect.id],
        defeff_id=eve_module_effect.id)
    eve_affectee_ship = client.mk_eve_ship(attrs={eve_affectee_attr.id: 500, eve_resist_attr_id: 0.4})
    client.create_sources()
    api_sol = client.create_sol()
    api_affector_fit = api_sol.create_fit()
    api_affectee_fit = api_sol.create_fit()
...
def test_invalid_reference_on_item(client, consts):
    ...
    eve_affector_module1 = client.mk_eve_item(
        ...
        defeff_id=eve_module_effect.id)
    eve_affector_module2 = client.mk_eve_item(
        attrs={eve_affector_attr.id: -55, eve_resist_def_attr.id: eve_resist_attr2_id},
        eff_ids=[eve_module_effect.id],
        defeff_id=eve_module_effect.id)
    eve_affectee_ship = client.mk_eve_ship(
        attrs={eve_affectee_attr.id: 500, eve_resist_attr1.id: 0.4, eve_resist_attr2_id: 0.3})
    client.create_sources()
    api_sol = client.create_sol()
    api_affector_fit = api_sol.create_fit()
    api_affectee_fit = api_sol.create_fit()
    ...
    api_affector_module2 = api_affector_fit.add_mod(type_id=eve_affector_module2.id, state=consts.ApiState.active)
    api_affector_module2.change_mod(add_projs=[api_affectee_ship.id])
    assert api_affectee_ship.update().attrs[eve_affectee_attr.id].dogma == approx(171)
