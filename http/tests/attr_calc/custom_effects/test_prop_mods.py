...
def test_ab(client, consts):
    ...
    eve_prop_item = client.mk_eve_item(
        attrs={
            ...
            eve_mass_add_attr.id: 500000, eve_sig_affector_attr.id: 200},
    ...
    # Verification
    api_ship.update()
    assert api_ship.attrs[eve_mass_attr.id].dogma == approx(1550000)
    assert api_ship.attrs[eve_sig_affectee_attr.id].dogma == approx(32)  # Not affected by sig blow
    assert api_ship.attrs[eve_speed_attr.id].dogma == approx(1049.435484)
    assert len(api_ship.mods) == 2
    # Mass modification
    api_mod_mass = api_ship.mods.find_by_affector_attr(
        affectee_attr_id=eve_mass_attr.id, affector_attr_id=eve_mass_add_attr.id).one()
    ...
    assert api_mod_mass.affectors.one().attr_id == eve_mass_add_attr.id
    # Speed modification
    api_mod_prop = api_ship.mods.find_by_affector_attr(
        affectee_attr_id=eve_speed_attr.id, affector_attr_id=eve_speed_boost_attr.id).one()
    ...
    assert api_mod_prop.affectors.find_by_attr(attr_id=eve_speed_boost_attr.id).one().item_id == api_prop_item.id
    assert api_mod_prop.affectors.find_by_attr(attr_id=eve_thrust_attr.id).one().item_id == api_prop_item.id
    assert api_mod_prop.affectors.find_by_attr(attr_id=eve_mass_attr.id).one().item_id == api_ship.id
...
def test_mwd(client, consts):
    ...
    eve_prop_item = client.mk_eve_item(
        attrs={
            ...
            eve_mass_add_attr.id: 500000, eve_sig_affector_attr.id: 450},
        eff_ids=[eve_prop_effect.id],
        defeff_id=eve_prop_effect.id)
    client.create_sources()
    api_sol = client.create_sol()
    ...
    api_ship.update()
    assert api_ship.attrs[eve_mass_attr.id].dogma == approx(1550000)
    assert api_ship.attrs[eve_sig_affectee_attr.id].dogma == approx(176)
    assert api_ship.attrs[eve_speed_attr.id].dogma == approx(2678.629032)
    ...
    api_mod_sig = api_ship.mods.find_by_affector_attr(
        affectee_attr_id=eve_sig_affectee_attr.id, affector_attr_id=eve_sig_affector_attr.id).one()
    ...
    assert api_mod_sig.affectors.one().attr_id == eve_sig_affector_attr.id
    # Speed modification
    api_mod_prop = api_ship.mods.find_by_affector_attr(
        affectee_attr_id=eve_speed_attr.id, affector_attr_id=eve_speed_boost_attr.id).one()
    assert api_mod_prop.op == consts.ApiModOp.post_mul
...
def test_speed_mod_stacking(client, consts):
    ...
    eve_mass_add_attr = client.mk_eve_attr(id_=consts.EveAttr.mass_addition)
    eve_prop_effect = client.mk_eve_effect(id_=consts.EveEffect.mod_bonus_afterburner, cat_id=consts.EveEffCat.active)
    eve_ship = client.mk_eve_ship(attrs={eve_speed_attr.id: 455, eve_mass_attr.id: 1050000})
    eve_prop_item = client.mk_eve_item(
    ...
    eve_sw_mod = client.mk_eve_effect_mod(
        ...
        affector_attr_id=eve_speed_boost_attr_sw.id,
        affectee_attr_id=eve_speed_attr.id)
    ...
    # 1951.95
    api_ship.update()
    assert api_ship.attrs[eve_speed_attr.id].dogma == approx(1833.828883)
...
def test_sig_mod_stacking(client, consts):
    ...
    eve_sig_affector_attr_prop = client.mk_eve_attr(id_=consts.EveAttr.sig_radius_bonus)
    eve_sig_affectee_attr = client.mk_eve_attr(id_=consts.EveAttr.sig_radius, stackable=False)
    eve_prop_effect = client.mk_eve_effect(
        id_=consts.EveEffect.mod_bonus_microwarpdrive,
        cat_id=consts.EveEffCat.active)
    eve_ship = client.mk_eve_ship(attrs={eve_sig_affectee_attr.id: 32})
    eve_prop_item = client.mk_eve_item(
        attrs={eve_sig_affector_attr_prop.id: 450},
        eff_ids=[eve_prop_effect.id],
        defeff_id=eve_prop_effect.id)
    eve_sig_affector_attr_rig = client.mk_eve_attr()
    eve_rig_mod = client.mk_eve_effect_mod(
        func=consts.EveModFunc.item,
        dom=consts.EveModDom.ship,
        op=consts.EveModOp.post_percent,
        affector_attr_id=eve_sig_affector_attr_rig.id,
        affectee_attr_id=eve_sig_affectee_attr.id)
    eve_rig_effect = client.mk_eve_effect(cat_id=consts.EveEffCat.passive, mod_info=[eve_rig_mod])
    eve_rig_item = client.mk_eve_item(attrs={eve_sig_affector_attr_rig.id: 10}, eff_ids=[eve_rig_effect.id])
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship.id)
    ...
    # 193.6
    api_ship.update()
    assert api_ship.attrs[eve_sig_affectee_attr.id].dogma == approx(191.296512)
    api_ship_mod_prop = api_ship.mods.find_by_affector_attr(
...
def test_speed_mod_mass_zero(client, consts):
    ...
    eve_mass_attr = client.mk_eve_attr(id_=consts.EveAttr.mass)
    eve_prop_effect = client.mk_eve_effect(id_=consts.EveEffect.mod_bonus_afterburner, cat_id=consts.EveEffCat.active)
    eve_ship = client.mk_eve_ship(attrs={eve_speed_attr.id: 455, eve_mass_attr.id: 0})
    eve_prop_item = client.mk_eve_item(
...
def test_speed_mod_mass_changed(client, consts):
    ...
    eve_rig_mod = client.mk_eve_effect_mod(
        ...
        dom=consts.EveModDom.ship,
        op=consts.EveModOp.post_percent,
        affector_attr_id=eve_mass_boost_attr.id,
        affectee_attr_id=eve_mass_attr.id)
    eve_rig_effect = client.mk_eve_effect(cat_id=consts.EveEffCat.passive, mod_info=[eve_rig_mod])
    ...
    assert api_ship1_item.attrs[eve_speed_attr.id].dogma == approx(1049.435484)
    assert len(api_ship1_item.mods[eve_speed_attr.id].find_by_affector_item(affector_item_id=api_prop_item.id)) == 1
    # Action
    api_rig_item = api_fit.add_rig(type_id=eve_rig_item.id)
    # Verification
    api_ship1_item.update()
    assert api_ship1_item.attrs[eve_mass_attr.id].dogma == approx(3100000)
    assert api_ship1_item.attrs[eve_speed_attr.id].dogma == approx(752.217742)
    assert len(api_ship1_item.mods[eve_speed_attr.id].find_by_affector_item(affector_item_id=api_prop_item.id)) == 1
    # Action
    api_rig_item.remove()
    # Verification
    ...
    assert api_ship2_item.attrs[eve_speed_attr.id].dogma == approx(965.192308)
    assert len(api_ship2_item.mods[eve_speed_attr.id].find_by_affector_item(affector_item_id=api_prop_item.id)) == 1
    # Action
    api_rig_item = api_fit.add_rig(type_id=eve_rig_item.id)
    # Verification
    api_ship2_item.update()
    assert api_ship2_item.attrs[eve_mass_attr.id].dogma == approx(3120000)
    assert api_ship2_item.attrs[eve_speed_attr.id].dogma == approx(692.596154)
    assert len(api_ship2_item.mods[eve_speed_attr.id].find_by_affector_item(affector_item_id=api_prop_item.id)) == 1
    # Action
    api_rig_item.remove()
    # Verification
...
def test_speed_mod_boost_changed(client, consts):
    ...
    eve_implant_mod = client.mk_eve_effect_mod(
        ...
        dom=consts.EveModDom.ship,
        op=consts.EveModOp.post_percent,
        affector_attr_id=eve_boost_booster_attr.id,
        affectee_attr_id=eve_speed_boost_attr.id)
    ...
    # Verification
    api_ship1_item.update()
    assert api_ship1_item.attrs[eve_speed_attr.id].dogma == approx(1108.879032)
    assert len(api_ship1_item.mods[eve_speed_attr.id].find_by_affector_item(affector_item_id=api_prop_item.id)) == 1
    ...
    # Verification
    api_ship2_item.update()
    assert api_ship2_item.attrs[eve_speed_attr.id].dogma == approx(1019.711538)
    assert len(api_ship2_item.mods[eve_speed_attr.id].find_by_affector_item(affector_item_id=api_prop_item.id)) == 1
    assert api_prop_item.update().attrs[eve_speed_boost_attr.id].dogma == approx(148.5)
    # Action
    api_implant_item.remove()
...
def test_speed_mod_thrust_changed(client, consts):
    ...
    eve_rig_mod = client.mk_eve_effect_mod(
        ...
        dom=consts.EveModDom.ship,
        op=consts.EveModOp.post_percent,
        affector_attr_id=eve_thrust_booster_attr.id,
        affectee_attr_id=eve_thrust_attr.id)
    eve_rig_effect = client.mk_eve_effect(cat_id=consts.EveEffCat.passive, mod_info=[eve_rig_mod])
