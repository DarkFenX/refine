...
def test_force_run(client, consts):
    ...
    assert api_item.effects[eve_effect.id] == (True, consts.EffMode.force_run)
    api_item.change_mod(effect_modes={eve_effect.id: consts.EffMode.full_compliance})
    api_item.update()
    assert api_item.attrs[eve_tgt_attr.id].dogma == approx(100)
    assert api_item.effects[eve_effect.id] == (False, consts.EffMode.full_compliance)
    # Ghost state has priority, even force-run effects do not affect anything
    api_item.change_mod(state=consts.State.ghost, effect_modes={eve_effect.id: consts.EffMode.force_run})
    api_item.update()
    assert api_item.attrs[eve_tgt_attr.id].dogma == approx(100)
    assert api_item.effects[eve_effect.id] == (False, consts.EffMode.force_run)
