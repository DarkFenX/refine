...
def test_calculation(client, consts):
    ...
    eve_mod3 = client.mk_eve_effect_mod(
    ...
    # If everything is processed properly, item1 will multiply attr2 by attr1 on self, resulting in
    # 20 * 5 = 100, then apply it as percentage modifier on ship's (item2) attr3, resulting in 150 +
    # 100% = 300, then it is applied to all entities assigned to ship, including item3, to theirs
    # attr4 as percentage modifier again - so final result is 12.5 + 300% = 50
    assert api_rig.update().attrs[eve_attr4.id].dogma == approx(50)
