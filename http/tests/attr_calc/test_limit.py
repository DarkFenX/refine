...
def test_default_max(client, consts):
    ...
    api_item.update()
    assert api_item.attrs[eve_limitee_attr.id].dogma == approx(5)
...
def test_unmodified_max(client, consts):
    ...
    eve_effect = client.mk_eve_effect(mod_info=[eve_mod])
    eve_item = client.mk_eve_item(
        attrs={eve_limiter_attr.id: 2, eve_limitee_attr.id: 3, eve_affector_attr.id: 6},
        eff_ids=[eve_effect.id])
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    ...
    api_item.update()
    assert api_item.attrs[eve_limitee_attr.id].dogma == approx(2)
    api_mod = api_item.mods.find_by_affector_attr(
        affectee_attr_id=eve_limitee_attr.id, affector_attr_id=eve_limiter_attr.id).one()
    assert api_mod.op == consts.ApiModOp.max_limit
...
def test_modified(client, consts):
    ...
    eve_mod2 = client.mk_eve_effect_mod(
        ...
        op=consts.EveModOp.post_mul,
        affector_attr_id=eve_affector_attr.id,
        affectee_attr_id=eve_limiter_attr.id)
    eve_effect = client.mk_eve_effect(mod_info=[eve_mod1, eve_mod2])
    eve_item = client.mk_eve_item(
        attrs={eve_limiter_attr.id: 0.1, eve_limitee_attr.id: 3, eve_affector_attr.id: 6},
        eff_ids=[eve_effect.id])
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    ...
    api_item.update()
    assert api_item.attrs[eve_limitee_attr.id].dogma == approx(0.6)
    api_mod = api_item.mods.find_by_affector_attr(
        affectee_attr_id=eve_limitee_attr.id, affector_attr_id=eve_limiter_attr.id).one()
    assert api_mod.op == consts.ApiModOp.max_limit
...
def test_update(client, consts):
    ...
    eve_limitee_mod = client.mk_eve_effect_mod(
        ...
        affector_attr_id=eve_affector_attr.id,
        affectee_attr_id=eve_limitee_attr.id)
    eve_limitee_effect = client.mk_eve_effect(mod_info=[eve_limitee_mod])
    eve_limitee_item = client.mk_eve_item(
        attrs={eve_limiter_attr.id: 2, eve_limitee_attr.id: 3, eve_affector_attr.id: 6},
        eff_ids=[eve_limitee_effect.id])
    eve_limiter_mod = client.mk_eve_effect_mod(
        ...
        affector_attr_id=eve_affector_attr.id,
        affectee_attr_id=eve_limiter_attr.id)
    eve_limiter_effect = client.mk_eve_effect(mod_info=[eve_limiter_mod])
    eve_limiter_item = client.mk_eve_item(attrs={eve_affector_attr.id: 3.5}, eff_ids=[eve_limiter_effect.id])
    eve_ship = client.mk_eve_ship()
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_fit.set_ship(type_id=eve_ship.id)
    api_limitee_item = api_fit.add_rig(type_id=eve_limitee_item.id)
    ...
    assert api_limitee_item.attrs[eve_limitee_attr.id].dogma == approx(2)
    ...
    # Action
    api_limiter_item = api_fit.add_implant(type_id=eve_limiter_item.id)
    ...
    api_limitee_item.update()
    assert api_limitee_item.attrs[eve_limitee_attr.id].dogma == approx(7)
    assert api_limitee_item.mods.find_by_affector_attr(
        affectee_attr_id=eve_limitee_attr.id, affector_attr_id=eve_limiter_attr.id).one().applied_val == approx(7)
    # Action
...
def test_unlimited(client, consts):
    ...
    eve_effect = client.mk_eve_effect(mod_info=[eve_mod])
    eve_item = client.mk_eve_item(
        attrs={eve_limiter_attr.id: 20, eve_limitee_attr.id: 3, eve_affector_attr.id: 6},
        eff_ids=[eve_effect.id])
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    ...
    # exposed
    api_item.update()
    assert api_item.attrs[eve_limitee_attr.id].dogma == approx(18)
    assert len(api_item.mods.find_by_affector_attr(
        affectee_attr_id=eve_limitee_attr.id, affector_attr_id=eve_limiter_attr.id)) == 0
