...
def test_fail_single(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(include=[consts.ApiValType.launched_standup_support_fighters])
    assert api_val.passed is False
    assert api_val.details.launched_standup_support_fighters.used == 1
    assert api_val.details.launched_standup_support_fighters.total == 0
    assert len(api_val.details.launched_standup_support_fighters.users) == 1
    assert api_fighter.id in api_val.details.launched_standup_support_fighters.users
...
def test_fail_multiple_ship(client, consts):
    ...
    api_val = api_fit.validate(include=[consts.ApiValType.launched_standup_support_fighters])
    assert api_val.passed is False
    assert api_val.details.launched_standup_support_fighters.used == 2
    assert api_val.details.launched_standup_support_fighters.total == 1
    assert len(api_val.details.launched_standup_support_fighters.users) == 2
    assert api_fighter1.id in api_val.details.launched_standup_support_fighters.users
    assert api_fighter2.id in api_val.details.launched_standup_support_fighters.users
...
def test_fractional_fighter_type(client, consts):
    ...
    api_val = api_fit.validate(include=[consts.ApiValType.launched_standup_support_fighters])
    assert api_val.passed is False
    assert api_val.details.launched_standup_support_fighters.used == 3
    assert api_val.details.launched_standup_support_fighters.total == 2
    assert len(api_val.details.launched_standup_support_fighters.users) == 3
    assert api_fighter1.id in api_val.details.launched_standup_support_fighters.users
    assert api_fighter2.id in api_val.details.launched_standup_support_fighters.users
    assert api_fighter3.id in api_val.details.launched_standup_support_fighters.users
...
def test_no_ship(client, consts):
    ...
    assert api_val.passed is False
    assert api_val.details.launched_standup_support_fighters.used == 1
    assert api_val.details.launched_standup_support_fighters.total is None
    assert len(api_val.details.launched_standup_support_fighters.users) == 1
    assert api_fighter.id in api_val.details.launched_standup_support_fighters.users
...
def test_no_attr_total(client, consts):
    ...
    # irrelevant
    eve_ftr_type_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_sq_is_standup_support)
    eve_total_attr_id = consts.EveAttr.ftr_standup_support_slots
    eve_fighter_id = client.mk_eve_item(cat_id=consts.EveItemCat.fighter, attrs={eve_ftr_type_attr_id: 1})
    eve_ship_id = client.mk_eve_ship(attrs={eve_total_attr_id: 0})
    client.create_sources()
    api_sol = client.create_sol()
