...
def test_unloaded_user(client, consts):
    # Unloaded subsystems still take slot
    eve_output_attr_id = client.mk_eve_attr(id_=consts.EveAttr.max_subsystems)
    eve_ship_id = client.mk_eve_ship(attrs={eve_output_attr_id: 0})
...
def test_no_attr_output(client, consts):
    ...
    # irrelevant
    eve_output_attr_id = consts.EveAttr.max_subsystems
    eve_subsystem_id = client.mk_eve_item()
    eve_ship_id = client.mk_eve_ship(attrs={eve_output_attr_id: 0})
    client.create_sources()
    api_sol = client.create_sol()
...
def test_t3c_slot_override(client, consts):
    ...
    eve_output_attr_id = client.mk_eve_attr(id_=consts.EveAttr.max_subsystems)
    eve_subsystem_id = client.mk_eve_item()
    eve_ship_id = client.mk_eve_ship(grp_id=consts.EveItemGrp.strategic_cruiser, attrs={eve_output_attr_id: 5})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_fit.set_ship(type_id=eve_ship_id)
    ...
    api_val = api_fit.validate(include=[consts.ApiValType.subsystem_slots])
    assert api_val.passed is False
    assert api_val.details.subsystem_slots.used == 5
    assert api_val.details.subsystem_slots.total == 4
    assert len(api_val.details.subsystem_slots.users) == 5
