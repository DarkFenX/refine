...
def test_fail_multiple_ship(client, consts):
    ...
    api_val = api_fit.validate(include=[consts.ApiValType.low_slots])
    assert api_val.passed is False
    assert api_val.details.low_slots.used == 3
    assert api_val.details.low_slots.total == 1
    assert api_val.details.low_slots.users == sorted([api_module2.id, api_module3.id])
...
def test_holes(client, consts):
    ...
    assert api_val.details.low_slots.users == [api_module1.id]
    # Action
    api_fit.add_mod(
        type_id=eve_module_id,
        rack=consts.ApiRack.low,
