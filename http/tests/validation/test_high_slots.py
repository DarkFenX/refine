...
def test_fail_single(client, consts):
    ...
    api_module = api_fit.add_mod(type_id=eve_module_id, rack=consts.ApiRack.high, state=consts.ApiState.offline)
    # Verification
    api_val = api_fit.validate(include=[consts.ApiValType.high_slots])
    assert api_val.passed is False
    assert api_val.details.high_slots.used == 1
    assert api_val.details.high_slots.total == 0
...
def test_fail_multiple_ship(client, consts):
    ...
    api_module2 = api_fit.add_mod(type_id=eve_module_id, rack=consts.ApiRack.high, state=consts.ApiState.offline)
    api_module3 = api_fit.add_mod(type_id=eve_module_id, rack=consts.ApiRack.high, state=consts.ApiState.offline)
    # Verification
    api_val = api_fit.validate(include=[consts.ApiValType.high_slots])
    assert api_val.passed is False
    assert api_val.details.high_slots.used == 3
    assert api_val.details.high_slots.total == 1
    assert api_val.details.high_slots.users == sorted([api_module2.id, api_module3.id])
...
def test_holes(client, consts):
    ...
    api_val = api_fit.validate(include=[consts.ApiValType.high_slots])
    assert api_val.passed is False
    assert api_val.details.high_slots.used == 6
    assert api_val.details.high_slots.total == 3
    assert api_val.details.high_slots.users == [api_module1.id]
    # Action
    api_fit.add_mod(
        type_id=eve_module_id,
        rack=consts.ApiRack.high,
    ...
    api_val = api_fit.validate(include=[consts.ApiValType.high_slots])
    assert api_val.passed is False
    assert api_val.details.high_slots.used == 7
    assert api_val.details.high_slots.total == 3
    assert api_val.details.high_slots.users == [api_module1.id]
    # Action
    api_module1.remove()
...
def test_no_ship(client, consts):
    ...
    assert api_val.passed is False
    assert api_val.details.high_slots.used == 1
    assert api_val.details.high_slots.total is None
    assert api_val.details.high_slots.users == [api_module.id]
...
def test_no_attr_total(client, consts):
    ...
    # irrelevant
    eve_total_attr_id = consts.EveAttr.hi_slots
    eve_module_id = client.mk_eve_item()
    eve_ship_id = client.mk_eve_ship(attrs={eve_total_attr_id: 0})
    client.create_sources()
    api_sol = client.create_sol()
