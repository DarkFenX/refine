...
def test_fail_single(client, consts):
    ...
    assert len(api_val.details.high_slots.users) == 1
    assert api_module.id in api_val.details.high_slots.users
...
def test_fail_multiple_ship(client, consts):
    ...
    api_val = api_fit.validate(include=[consts.ApiValType.high_slots])
    assert api_val.passed is False
    assert api_val.details.high_slots.used == 2
    assert api_val.details.high_slots.total == 1
    assert len(api_val.details.high_slots.users) == 1
    assert api_module2.id in api_val.details.high_slots.users
...
def test_holes(client, consts):
    ...
    assert len(api_val.details.high_slots.users) == 1
    assert api_module1.id in api_val.details.high_slots.users
    # Action
    api_fit.add_mod(
        type_id=eve_module_id,
        rack=consts.ApiRack.high,
