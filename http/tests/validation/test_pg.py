...
def test_fail_multiple(client, consts):
    ...
    assert api_val.details.pg.users[api_mod1.id] == 50
    assert api_val.details.pg.users[api_mod2.id] == 100
...
def test_no_ship(client, consts):
    ...
    assert api_val.details.pg.output == 0
    assert len(api_val.details.pg.users) == 1
    assert api_val.details.pg.users[api_mod.id] == 5
...
def test_non_positive(client, consts):
    ...
    assert api_val.details.pg.output == 125
    assert len(api_val.details.pg.users) == 1
    assert api_val.details.pg.users[api_mod2.id] == 150
