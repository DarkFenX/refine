...
def test_fail_multiple_ship(client, consts):
    ...
    assert len(api_val.details.rig_slots.users) == 2
    assert api_rig1.id in api_val.details.rig_slots.users
    assert api_rig2.id in api_val.details.rig_slots.users
...
def test_no_attr_output(client, consts):
    ...
    # irrelevant
    eve_output_attr_id = consts.EveAttr.upgrade_slots_left
    eve_rig_id = client.mk_eve_item()
    eve_ship_id = client.mk_eve_ship(attrs={eve_output_attr_id: 0})
    client.create_sources()
    api_sol = client.create_sol()
