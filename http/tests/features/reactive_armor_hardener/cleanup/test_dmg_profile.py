...
def test_rah_to_rah(client, consts):
    ...
    api_fit = api_sol.create_fit(rah_incoming_dmg=(1, 1, 0, 0))
    ...
    assert api_rah.attrs[eve_basic_info.res_therm_attr_id].dogma == approx(0.53)
    ...
    api_ship.update()
    assert api_ship.attrs[eve_basic_info.res_em_attr_id].dogma == approx(0.265)
    assert api_ship.attrs[eve_basic_info.res_therm_attr_id].dogma == approx(0.3445)
    ...
    assert api_rah.attrs[eve_basic_info.res_kin_attr_id].dogma == approx(0.53)
    assert api_rah.attrs[eve_basic_info.res_expl_attr_id].dogma == approx(0.53)
    api_ship.update()
    ...
    assert api_ship.attrs[eve_basic_info.res_kin_attr_id].dogma == approx(0.3975)
    assert api_ship.attrs[eve_basic_info.res_expl_attr_id].dogma == approx(0.477)
...
def test_default_to_default(client, consts):
    ...
    assert api_rah.attrs[eve_basic_info.res_therm_attr_id].dogma == approx(0.7)
    assert api_rah.attrs[eve_basic_info.res_kin_attr_id].dogma == approx(1)
    assert api_rah.attrs[eve_basic_info.res_expl_attr_id].dogma == approx(1)
    api_ship.update()
    assert api_ship.attrs[eve_basic_info.res_em_attr_id].dogma == approx(0.35)
    assert api_ship.attrs[eve_basic_info.res_therm_attr_id].dogma == approx(0.455)
    assert api_ship.attrs[eve_basic_info.res_kin_attr_id].dogma == approx(0.75)
    assert api_ship.attrs[eve_basic_info.res_expl_attr_id].dogma == approx(0.9)
    # Action
    api_sol.set_default_incoming_dmg(dmg_profile=(0, 0, 1, 1))
    # Verification
    api_rah.update()
    assert api_rah.attrs[eve_basic_info.res_em_attr_id].dogma == approx(1)
    assert api_rah.attrs[eve_basic_info.res_therm_attr_id].dogma == approx(1)
    assert api_rah.attrs[eve_basic_info.res_kin_attr_id].dogma == approx(0.7)
    assert api_rah.attrs[eve_basic_info.res_expl_attr_id].dogma == approx(0.7)
    api_ship.update()
    assert api_ship.attrs[eve_basic_info.res_em_attr_id].dogma == approx(0.5)
    assert api_ship.attrs[eve_basic_info.res_therm_attr_id].dogma == approx(0.65)
    assert api_ship.attrs[eve_basic_info.res_kin_attr_id].dogma == approx(0.525)
    assert api_ship.attrs[eve_basic_info.res_expl_attr_id].dogma == approx(0.63)
...
def test_rah_to_default(client, consts):
    ...
    eve_ship_id = make_eve_ship(client=client, basic_info=eve_basic_info, resos=(0.5, 0.65, 0.75, 0.9))
    client.create_sources()
    api_sol = client.create_sol(default_incoming_dmg=(0, 0, 1, 1))
    api_fit = api_sol.create_fit(rah_incoming_dmg=(1, 1, 0, 0))
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
    api_rah = api_fit.add_mod(type_id=eve_rah_id, state=consts.ApiState.active)
    # Verification
