...
def test_attr_changed_shift_amount(client, consts):
    ...
    # Verification - results for both RAHs should be reset, despite only one having its attr updated
    api_rah1.update()
    assert api_rah1.attrs[eve_basic_info.res_em_attr_id].dogma == approx(0.91)
    assert api_rah1.attrs[eve_basic_info.res_therm_attr_id].dogma == approx(0.79)
    assert api_rah1.attrs[eve_basic_info.res_kin_attr_id].dogma == approx(0.79)
    assert api_rah1.attrs[eve_basic_info.res_expl_attr_id].dogma == approx(0.91)
    api_rah2.update()
    assert api_rah2.attrs[eve_basic_info.res_em_attr_id].dogma == approx(0.88)
    assert api_rah2.attrs[eve_basic_info.res_therm_attr_id].dogma == approx(0.82)
    assert api_rah2.attrs[eve_basic_info.res_kin_attr_id].dogma == approx(0.82)
    assert api_rah2.attrs[eve_basic_info.res_expl_attr_id].dogma == approx(0.88)
    api_ship.update()
    assert api_ship.attrs[eve_basic_info.res_em_attr_id].dogma == approx(0.4055828)
