...
def test_no_chance_val(client, consts):
    ...
    eve_effect = client.mk_eve_effect(chance_attr_id=eve_chance_attr_id, mod_info=[eve_mod])
    eve_booster = client.mk_eve_item(attrs={eve_affector_attr.id: 25}, eff_ids=[eve_effect.id])
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
...
def test_no_str_val(client, consts):
    ...
    eve_booster = client.mk_eve_item(attrs={eve_chance_attr.id: 0.4}, eff_ids=[eve_effect.id])
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_booster = api_fit.add_booster(type_id=eve_booster.id)
