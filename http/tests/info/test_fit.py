...
def test_error_no_fit_full(client, consts):
    ...
    client.create_sources()
    api_ss = client.create_ss()
    resp = client.get_fit_request(ss_id=api_ss.id, fit_id='1', fit_info_mode=consts.ApiFitInfoMode.full).send()
...
def test_error_no_fit_id(client, consts):
    ...
    client.create_sources()
    api_ss = client.create_ss()
    resp = client.get_fit_request(ss_id=api_ss.id, fit_id='1', fit_info_mode=consts.ApiFitInfoMode.id).send()
    resp.check(status_code=404, json_predicate={'code': 'COR-003', 'message': 'core library error: fit 1 not found'})
...
def test_error_no_fit_malformed(client):
    ...
    client.create_sources()
    api_ss = client.create_ss()
    resp = client.get_fit_request(ss_id=api_ss.id, fit_id='abc').send()
