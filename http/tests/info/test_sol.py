...
def test_fleet(client):
    ...
    api_fleet.remove()
    assert len(api_sol.update().fits) == 0
...
def test_fit_item(client):
    ...
    with raises(AttributeError):
        api_sol.update().fits[api_fit.id].character  # pylint: disable=W0106
...
def test_sw_effect(client):
    ...
    api_item.remove()
    with raises(AttributeError):
        api_sol.update().sw_effects  # pylint: disable=W0106
...
def test_proj_effect(client):
    ...
    api_item.remove()
    with raises(AttributeError):
        api_sol.update().proj_effects  # pylint: disable=W0106


def test_error_no_sol_full(client, consts):
    # Check case when there is no solar system with such ID
    client.create_sources()
    resp = client.get_sol_request(sol_id='1', sol_info_mode=consts.ApiSolInfoMode.full).send()
...
def test_error_no_sol_id(client, consts):
    # Check case when there is no solar system with such ID
    client.create_sources()
    resp = client.get_sol_request(sol_id='1', sol_info_mode=consts.ApiSolInfoMode.id).send()
    resp.check(status_code=404, json_predicate={'code': 'SOL-001', 'message': 'no solar system with ID "1"'})
