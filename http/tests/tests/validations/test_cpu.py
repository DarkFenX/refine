...
def test_fail_single(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(cpu=True))
...
def test_known_failures(client, consts):
    ...
    api_module1 = api_fit.add_module(type_id=eve_module1_id, state=consts.ApiModuleState.online)
    # Verification
    api_val = api_fit.validate(options=FitValOptions(cpu=(True, [api_module1.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
    ...
    api_val = api_fit.validate(options=FitValOptions(cpu=(True, [api_module2.id])))
    assert api_val.passed is False
    assert api_val.details.cpu.used == 250
    assert api_val.details.cpu.max == 125
    assert api_val.details.cpu.users == {api_module1.id: 150}
    api_val = api_fit.validate(options=FitValOptions(cpu=(True, [api_module1.id, api_module2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    api_val = api_fit.validate(options=FitValOptions(cpu=(True, [api_module1.id, api_other.id, api_module2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
