...
def test_bundled(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(charge_group=True), flip_order=False)
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(charge_group=True), flip_order=True)
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
...
def test_separate(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(charge_group=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
...
def test_multiple_different(client, consts):
    ...
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
    api_module.change_module(charge_type_id=eve_charge3_id)
    # Verification
    api_val = api_fit.validate(options=FitValOptions(charge_group=True), flip_order=False)
    assert api_val.passed is False
...
def test_known_failures(client, consts):
    ...
    api_module3 = api_fit.add_module(type_id=eve_module_id, charge_type_id=eve_charge2_id)
    # Verification
    api_val = api_fit.validate(options=FitValOptions(charge_group=(True, [api_module2.charge.id])), flip_order=False)
    assert api_val.passed is False
    assert api_val.details.charge_group == {api_module3.charge.id: (api_module3.id, eve_grp2_id, [eve_grp1_id])}
    # Action
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(charge_group=(True, [api_module2.charge.id])), flip_order=True)
    assert api_val.passed is False
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(charge_group=(True, [api_module3.charge.id])), flip_order=False)
    assert api_val.passed is False
    ...
    api_module3.change_module(charge_type_id=eve_charge3_id)
    # Verification
    api_val = api_fit.validate(options=FitValOptions(charge_group=(True, [api_module3.charge.id])), flip_order=True)
    assert api_val.passed is False
    ...
    api_val = api_fit.validate(
        options=FitValOptions(charge_group=(True, [api_module2.charge.id, api_module3.charge.id])),
        flip_order=False)
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
