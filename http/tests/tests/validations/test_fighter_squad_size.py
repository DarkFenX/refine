...
def test_add_remove(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(fighter_squad_size=True))
...
def test_known_failures(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(fighter_squad_size=(True, [api_fighter1.id])))
    assert api_val.passed is False
    assert api_val.details.fighter_squad_size == {api_fighter2.id: (11, 9)}
    api_val = api_fit.validate(options=FitValOptions(fighter_squad_size=(True, [api_fighter2.id])))
    assert api_val.passed is False
    assert api_val.details.fighter_squad_size == {api_fighter1.id: (10, 9)}
    api_val = api_fit.validate(options=FitValOptions(fighter_squad_size=(True, [api_fighter1.id, api_fighter2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
