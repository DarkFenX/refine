...
def test_fail_single(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(high_slot_count=True))
...
def test_fail_multiple_ship(client, consts):
    ...
    assert api_val.details.high_slot_count.max == 1
    assert api_val.details.high_slot_count.users == sorted([api_module2.id, api_module3.id])
...
def test_holes(client, consts):
    ...
    assert api_val.details.high_slot_count.users == [api_module1.id]
    # Action
    api_fit.add_module(
        type_id=eve_module_id,
        rack=consts.ApiRack.high,
...
def test_known_failures(client, consts):
    ...
    api_val = api_fit.validate(options=FitValOptions(high_slot_count=(True, [api_module1.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
    ...
    # Verification - check case with KF specified, but used <= max being true
    api_val = api_fit.validate(options=FitValOptions(high_slot_count=(True, [api_module2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
    ...
    api_val = api_fit.validate(options=FitValOptions(high_slot_count=(True, [api_module3.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
    ...
    api_val = api_fit.validate(options=FitValOptions(high_slot_count=(True, [api_module4.id])))
    assert api_val.passed is False
    assert api_val.details.high_slot_count.used == 7
    assert api_val.details.high_slot_count.max == 3
    assert api_val.details.high_slot_count.users == [api_module3.id]
    api_val = api_fit.validate(options=FitValOptions(high_slot_count=(True, [api_module3.id, api_module4.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
...
def test_criterion_item_kind(client, consts):
    ...
    api_val = api_fit.validate(options=FitValOptions(high_slot_count=(True, [api_module.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
