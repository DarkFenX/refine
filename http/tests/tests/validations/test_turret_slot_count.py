...
def test_fail_single(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(turret_slot_count=True))
...
def test_known_failures(client, consts):
    ...
    api_module2 = api_fit.add_module(type_id=eve_module_id, state=consts.ApiModuleState.offline)
    # Verification
    api_val = api_fit.validate(options=FitValOptions(turret_slot_count=(True, [api_module1.id])))
    assert api_val.passed is False
    assert api_val.details.turret_slot_count.used == 2
    assert api_val.details.turret_slot_count.max == 1
    assert api_val.details.turret_slot_count.users == [api_module2.id]
    api_val = api_fit.validate(options=FitValOptions(turret_slot_count=(True, [api_module2.id])))
    assert api_val.passed is False
    assert api_val.details.turret_slot_count.used == 2
    assert api_val.details.turret_slot_count.max == 1
    assert api_val.details.turret_slot_count.users == [api_module1.id]
    api_val = api_fit.validate(options=FitValOptions(turret_slot_count=(True, [api_module1.id, api_module2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
