...
def test_main(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(sec_zone_online=True))
...
def test_known_failures(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(sec_zone_online=(True, [api_lowsec_service.id])))
    assert api_val.passed is False
    assert api_val.details.sec_zone_online.zone == consts.ApiSecZone.hisec
    assert api_val.details.sec_zone_online.items == {
        api_nullsec_service.id: sorted([consts.ApiSecZone.nullsec, consts.ApiSecZone.wspace, consts.ApiSecZone.hazard])}
    api_val = api_fit.validate(options=FitValOptions(sec_zone_online=(True, [api_nullsec_service.id])))
    assert api_val.passed is False
    assert api_val.details.sec_zone_online.zone == consts.ApiSecZone.hisec
