...
def test_fail_single(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(launched_standup_support_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_standup_support_fighter_count.used == 1
    assert api_val.details.launched_standup_support_fighter_count.max == 0
...
def test_fail_multiple_ship(client, consts):
    ...
    assert api_val.details.launched_standup_support_fighter_count.max == 1
    assert api_val.details.launched_standup_support_fighter_count.users == sorted([api_fighter1.id, api_fighter2.id])
...
def test_known_failures(client, consts):
    ...
    api_fighter2 = api_fit.add_fighter(type_id=eve_fighter_id, state=consts.ApiMinionState.in_space)
    # Verification
    api_val = api_fit.validate(options=FitValOptions(launched_standup_support_fighter_count=(True, [api_fighter1.id])))
    assert api_val.passed is False
    assert api_val.details.launched_standup_support_fighter_count.used == 2
    assert api_val.details.launched_standup_support_fighter_count.max == 1
    assert api_val.details.launched_standup_support_fighter_count.users == [api_fighter2.id]
    api_val = api_fit.validate(options=FitValOptions(launched_standup_support_fighter_count=(True, [api_fighter2.id])))
    assert api_val.passed is False
    assert api_val.details.launched_standup_support_fighter_count.used == 2
    assert api_val.details.launched_standup_support_fighter_count.max == 1
    assert api_val.details.launched_standup_support_fighter_count.users == [api_fighter1.id]
    api_val = api_fit.validate(options=FitValOptions(
        launched_standup_support_fighter_count=(True, [api_fighter1.id, api_fighter2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    api_val = api_fit.validate(options=FitValOptions(
        launched_standup_support_fighter_count=(True, [api_fighter1.id, api_other.id, api_fighter2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
...
def test_fractional_fighter_type(client, consts):
    ...
    assert api_val.details.launched_standup_support_fighter_count.max == 2
    assert api_val.details.launched_standup_support_fighter_count.users == sorted([
...
def test_no_ship(client, consts):
    eve_ftr_type_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_sq_is_standup_support)
    client.mk_eve_attr(id_=consts.EveAttr.ftr_standup_support_slots)
    eve_fighter_id = client.mk_eve_item(attrs={eve_ftr_type_attr_id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    ...
    assert api_val.passed is False
    assert api_val.details.launched_standup_support_fighter_count.used == 1
    assert api_val.details.launched_standup_support_fighter_count.max is None
    assert api_val.details.launched_standup_support_fighter_count.users == [api_fighter.id]
...
def test_criterion_item_kind(client, consts):
    ...
    api_val = api_fit.validate(options=FitValOptions(launched_standup_support_fighter_count=(True, [api_fighter.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
