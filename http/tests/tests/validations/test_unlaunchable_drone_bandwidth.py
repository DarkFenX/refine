...
def test_fail_single(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(unlaunchable_drone_bandwidth=True))
...
def test_known_failures(client, consts):
    ...
    api_drone2 = api_fit.add_drone(type_id=eve_drone2_id, state=consts.ApiMinionState.in_bay)
    # Verification
    api_val = api_fit.validate(options=FitValOptions(unlaunchable_drone_bandwidth=(True, [api_drone2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
    ...
    api_val = api_fit.validate(options=FitValOptions(unlaunchable_drone_bandwidth=(True, [api_drone3.id])))
    assert api_val.passed is False
    assert api_val.details.unlaunchable_drone_bandwidth.max == approx(20)
