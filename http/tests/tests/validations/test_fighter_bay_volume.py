...
def test_fail_single(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(fighter_bay_volume=True))
...
def test_known_failures(client, consts):
    ...
    api_fighter1 = api_fit.add_fighter(type_id=eve_fighter1_id)
    # Verification
    api_val = api_fit.validate(options=FitValOptions(fighter_bay_volume=(True, [api_fighter1.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
    ...
    api_val = api_fit.validate(options=FitValOptions(fighter_bay_volume=(True, [api_fighter2.id])))
    assert api_val.passed is False
    assert api_val.details.fighter_bay_volume.used == approx(18600)
    assert api_val.details.fighter_bay_volume.max == approx(8000)
    assert api_val.details.fighter_bay_volume.users == {api_fighter1.id: approx(9000)}
    api_val = api_fit.validate(options=FitValOptions(fighter_bay_volume=(True, [api_fighter1.id, api_fighter2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
