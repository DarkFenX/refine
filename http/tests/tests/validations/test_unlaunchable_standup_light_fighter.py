...
def test_fail_single(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=ValOptions(unlaunchable_standup_light_fighter=True))
...
def test_known_failures(client, consts):
    ...
    api_fighter2 = api_fit.add_fighter(type_id=eve_fighter_id, state=consts.ApiMinionState.in_bay)
    # Verification
    api_val = api_fit.validate(options=ValOptions(unlaunchable_standup_light_fighter=(True, [api_fighter1.id])))
    assert api_val.passed is False
    assert api_val.details.unlaunchable_standup_light_fighter.max == 0
    assert api_val.details.unlaunchable_standup_light_fighter.users == [api_fighter2.id]
    api_val = api_fit.validate(options=ValOptions(unlaunchable_standup_light_fighter=(True, [api_fighter2.id])))
    assert api_val.passed is False
    assert api_val.details.unlaunchable_standup_light_fighter.max == 0
...
def test_criterion_item_kind(client, consts):
    ...
    api_val = api_fit.validate(options=ValOptions(unlaunchable_standup_light_fighter=(True, [api_fighter.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
