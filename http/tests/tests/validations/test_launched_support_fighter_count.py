...
def test_fail_single(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(launched_support_fighter_count=True))
...
def test_known_failures(client, consts):
    ...
    api_fighter2 = api_fit.add_fighter(type_id=eve_fighter_id, state=consts.ApiMinionState.in_space)
    # Verification
    api_val = api_fit.validate(options=FitValOptions(launched_support_fighter_count=(True, [api_fighter1.id])))
    assert api_val.passed is False
    assert api_val.details.launched_support_fighter_count.used == 2
    assert api_val.details.launched_support_fighter_count.max == 1
    assert api_val.details.launched_support_fighter_count.users == [api_fighter2.id]
    api_val = api_fit.validate(options=FitValOptions(launched_support_fighter_count=(True, [api_fighter2.id])))
    assert api_val.passed is False
    assert api_val.details.launched_support_fighter_count.used == 2
    assert api_val.details.launched_support_fighter_count.max == 1
...
def test_criterion_item_kind(client, consts):
    ...
    api_val = api_fit.validate(options=FitValOptions(launched_support_fighter_count=(True, [api_fighter.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
