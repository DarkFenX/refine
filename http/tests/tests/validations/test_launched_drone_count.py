...
def test_fail_single(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=FitValOptions(launched_drone_count=True))
...
def test_known_failures(client, consts):
    ...
    api_drone2 = api_fit.add_drone(type_id=eve_drone_id, state=consts.ApiMinionState.in_space)
    # Verification
    api_val = api_fit.validate(options=FitValOptions(launched_drone_count=(True, [api_drone1.id])))
    assert api_val.passed is False
    assert api_val.details.launched_drone_count.used == 2
    assert api_val.details.launched_drone_count.max == 1
    assert api_val.details.launched_drone_count.users == [api_drone2.id]
    api_val = api_fit.validate(options=FitValOptions(launched_drone_count=(True, [api_drone2.id])))
    assert api_val.passed is False
    assert api_val.details.launched_drone_count.used == 2
    assert api_val.details.launched_drone_count.max == 1
    assert api_val.details.launched_drone_count.users == [api_drone1.id]
    api_val = api_fit.validate(options=FitValOptions(launched_drone_count=(True, [api_drone1.id, api_drone2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
