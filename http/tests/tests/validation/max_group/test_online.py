...
def test_same_value_modules(client, consts):
    ...
    api_module1 = api_fit.add_module(type_id=eve_module_id, state=consts.ApiModuleState.online)
...
def test_different_values(client, consts):
    ...
    api_val = api_fit.validate(options=ValOptions(max_group_online=True))
    assert api_val.passed is False
    assert api_val.details.max_group_online == {eve_grp_id: [3, {api_module2.id: 2}]}
    # Action
    api_module1.remove()
    # Verification
    api_val = api_fit.validate(options=ValOptions(max_group_online=True))
...
def test_known_failures(client, consts):
    ...
    api_module2 = api_fit.add_module(type_id=eve_module_id, state=consts.ApiModuleState.online)
    # Verification
    api_val = api_fit.validate(options=ValOptions(max_group_online=(True, [api_module1.id])))
    assert api_val.passed is False
    assert api_val.details.max_group_online == {eve_grp_id: [2, {api_module2.id: 1}]}
    api_val = api_fit.validate(options=ValOptions(max_group_online=(True, [api_module2.id])))
    assert api_val.passed is False
    assert api_val.details.max_group_online == {eve_grp_id: [2, {api_module1.id: 1}]}
    api_val = api_fit.validate(options=ValOptions(max_group_online=(True, [api_module1.id, api_module2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    api_val = api_fit.validate(options=ValOptions(
        max_group_online=(True, [api_module1.id, api_other.id, api_module2.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
...
def test_modified(client, consts):
    ...
    api_module2 = api_fit.add_module(type_id=eve_module2_id, state=consts.ApiModuleState.online)
    # Verification
    assert api_module1.update().attrs[eve_limit_attr_id].modified == approx(0.6)
    assert api_module2.update().attrs[eve_limit_attr_id].modified == approx(1.4)
    api_val = api_fit.validate(options=ValOptions(max_group_online=True))
    assert api_val.passed is False
    assert api_val.details.max_group_online == {eve_grp_id: [2, {api_module1.id: 1, api_module2.id: 1}]}
    # Action
    ...
    # Verification
    assert api_module1.update().attrs[eve_limit_attr_id].modified == approx(1.6)
    assert api_module2.update().attrs[eve_limit_attr_id].modified == approx(2.4)
    api_val = api_fit.validate(options=ValOptions(max_group_online=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
...
def test_mutation_limit_priority(client, consts):
    ...
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_fit.add_module(type_id=eve_base_module_id, state=consts.ApiModuleState.online)
    api_module2 = api_fit.add_module(type_id=eve_base_module_id, state=consts.ApiModuleState.online)
    # Verification
...
def test_mutation_limit_inheritance(client, consts):
    ...
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_fit.add_module(type_id=eve_other_module_id, state=consts.ApiModuleState.online)
    api_module2 = api_fit.add_module(type_id=eve_base_module_id, state=consts.ApiModuleState.online)
    # Verification
    assert api_module2.update().attrs[eve_limit_attr_id].modified == approx(1)
    api_val = api_fit.validate(options=ValOptions(max_group_online=True))
    ...
    api_val = api_fit.validate(options=ValOptions(max_group_online=True))
    assert api_val.passed is False
    assert api_val.details.max_group_online == {eve_grp2_id: [2, {api_module2.id: 1}]}
    # Action
    api_module2.change_module(mutation=None)
    # Verification
    assert api_module2.update().attrs[eve_limit_attr_id].modified == approx(1)
...
def test_no_attr(client, consts):
    eve_grp_id = client.mk_eve_item_group()
    eve_limit_attr_id = consts.EveAttr.max_group_online
    eve_module_id = client.mk_eve_item(grp_id=eve_grp_id, attrs={eve_limit_attr_id: 1})
    eve_ship_id = client.mk_eve_ship()
    client.create_sources()
    api_sol = client.create_sol()
...
def test_not_loaded(client, consts):
    client.mk_eve_attr(id_=consts.EveAttr.max_group_online)
...
def test_criterion_module_state(client, consts):
    ...
    assert api_val.details.max_group_online == {eve_grp_id: [2, {api_module1.id: 1, api_module2.id: 1}]}
    # Action
    api_module2.change_module(state=consts.ApiModuleState.offline)
    # Verification
    api_val = api_fit.validate(options=ValOptions(max_group_online=True))
    assert api_val.passed is True
    with check_no_field():
