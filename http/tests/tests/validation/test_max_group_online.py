...
def test_same(client, consts):
    ...
    api_module1 = api_fit.add_mod(type_id=eve_module_id, state=consts.ApiState.online)
    api_module2 = api_fit.add_mod(type_id=eve_module_id, state=consts.ApiState.online)
    # Verification
...
def test_mutation_limit_priority(client, consts):
    ...
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_fit.add_mod(type_id=eve_base_module_id, state=consts.ApiState.online)
    api_module2 = api_fit.add_mod(type_id=eve_base_module_id, state=consts.ApiState.online)
    # Verification
...
def test_mutation_limit_inheritance(client, consts):
    ...
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_fit.add_mod(type_id=eve_other_module_id, state=consts.ApiState.online)
    api_module2 = api_fit.add_mod(type_id=eve_base_module_id, state=consts.ApiState.online)
    # Verification
    assert api_module2.update().attrs[eve_limit_attr_id].extra == approx(1)
    api_val = api_fit.validate(include=[consts.ApiValType.max_group_online])
...
def test_criterion_state(client, consts):
    ...
    assert api_val.details.max_group_online == {eve_grp_id: [2, {api_module1.id: 1, api_module2.id: 1}]}
    # Action
    api_module2.change_mod(state=consts.ApiState.offline)
    # Verification
    api_val = api_fit.validate(include=[consts.ApiValType.max_group_online])
    assert api_val.passed is True
    with check_no_field():
