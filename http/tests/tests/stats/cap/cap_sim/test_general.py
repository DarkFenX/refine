...
def test_error_fatality(client, consts):
    ...
    eve_ship1_id = client.mk_eve_ship(attrs={eve_ship_amount_attr_id: 500})
    ...
    api_nosf_stats = api_nosf.get_stats(options=ItemStatsOptions(cap_sim=True))
    assert api_nosf_stats.cap_sim is None
    # Action
    api_ship.change_ship(type_id=eve_ship2_id)
    # Verification - attempt to get stats of ship which is not loaded fails whole stat batch
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(cap_sim=True))
    assert api_fit_stats.cap_sim is None
    api_src_ship_stats = api_ship.get_stats(options=ItemStatsOptions(cap_sim=True))
    assert api_src_ship_stats.cap_sim is None
    # Action
    api_ship.remove()
    # Verification - attempt to get stats of a fit without ship fails whole stat batch
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(cap_sim=True))
    assert api_fit_stats.cap_sim is None
