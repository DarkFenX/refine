...
def test_nosf(client, consts):
    # Active nosfs do not have separate category, and are considered are cap consumers (since they
    # technically are)
    ...
    # Verification
    api_options = [
        StatsOptionCapBalance(src_kinds=StatCapSrcKinds(default=True)),
        StatsOptionCapBalance(src_kinds=StatCapSrcKinds(consumers=True))]
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(cap_balance=(True, api_options)))
    assert api_fit_stats.cap_balance == [approx(21), approx(21)]
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(cap_balance=(True, api_options)))
    assert api_ship_stats.cap_balance == [approx(21), approx(21)]
    # Action
    api_nosf.change_module(type_id=eve_nosf2_id)
    # Verification - non-overridden nosfs also contribute to cap balance
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(cap_balance=(True, api_options)))
    assert api_fit_stats.cap_balance == [approx(12), approx(12)]
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(cap_balance=(True, api_options)))
    assert api_ship_stats.cap_balance == [approx(12), approx(12)]
