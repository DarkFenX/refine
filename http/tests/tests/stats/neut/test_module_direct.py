...
def test_projection(client, consts):
    ...
    api_tgt_ship = api_tgt_fit.set_ship(type_id=eve_tgt_ship_id, coordinates=(0, 20770, 0))
    api_fleet = api_sol.create_fleet()
...
def test_application(client, consts):
    ...
    eve_module_id = client.mk_eve_item(
        attrs={
            eve_neut_amount_attr_id: 4400, eve_cycle_time_attr_id: 48000,
            eve_optimal_attr_id: 36000, eve_falloff_attr_id: 18000, eve_neut_sig_res_attr_id: 8000},
        eff_ids=[eve_effect_id],
        defeff_id=eve_effect_id)
    eve_tgt_ship_id = client.mk_eve_ship(attrs={eve_sig_radius_attr_id: 200, eve_cap_attr_id: 10000})
    ...
    assert api_fleet_stats.remote_nps == [approx(4.583333), approx(2.291667)]
    api_src_fit_stats = api_src_fit.get_stats(options=FitStatsOptions(remote_nps=(True, [
        StatsOptionFitRemoteNps(projectee_item_id=api_tgt_ship.id),
        StatsOptionFitRemoteNps(projectee_item_id=api_tgt_drone.id)])))
    assert api_src_fit_stats.remote_nps == [approx(4.583333), approx(2.291667)]
    ...
    assert api_src_module_proj_stats.remote_nps == [approx(2.291667), approx(1.145833)]
    api_src_module_nonproj_stats = api_src_module_nonproj.get_stats(options=ItemStatsOptions(remote_nps=(True, [
        StatsOptionItemRemoteNps(projectee_item_id=api_tgt_ship.id),
        StatsOptionItemRemoteNps(projectee_item_id=api_tgt_drone.id)])))
    assert api_src_module_nonproj_stats.remote_nps == [approx(2.291667), approx(1.145833)]
    # Action
    ...
    assert api_fleet_stats.remote_nps.one() == approx(13.75)
    api_src_fit_stats = api_src_fit.get_stats(options=FitStatsOptions(
        remote_nps=(True, [StatsOptionFitRemoteNps(projectee_item_id=api_tgt_drone.id)])))
    assert api_src_fit_stats.remote_nps.one() == approx(13.75)
    api_src_module_proj_stats = api_src_module_proj.get_stats(options=ItemStatsOptions(
        remote_nps=(True, [StatsOptionItemRemoteNps(projectee_item_id=api_tgt_drone.id)])))
    assert api_src_module_proj_stats.remote_nps.one() == approx(6.875)
    api_src_module_nonproj_stats = api_src_module_nonproj.get_stats(options=ItemStatsOptions(
        remote_nps=(True, [StatsOptionItemRemoteNps(projectee_item_id=api_tgt_drone.id)])))
    assert api_src_module_nonproj_stats.remote_nps.one() == approx(6.875)
    # Action
