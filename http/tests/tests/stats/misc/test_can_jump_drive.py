...
def test_ship_jump_modified(client, consts):
    ...
    eve_ship_id = client.mk_eve_ship(attrs={eve_jump_attr_id: 0, eve_warp_attr_id: 0})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
    # Verification
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(can_jump_drive=True))
    assert api_fit_stats.can_jump_drive.one() is True
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(can_jump_drive=True))
    assert api_ship_stats.can_jump_drive.one() is True
    # Action
    api_rig = api_fit.add_rig(type_id=eve_rig_id)
    # Verification
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(can_jump_drive=True))
    assert api_fit_stats.can_jump_drive.one() is False
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(can_jump_drive=True))
...
def test_ship_aggro(client, consts):
    ...
    # Verification - aggro modules do not prevent jumping
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(can_jump_drive=True))
    assert api_fit_stats.can_jump_drive.one() is True
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(can_jump_drive=True))
    assert api_ship_stats.can_jump_drive.one() is True
...
def test_ship_jump_values(client, consts):
    eve_jump_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_drive_jumping)
    eve_warp_attr_id = client.mk_eve_attr(id_=consts.EveAttr.warp_scramble_status)
    eve_ship1_id = client.mk_eve_ship(attrs={eve_warp_attr_id: 0, eve_jump_attr_id: -100})
    eve_ship2_id = client.mk_eve_ship(attrs={eve_warp_attr_id: 0, eve_jump_attr_id: 0})
    eve_ship3_id = client.mk_eve_ship(attrs={eve_warp_attr_id: 0, eve_jump_attr_id: 100})
...
def test_ship_jump_no_attr(client, consts):
    ...
    eve_warp_attr_id = client.mk_eve_attr(id_=consts.EveAttr.warp_scramble_status)
    eve_ship_id = client.mk_eve_ship(attrs={eve_jump_attr_id: 1, eve_warp_attr_id: 0})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
...
def test_ship_warp_values(client, consts):
    eve_jump_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_drive_jumping)
    eve_warp_attr_id = client.mk_eve_attr(id_=consts.EveAttr.warp_scramble_status)
    eve_ship1_id = client.mk_eve_ship(attrs={eve_jump_attr_id: 0, eve_warp_attr_id: -100})
    eve_ship2_id = client.mk_eve_ship(attrs={eve_jump_attr_id: 0, eve_warp_attr_id: 0})
    eve_ship3_id = client.mk_eve_ship(attrs={eve_jump_attr_id: 0, eve_warp_attr_id: 100})
...
def test_ship_warp_no_attr(client, consts):
    ...
    eve_ship_id = client.mk_eve_ship(attrs={eve_jump_attr_id: 0, eve_warp_attr_id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
...
def test_ship_absent(client, consts):
    ...
    # Verification
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(can_jump_drive=True))
    assert api_fit_stats.can_jump_drive.one() is None
...
def test_ship_not_loaded(client, consts):
    ...
    assert api_fit_stats.can_jump_drive.one() is None
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(can_jump_drive=True))
    assert api_ship_stats.can_jump_drive.one() is None


def test_struct(client, consts):
    eve_jump_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_drive_jumping)
    eve_warp_attr_id = client.mk_eve_attr(id_=consts.EveAttr.warp_scramble_status)
    eve_struct_id = client.mk_eve_struct(attrs={eve_jump_attr_id: 1, eve_warp_attr_id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
...
def test_incorrect_item_kind(client, consts):
    ...
    # Verification
    api_drone_stats = api_fighter.get_stats(options=ItemStatsOptions(can_jump_drive=True))
    assert api_drone_stats.can_jump_drive.one() is None


def test_not_requested(client, consts):
    eve_jump_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_drive_jumping)
    eve_warp_attr_id = client.mk_eve_attr(id_=consts.EveAttr.warp_scramble_status)
    eve_ship_id = client.mk_eve_ship(attrs={eve_jump_attr_id: 100, eve_warp_attr_id: 100})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
    # Verification
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(can_jump_drive=False))
    with check_no_field():
        api_fit_stats.can_jump_drive  # noqa: B018
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(can_jump_drive=False))
    with check_no_field():
        api_ship_stats.can_jump_drive  # noqa: B018
