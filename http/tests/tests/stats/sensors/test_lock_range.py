...
def test_ship_modified(client, consts):
    ...
    eve_mod = client.mk_eve_effect_mod(
        ...
        affectee_attr_id=eve_lock_range_attr_id)
...
def test_ship_no_value(client, consts):
    client.mk_eve_attr(id_=consts.EveAttr.max_target_range)
...
def test_ship_absent(client, consts):
    ...
    # Verification
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(lock_range=True))
    assert api_fit_stats.lock_range is None
...
def test_ship_not_loaded(client, consts):
    ...
    assert api_fit_stats.lock_range is None
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(lock_range=True))
    assert api_ship_stats.lock_range is None
...
def test_struct_modified(client, consts):
    ...
    eve_mod_effect_id = client.mk_eve_effect(mod_info=[eve_mod])
    eve_rig_id = client.mk_eve_item(attrs={eve_mod_attr_id: 25}, eff_ids=[eve_mod_effect_id])
    eve_struct_id = client.mk_eve_struct(attrs={eve_lock_range_attr_id: 100})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
...
def test_drone(client, consts):
    eve_lock_range_attr_id = client.mk_eve_attr(id_=consts.EveAttr.max_target_range)
    eve_drone_id = client.mk_eve_drone(attrs={eve_lock_range_attr_id: 1350})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_drone = api_fit.add_drone(type_id=eve_drone_id)
    # Verification
    api_drone_stats = api_drone.get_stats(options=ItemStatsOptions(lock_range=True))
    assert api_drone_stats.lock_range is None
...
def test_fighter_modified(client, consts):
    ...
    eve_buff_id = client.mk_eve_buff(
        ...
        item_mods=[client.mk_eve_buff_mod(attr_id=eve_lock_range_attr_id)])
    ...
    eve_fighter_id = client.mk_eve_fighter(attrs={eve_lock_range_attr_id: 873, eve_max_count_attr_id: 9})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
...
def test_other(client, consts):
    eve_lock_range_attr_id = client.mk_eve_attr(id_=consts.EveAttr.max_target_range)
    eve_module_id = client.mk_eve_item(attrs={eve_lock_range_attr_id: 100})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
