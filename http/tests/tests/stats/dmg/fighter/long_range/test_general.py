...
def test_state(client, consts):
    ...
    # TODO: autocharge with ignore state should deal DPS
    ...
    # Verification
    # TODO: autocharge DPS should scale with fighter count
    api_fleet_dmg_stats = api_fleet.get_stats(options=FleetStatsOptions(dmg=True)).dmg.one()
    assert api_fleet_dmg_stats.dps == [0, 0, 0, approx(709.397135)]
    assert api_fleet_dmg_stats.volley == [0, 0, 0, approx(6229.84375)]
    api_fit_dmg_stats = api_fit.get_stats(options=FitStatsOptions(dmg=True)).dmg.one()
    assert api_fit_dmg_stats.dps == [0, 0, 0, approx(709.397135)]
    assert api_fit_dmg_stats.volley == [0, 0, 0, approx(6229.84375)]
    ...
    assert api_fighter_dmg_stats.dps == [0, 0, 0, approx(709.397135)]
    assert api_fighter_dmg_stats.volley == [0, 0, 0, approx(6229.84375)]
