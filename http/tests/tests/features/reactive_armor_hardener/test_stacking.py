...
def test_regular_resist(client, consts):
    ...
    eve_resmod_attr_id = client.mk_eve_attr()
    eve_resmod_mods = [
    ...
    eve_resmod_effect_id = client.mk_eve_effect(cat_id=consts.EveEffCat.passive, mod_info=eve_resmod_mods)
    eve_resmod_id = client.mk_eve_item(attrs={eve_resmod_attr_id: -15}, eff_ids=[eve_resmod_effect_id])
    ...
    assert api_ship.attrs[eve_basic_info.res_therm_attr_id].modified == approx(0.5110625)
    assert api_ship.attrs[eve_basic_info.res_kin_attr_id].modified == approx(0.52275)
    assert api_ship.attrs[eve_basic_info.res_expl_attr_id].modified == approx(0.501075)
...
def test_damage_control(client, consts):
    ...
    api_ship.update()
    assert api_ship.attrs[eve_basic_info.res_em_attr_id].modified == approx(0.425)
    assert api_ship.attrs[eve_basic_info.res_therm_attr_id].modified == approx(0.5164858)
    assert api_ship.attrs[eve_basic_info.res_kin_attr_id].modified == approx(0.5348237)
    assert api_ship.attrs[eve_basic_info.res_expl_attr_id].modified == approx(0.5126481)
