...
def test_valid_effect_to_valid_effect_matching(client, consts):
...
def test_valid_effect_to_valid_effect_different(client, consts):
    eve_d1 = client.mk_eve_data()
    eve_d2 = client.mk_eve_data()
...
def test_absent_effect_to_valid_effect(client, consts):
    eve_d1 = client.mk_eve_data()
    eve_d2 = client.mk_eve_data()
    ...
    with check_no_field():
        api_booster1.side_effects  # noqa: B018
    # Action
    api_booster1.change_booster(side_effects={eve_effect_id: True})
    # Verification
    assert api_ship.update().attrs[eve_affectee_attr_id].modified == approx(100)
    api_booster1.update()
    with check_no_field():
    ...
    with check_no_field():
        api_booster1.side_effects  # noqa: B018
    # Action
    api_booster2.change_booster(side_effects={eve_effect_id: True})
    # Verification
    assert api_ship.update().attrs[eve_affectee_attr_id].modified == approx(100)
    api_booster1.update()
    with check_no_field():
...
def test_not_loaded_booster_to_valid_effect(client, consts):
    eve_d1 = client.mk_eve_data()
    eve_d2 = client.mk_eve_data()
    eve_chance_attr_id = client.mk_eve_attr(datas=[eve_d2])
    eve_affector_attr_id = client.mk_eve_attr(datas=[eve_d2])
