...
def test_insignificant_earlier_ops(client, consts):
    ...
    api_post_mul_mod = api_affectee.mods[eve_affectee_attr_id].one()
    assert api_post_mul_mod.op == consts.ApiModOp.post_mul
...
def test_insignificant_op_collision(client, consts):
    ...
    api_post_mul_mod = api_affectee.mods[eve_affectee_attr_id].one()
    assert api_post_mul_mod.op == consts.ApiModOp.post_mul
    assert api_post_mul_mod.initial_str == approx(0)
    assert api_post_mul_mod.stacking_mult is None
    assert api_post_mul_mod.applied_str == approx(0)
