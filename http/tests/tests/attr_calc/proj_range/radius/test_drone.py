...
def test_mutation(client, consts):
    ...
    # Drone 1 items
    eve_mid_drone1_base1_id = make_eve_drone(radius=25)
    eve_mid_drone1_base2_id = make_eve_drone(radius=200)
    ...
    eve_mid_drone1_mutator2_id = client.mk_eve_mutator(items=[([eve_mid_drone1_base2_id], eve_mid_drone1_mutated1_id)])
    eve_affectee_ship_id = client.mk_eve_ship(attrs={eve_radius_attr_id: 1000, eve_affectee_attr_id: 500})
    client.create_sources()
    api_sol = client.create_sol()
    api_affector_fit = api_sol.create_fit()
    ...
    api_affectee_fit = api_sol.create_fit()
    api_affectee_ship = api_affectee_fit.set_ship(type_id=eve_affectee_ship_id)
    api_affector_module.change_module(add_projs=[(api_mid_drone1.id, range_s2s_to_api(val=11000))])
    api_mid_drone1.change_drone(add_projs=[(api_affectee_ship.id, range_s2s_to_api(val=11000))])
    # Verification - outgoing projection
    api_mid_drone1.update()
    assert api_mid_drone1.projs[api_affectee_ship.id] == (12025, 11000)
    assert api_affectee_ship.update().attrs[eve_affectee_attr_id].dogma == approx(287.5)
    # Verification - incoming projection
    assert api_affector_module.update().projs[api_mid_drone1.id] == (11025, 11000)
    ...
    # Verification - outgoing projection
    api_mid_drone1.update()
    assert api_mid_drone1.projs[api_affectee_ship.id] == (12025, 10525)
    assert api_affectee_ship.update().attrs[eve_affectee_attr_id].dogma == approx(273.390775)
    # Verification - incoming projection
    assert api_affector_module.update().projs[api_mid_drone1.id] == (11025, 10525)
    assert api_mid_drone1.attrs[eve_affectee_attr_id].dogma == approx(546.781550)
    # Action
    api_mid_drone1.change_drone(type_id=eve_mid_drone1_base2_id)
    # Verification - outgoing projection
    api_mid_drone1.update()
    assert api_mid_drone1.projs[api_affectee_ship.id] == (12025, 10025)
    assert api_affectee_ship.update().attrs[eve_affectee_attr_id].dogma == approx(258.343703)
    # Verification - incoming projection
    assert api_affector_module.update().projs[api_mid_drone1.id] == (11025, 10025)
    assert api_mid_drone1.attrs[eve_affectee_attr_id].dogma == approx(516.687405)
    # Action
    ...
    # Verification - outgoing projection
    api_mid_drone1.update()
    assert api_mid_drone1.projs[api_affectee_ship.id] == (12025, 10825)
    assert api_affectee_ship.update().attrs[eve_affectee_attr_id].dogma == approx(282.327887)
    # Verification - incoming projection
    assert api_affector_module.update().projs[api_mid_drone1.id] == (11025, 10825)
    assert api_mid_drone1.attrs[eve_affectee_attr_id].dogma == approx(564.655773)
