...
def test_proj_add_change_outgoing(client, consts):
    ...
    assert api_affectee_ship.attrs[eve_affectee_attr1_id].modified == approx(286.763177)
    assert api_affectee_ship.attrs[eve_affectee_attr2_id].modified == approx(286.763177)
    # Action
...
def test_proj_add_change_incoming(client, consts):
    ...
    eve_affectee_fighter_id = client.mk_eve_fighter(attrs={eve_radius_attr_id: 100, eve_affectee_attr_id: 1000})
    client.create_sources()
    api_sol = client.create_sol()
    api_affector_fit = api_sol.create_fit()
...
def test_switch_type_id_outgoing(client, consts):
    ...
    api_affectee_ship.update()
    assert api_affectee_ship.attrs[eve_affectee_attr1_id].modified == approx(273.390775)
    assert api_affectee_ship.attrs[eve_affectee_attr2_id].modified == approx(273.390775)
    # Action
    ...
    api_affectee_ship.update()
    assert api_affectee_ship.attrs[eve_affectee_attr1_id].modified == approx(288.236112)
    assert api_affectee_ship.attrs[eve_affectee_attr2_id].modified == approx(288.236112)
    # Action
...
def test_switch_type_id_incoming(client, consts):
    ...
    eve_affector_drone_id = client.mk_eve_drone(
        ...
        eff_ids=[eve_effect_id],
        defeff_id=eve_effect_id)
    eve_affectee_fighter1_id = client.mk_eve_fighter(attrs={eve_radius_attr_id: 25, eve_affectee_attr_id: 1000})
    eve_affectee_fighter2_id = client.mk_eve_fighter(attrs={eve_radius_attr_id: 500, eve_affectee_attr_id: 1000})
    eve_affectee_fighter3_id = client.mk_eve_fighter(attrs={eve_affectee_attr_id: 1000})
    ...
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(546.78155)
    # Action
    ...
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(576.472223)
    # Action
...
def test_switch_src_incoming(client, consts):
    ...
    client.mk_eve_fighter(datas=[eve_d3], id_=eve_affectee_fighter_id, attrs={eve_affectee_attr_id: 1000})
    client.create_sources()
    api_sol = client.create_sol(data=eve_d1)
    api_affector_fit = api_sol.create_fit()
    api_affector_drone = api_affector_fit.add_drone(
...
def test_modified_radius_outgoing(client, consts):
    ...
    assert api_affector_fighter.attrs[eve_radius_attr_id].modified == approx(1000)
    api_affectee_ship.update()
    assert api_affectee_ship.attrs[eve_affectee_attr1_id].modified == approx(287.5)
    assert api_affectee_ship.attrs[eve_affectee_attr2_id].modified == approx(287.5)
    # Action
    ...
    api_affector_fighter.update()
    assert api_affector_fighter.projs[api_affectee_ship.id] == (approx(11025), approx(11000))
    assert api_affector_fighter.attrs[eve_radius_attr_id].modified == approx(25)
    api_affectee_ship.update()
    assert api_affectee_ship.attrs[eve_affectee_attr1_id].modified == approx(287.5)
    assert api_affectee_ship.attrs[eve_affectee_attr2_id].modified == approx(287.5)
    # Action
...
def test_modified_radius_incoming(client, consts):
    ...
    eve_affector_drone_id = client.mk_eve_drone(
        ...
        eff_ids=[eve_effect_id],
        defeff_id=eve_effect_id)
    eve_affectee_fighter_id = client.mk_eve_fighter(
        attrs={eve_radius_attr_id: 25, eve_affectee_attr_id: 1000},
        srqs={eve_skill_id: 1})
    eve_radius_mod = client.mk_eve_effect_mod(
        func=consts.EveModFunc.own_srq,
    ...
    assert api_affector_drone.update().projs[api_affectee_fighter.id] == (approx(11025), approx(11000))
    api_affectee_fighter.update()
    assert api_affectee_fighter.attrs[eve_radius_attr_id].modified == approx(1000)
    assert api_affectee_fighter.attrs[eve_affectee_attr_id].modified == approx(575)
    # Action
    api_rig.remove()
    # Verification
    assert api_affector_drone.update().projs[api_affectee_fighter.id] == (approx(11025), approx(11000))
    api_affectee_fighter.update()
    assert api_affectee_fighter.attrs[eve_radius_attr_id].modified == approx(25)
    assert api_affectee_fighter.attrs[eve_affectee_attr_id].modified == approx(575)
    # Action
