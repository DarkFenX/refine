...
def test_affected_child_ship_multiple(client, consts):
    ...
    assert api_module1.update().attrs[eve_affectee_attr_id].dogma == approx(260)
    assert api_module2.update().attrs[eve_affectee_attr_id].dogma == approx(260)
...
def test_unaffected_other_group(client, consts):
    ...
    eve_sw_effect_id = client.mk_eve_item(
        ...
        eff_ids=[eve_effect_id],
        defeff_id=eve_effect_id)
    eve_module_id = client.mk_eve_item(grp_id=eve_grp2_id, attrs={eve_affectee_attr_id: 200})
    eve_ship_id = client.mk_eve_ship()
    client.create_sources()
    api_sol = client.create_sol()
...
def test_unaffected_root_char(client, consts):
    ...
    eve_sw_effect_id = client.mk_eve_item(
        ...
        eff_ids=[eve_effect_id],
        defeff_id=eve_effect_id)
    eve_char_id = client.mk_eve_item(grp_id=eve_grp_id, attrs={eve_affectee_attr_id: 200})
    client.create_sources()
    api_sol = client.create_sol()
    api_sol.add_sw_effect(type_id=eve_sw_effect_id)
    api_fit = api_sol.create_fit()
