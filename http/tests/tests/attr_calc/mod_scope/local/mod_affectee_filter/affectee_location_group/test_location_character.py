...
def test_unaffected_other_group(client, consts):
    ...
    eve_effect_id = client.mk_eve_effect(mod_info=[eve_mod])
    eve_affector_item_id = client.mk_eve_item(attrs={eve_affector_attr_id: 20}, eff_ids=[eve_effect_id])
    eve_affectee_item_id = client.mk_eve_item(attrs={eve_affectee_attr_id: 100}, grp_id=eve_grp2_id)
    eve_char_item_id = client.mk_eve_item()
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
...
def setup_switch_type_id_root_test(*, client, consts):
    ...
    eve_affector_item_id = client.mk_eve_item(attrs={eve_affector_attr_id: 20}, eff_ids=[eve_effect_id])
    eve_affectee_item_id = client.mk_eve_item(grp_id=eve_grp_id, attrs={eve_affectee_attr_id: 100})
    eve_root_loaded_id = client.mk_eve_item()
    eve_root_not_loaded_id = client.alloc_item_id()
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_rig = api_fit.add_rig(type_id=eve_affector_item_id)
    api_affectee = api_fit.add_implant(type_id=eve_affectee_item_id)
    return eve_affectee_attr_id, eve_root_loaded_id, eve_root_not_loaded_id, api_fit, api_rig, api_affectee
...
def test_switch_type_id_root_loaded_to_not_loaded_remove(client, consts):
    ...
    api_character = api_fit.set_character(type_id=eve_root_loaded_id)
    # Verification
    assert api_affectee.update().attrs[eve_affectee_attr_id].dogma == approx(120)
    # Action
    api_character.change_character(type_id=eve_root_not_loaded_id)
    # Verification
    assert api_affectee.update().attrs[eve_affectee_attr_id].dogma == approx(120)
    # Action
    api_rig.remove()
...
def test_switch_type_id_root_not_loaded_to_loaded_remove(client, consts):
    (eve_affectee_attr_id,
     ...
     api_rig,
     api_affectee) = setup_switch_type_id_root_test(client=client, consts=consts)
    api_character = api_fit.set_character(type_id=eve_root_not_loaded_id)
    # Verification
    assert api_affectee.update().attrs[eve_affectee_attr_id].dogma == approx(120)
    # Action
    api_character.change_character(type_id=eve_root_loaded_id)
    # Verification
    assert api_affectee.update().attrs[eve_affectee_attr_id].dogma == approx(120)
    # Action
    api_rig.remove()
