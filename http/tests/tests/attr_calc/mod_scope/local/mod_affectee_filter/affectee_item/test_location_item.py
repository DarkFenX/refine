...
def test_unaffected_root(client, consts):
    # Check that item which is root of location does not affect root items of other locations and
    # children of its location
    eve_affector_attr_id = client.mk_eve_attr()
    eve_affectee_attr_id = client.mk_eve_attr()
    eve_mod = client.mk_eve_effect_mod(
        func=consts.EveModFunc.item,
...
def test_unaffected_child(client, consts):
    # Check that item which belongs to a location does not affect top item of its location and other
    # items within its location
    eve_affector_attr_id = client.mk_eve_attr()
    eve_affectee_attr_id = client.mk_eve_attr()
    eve_mod = client.mk_eve_effect_mod(
        func=consts.EveModFunc.item,
