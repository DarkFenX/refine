...
def test_add_with_change_without_remove(client, consts):
    ...
    assert api_affectee_drone.update().attrs[eve_affectee_attr_id].modified == approx(500)
    api_affector_module.change_module(add_projs=[api_affectee_ship.id])
    assert api_affectee_drone.update().attrs[eve_affectee_attr_id].modified == approx(350)
    api_affectee_ship.change_ship(coordinates=(0, 0, 0))
    assert api_affectee_drone.update().attrs[eve_affectee_attr_id].modified == approx(200)
...
def test_add_without_change_with_remove(client, consts):
    ...
    eve_affector_module_id = client.mk_eve_item(
        ...
        defeff_id=eve_effect_id)
    eve_ship_id = client.mk_eve_ship()
    eve_affectee_fighter_id = client.mk_eve_fighter(attrs={eve_affectee_attr_id: 500}, srqs={eve_skill_id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_affector_fit = api_sol.create_fit()
    ...
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(500)
    api_affector_module.change_module(add_projs=[api_affectee_ship.id])
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(200)
    api_affectee_ship.change_ship(coordinates=(15000, 0, 0))
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(350)
    api_affector_module.change_module(rm_projs=[api_affectee_ship.id])
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(500)
...
def test_add_with_change_with_remove(client, consts):
    ...
    assert api_affectee_drone.update().attrs[eve_affectee_attr_id].modified == approx(481.25)
...
def test_value_change_optimal(client, consts):
    ...
    assert api_affectee_drone.update().attrs[eve_affectee_attr_id].modified == approx(247.731075)
...
def test_value_change_falloff(client, consts):
    ...
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(350)
    api_affector_rig = api_affector_fit.add_rig(type_id=eve_affector_rig_id)
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(279.539826)
    api_affector_rig.remove()
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(350)
