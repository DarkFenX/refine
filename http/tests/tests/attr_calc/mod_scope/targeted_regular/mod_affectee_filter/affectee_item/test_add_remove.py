...
def test_root_add_afee_afor_proj_state_remove_afor_afee(client, consts):
    ...
    api_affector_fit = api_sol.create_fit()
    api_affector_module = api_affector_fit.add_mod(type_id=eve_affector_module_id, state=consts.ApiModuleState.online)
    assert api_affectee_ship.update().attrs[eve_affectee_attr_id].dogma == approx(-2)
...
def test_child_add_afor_afee_proj_remove_state_proj_fit(client, consts):
    ...
    eve_affector_module_id = client.mk_eve_item(
        ...
        eff_ids=[eve_effect_id],
        defeff_id=eve_effect_id)
    eve_affectee_drone_id = client.mk_eve_ship(attrs={eve_affectee_attr_id: -2})
    client.create_sources()
    api_sol = client.create_sol()
    api_affector_fit = api_sol.create_fit()
    api_affector_module = api_affector_fit.add_mod(type_id=eve_affector_module_id, state=consts.ApiModuleState.active)
...
def test_child_add_afee_afor_proj_state_remove_afor_afee(client, consts):
    ...
    eve_affector_module_id = client.mk_eve_item(
        ...
        eff_ids=[eve_effect_id],
        defeff_id=eve_effect_id)
    eve_affectee_fighter_id = client.mk_eve_ship(attrs={eve_affectee_attr_id: -2})
    client.create_sources()
    api_sol = client.create_sol()
    api_affectee_fit = api_sol.create_fit()
