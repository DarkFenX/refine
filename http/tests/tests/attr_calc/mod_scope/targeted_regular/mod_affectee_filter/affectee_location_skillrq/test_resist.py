...
def test_resisted_value_change(client, consts):
    ...
    assert api_affectee_module.update().attrs[eve_affectee_attr_id].modified == approx(380)
    api_affectee_rig = api_affectee_fit.add_rig(type_id=eve_affectee_rig_id)
    assert api_affectee_module.update().attrs[eve_affectee_attr_id].modified == approx(410)
