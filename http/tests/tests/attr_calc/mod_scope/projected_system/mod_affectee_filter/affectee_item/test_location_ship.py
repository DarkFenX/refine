...
def test_unaffected_other_location(client, consts):
    ...
    assert api_struct.update().attrs[eve_affectee_attr_id].modified == approx(100)
...
def test_unaffected_other_fit(client, consts):
    ...
    api_proj_effect = api_sol.add_proj_effect(type_id=eve_proj_effect_id)
    api_proj_effect.change_proj_effect(add_proj_item_ids=[api_ship1.id])
    assert api_ship2.update().attrs[eve_affectee_attr_id].modified == approx(100)
...
def test_replace_proj(client, consts):
    ...
    api_proj_effect = api_sol.add_proj_effect(type_id=eve_proj_effect_id)
    api_proj_effect.change_proj_effect(add_proj_item_ids=[api_ship1.id])
    assert api_ship1.update().attrs[eve_affectee_attr_id].modified == approx(120)
    api_ship2 = api_fit.set_ship(type_id=eve_ship2_id)
    assert api_ship2.update().attrs[eve_affectee_attr_id].modified == approx(50)
    api_proj_effect.change_proj_effect(add_proj_item_ids=[api_ship2.id])
    assert api_ship2.update().attrs[eve_affectee_attr_id].modified == approx(60)
