...
def test_affected_via_ship(client, consts):
    ...
    assert api_char.update().attrs[eve_affectee_attr_id].modified == approx(120)
...
def test_replace_root(client, consts):
    ...
    eve_effect_id = client.mk_eve_effect(cat_id=consts.EveEffCat.system, mod_info=[eve_mod])
    eve_proj_effect_id = client.mk_eve_item(attrs={eve_affector_attr_id: 20}, eff_ids=[eve_effect_id])
    eve_char1_id = client.mk_eve_item(attrs={eve_affectee_attr_id: 100})
    eve_char2_id = client.mk_eve_item(attrs={eve_affectee_attr_id: 50})
    eve_ship_id = client.mk_eve_ship()
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_char1 = api_fit.set_character(type_id=eve_char1_id)
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
    api_proj_effect = api_sol.add_proj_effect(type_id=eve_proj_effect_id)
    api_proj_effect.change_proj_effect(add_projs=[api_ship.id])
    assert api_char1.update().attrs[eve_affectee_attr_id].modified == approx(120)
    api_char2 = api_fit.set_character(type_id=eve_char2_id)
    assert api_char2.update().attrs[eve_affectee_attr_id].modified == approx(60)
...
def setup_switch_type_id_char_test(*, client, consts):
    ...
    eve_effect_id = client.mk_eve_effect(cat_id=consts.EveEffCat.system, mod_info=[eve_mod])
    eve_proj_effect_id = client.mk_eve_item(attrs={eve_affector_attr_id: 20}, eff_ids=[eve_effect_id])
    eve_char_loaded_id = client.mk_eve_item(attrs={eve_affectee_attr_id: 100})
    ...
    api_proj_effect = api_sol.add_proj_effect(type_id=eve_proj_effect_id)
    api_proj_effect.change_proj_effect(add_projs=[api_ship.id])
    return eve_affectee_attr_id, eve_char_loaded_id, eve_char_not_loaded_id, api_fit, api_proj_effect


def test_switch_type_id_char_affected_to_not_loaded_remove(client, consts):
    (eve_affectee_attr_id,
     eve_char_loaded_id,
     eve_char_not_loaded_id,
     api_fit,
     api_proj_effect) = setup_switch_type_id_char_test(client=client, consts=consts)
    api_char = api_fit.set_character(type_id=eve_char_loaded_id)
    # Verification
    assert api_char.update().attrs[eve_affectee_attr_id].modified == approx(120)
    # Action
...
def test_switch_type_id_char_not_loaded_to_affected_remove(client, consts):
    (eve_affectee_attr_id,
     ...
     api_fit,
     api_proj_effect) = setup_switch_type_id_char_test(client=client, consts=consts)
    api_char = api_fit.set_character(type_id=eve_char_not_loaded_id)
    # Verification
    api_char.update()
    with check_no_field():
        api_char.attrs  # noqa: B018
