...
def test_switch_type_id_affectee(client, consts):
    ...
    eve_char1_id = client.mk_eve_item(attrs={eve_affectee_attr_id: 100})
    eve_char2_id = client.mk_eve_item(attrs={eve_affectee_attr_id: 50})
    eve_char3_id = client.alloc_item_id()
    eve_ship_id = client.mk_eve_ship()
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_char = api_fit.set_character(type_id=eve_char1_id)
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
    api_proj_effect = api_sol.add_proj_effect(type_id=eve_proj_effect_id)
    api_proj_effect.change_proj_effect(add_projs=[api_ship.id])
    # Verification
    assert api_char.update().attrs[eve_affectee_attr_id].dogma == approx(120)
    # Action
    api_char.change_character(type_id=eve_char2_id)
    # Verification
    assert api_char.update().attrs[eve_affectee_attr_id].dogma == approx(60)
    # Action
    api_char.change_character(type_id=eve_char3_id)
    # Verification
    api_char.update()
    with check_no_field():
        api_char.attrs  # noqa: B018
    # Action
    api_char.change_character(type_id=eve_char1_id)
    # Verification
    assert api_char.update().attrs[eve_affectee_attr_id].dogma == approx(120)
