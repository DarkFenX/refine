...
def test_sec_zones(client, consts):
    # Realistic case, pretty much a t2 ME rig, except for general attr modifier value being 1.1
    # instead of 1
    ...
    # Verification
    api_rig.update()
    assert api_rig.attrs[eve_sec_attr_id].dogma == approx(1)
    assert api_rig.attrs[eve_affectee_attr_id].dogma == approx(-2.4)
    api_sec_mod = api_rig.mods[eve_sec_attr_id].one()
    assert api_sec_mod.op == consts.ApiModOp.pre_assign
    assert api_sec_mod.initial_val == approx(1)
    assert api_sec_mod.stacking_mult is None
    assert api_sec_mod.applied_val == approx(1)
    assert api_sec_mod.affectors.one().item_id == api_rig.id
    assert api_sec_mod.affectors.one().attr_id == eve_hisec_attr_id
    # Multiplication by 1 is insignificant and is not exposed
    assert eve_affectee_attr_id not in api_rig.mods
    # Action
