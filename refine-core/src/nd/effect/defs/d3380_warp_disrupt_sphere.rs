...
use crate::{
    ...
    nd::{NEffect, NEffectCharge, NEffectChargeDepl, NEffectChargeLoc, NEffectProjModSpec, NEffectProjMultGetter},
};
...
fn update_effect(a_effect: &mut AEffect, a_warnings: &mut Vec<String>) {
    if !a_effect.modifiers.is_empty() {
        let warning = format!("effect {EFFECT_AID}: WDFG bubble effect has modifiers, overwriting them");
