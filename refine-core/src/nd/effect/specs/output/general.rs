...
impl NEffectOutputGetter for NEffectGeneralOutputGetter {
    ...
    fn get(
        ...
        match self {
            // Use dogma getter because of ASBs, they need pre-cap-nullification value to work with
            Self::CapConsumer => get_attr_dogma(ctx, calc, item_uid, effect, effect.discharge_attr_rid, true),
            Self::RepShield => get_attr_extra(ctx, calc, item_uid, effect, ctx.ac().shield_bonus, true),
            // Use dogma getter because of AARs, they need pre-multiplication value to work with
            Self::RepArmor => get_attr_dogma(ctx, calc, item_uid, effect, ctx.ac().armor_dmg_amount, false),
            Self::RepHull => get_attr_extra(ctx, calc, item_uid, effect, ctx.ac().struct_dmg_amount, false),
            Self::PowerTransfer => get_attr_extra(ctx, calc, item_uid, effect, ctx.ac().power_transfer_amount, false),
            // Neut-related
            Self::Neut => get_attr_extra(ctx, calc, item_uid, effect, ctx.ac().energy_neut_amount, true),
            ...
            Self::NeutDdWarmup => {
                get_attr_extra(ctx, calc, item_uid, effect, ctx.ac().doomsday_energy_neut_amount, true)
            }
...
fn get_attr_dogma(
    ...
    Some(Output::Simple(OutputSimple {
        instance: PValue::from_value_clamped(calc.get_item_oattr_ffb_dogma(ctx, item_uid, attr_rid, Value::ZERO)),
...
fn get_attr_extra(
    ...
    Some(Output::Simple(OutputSimple {
        instance: PValue::from_value_clamped(calc.get_item_oattr_ffb_extra(ctx, item_uid, attr_rid, Value::ZERO)),
        delay: match applied_at_start {
            true => PValue::ZERO,
            false => funcs::get_effect_duration_s(ctx, calc, item_uid, effect)?,
        },
