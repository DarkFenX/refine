...
impl<'s> FitMut<'s> {
    pub fn set_fleet(&mut self, fleet_id: &FleetId) -> Result<(), FitFleetSetError> {
        let fleet_uid = self.sol.u_data.fleets.int_id_by_ext_id_err(fleet_id)?;
