    ...
    fn from_u_data(u_data: &UData) -> Self {
        ...
        for (item_uid, u_item) in u_data.items.iter() {
            match u_item {
                ...
                UItem::Autocharge(..) => (),
                UItem::Booster(..) => data.boosters.push(item_uid),
                UItem::Character(..) => data.characters.push(item_uid),
                UItem::Charge(..) => data.charges.push(item_uid),
                UItem::Drone(..) => data.drones.push(item_uid),
                UItem::Fighter(..) => data.fighters.push(item_uid),
                UItem::FwEffect(..) => data.fw_effects.push(item_uid),
                UItem::Implant(..) => data.implants.push(item_uid),
                UItem::Module(..) => data.modules.push(item_uid),
                UItem::ProjEffect(..) => data.proj_effects.push(item_uid),
                UItem::Rig(..) => data.rigs.push(item_uid),
                UItem::Service(..) => data.services.push(item_uid),
                UItem::Ship(..) => data.ships.push(item_uid),
                UItem::Skill(..) => data.skills.push(item_uid),
                UItem::Stance(..) => data.stances.push(item_uid),
                UItem::Subsystem(..) => data.subsystems.push(item_uid),
                UItem::SwEffect(..) => data.sw_effects.push(item_uid),
            }
        }
        data
    }
    ...
    pub fn set_src(&mut self, src: &Src) {
        ...
        for fit in self.u_data.fits.values_mut() {
            fit.ship_kind = match fit.ship {
                Some(ship_uid) => self.u_data.items.get(ship_uid).dc_ship().unwrap().get_ship_kind(),
