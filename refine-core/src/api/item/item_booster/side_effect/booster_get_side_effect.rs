...
impl<'s> Booster<'s> {
    pub fn get_side_effect(&self, effect_id: &EffectId) -> Result<SideEffect<'_>, SideEffectGetError> {
...
impl<'s> BoosterMut<'s> {
    ...
    pub fn get_side_effect_mut(&mut self, effect_id: &EffectId) -> Result<SideEffectMut<'_>, SideEffectGetError> {
        ...
        match get_se_chance_attr_aid_by_effect_aid(&self.sol.u_data.r_data, &effect_aid) {
            ...
            None => Err(SideEffectGetError::SideEffectNotFound(
...
fn get_side_effect<'s>(
    ...
    booster_uid: UItemId,
    effect_id: &EffectId,
) -> Result<SideEffect<'s>, SideEffectGetError> {
    let effect_aid = effect_id.into_aid();
