    ...
    T: ItemMutCommon,
{
    ...
    let item_uid = item.get_uid();
    let u_item = item.get_sol().u_data.items.get(item_uid);
    if !u_item.is_loaded() {
        return None;
    }
    match u_item {
        ...
        UItem::Fighter(fighter) => {
            ...
            if charge_options.is_enabled() {
                for autocharge_uid in fighter.get_autocharges().values() {
                    let autocharge = item.get_sol().u_data.items.get(autocharge_uid).dc_autocharge().unwrap();
