use crate::{
    sol::SolarSystem,
    svc::Svc,
    ud::{UData, UEffectUpdates, UItemId},
};

impl SolarSystem {
    pub(in crate::api::item) fn util_process_autocharge_activations(
        u_data: &mut UData,
        svc: &mut Svc,
        ac_activations: Vec<(UItemId, bool)>,
        reuse_eupdates: &mut UEffectUpdates,
    ) {
        for (ac_uid, ac_activated) in ac_activations {
            let u_autocharge = u_data.items.get_mut(ac_uid).dc_autocharge_mut().unwrap();
            let old_a_state = u_autocharge.get_state();
            u_autocharge.set_activated(ac_activated);
            let new_a_state = u_autocharge.get_state();
            u_autocharge.update_reffs(reuse_eupdates, &u_data.r_data);
            SolarSystem::util_switch_item_state(u_data, svc, ac_uid, old_a_state, new_a_state, reuse_eupdates);
        }
    }
}
