use crate::{misc::InfCount, num::Count, rd::REffectId, svc::cycle::effect_charge_info::EffectChargeInfo, ud::UItem};

pub(in crate::svc::cycle) fn get_eci_autocharge(item: &UItem, effect_rid: REffectId) -> EffectChargeInfo {
    EffectChargeInfo {
        fully_charged: internal_cycle_count(item, effect_rid),
        part_charged: None,
        can_run_uncharged: false,
    }
}

fn internal_cycle_count(item: &UItem, effect_rid: REffectId) -> InfCount {
    let Some(autocharges) = item.get_autocharges() else {
        // Effect wants autocharge, but item does not support autocharges -> can't cycle
        return InfCount::Count(Count::ZERO);
    };
    if !autocharges.contains_ac_for_effect(&effect_rid) {
        // Effect wants autocharge, but no autocharge in the item - can't cycle. Since autocharges
        // are not added here when they cannot be loaded, non-loaded autocharges are covered by this
        // as well.
        return InfCount::Count(Count::ZERO);
    };
    // - unwrap #1: item base should always be available, since this method should never be
    //   requested for non-loaded items
    // - unwrap #2: this method should be called only for effects which are present on the item
    match item
        .get_r_item_base()
        .unwrap()
        .effects
        .get(&effect_rid)
        .unwrap()
        .ability_charge_count
    {
        Some(charge_count) => InfCount::Count(charge_count),
        None => InfCount::Infinite,
    }
}
