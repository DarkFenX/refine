...
impl VastFitData {
    // Fast validations
    pub(in crate::svc::vast::val) fn validate_charge_size_fast(&self, kfs: &RSet<UItemId>) -> bool {
        match kfs.is_empty() {
    ...
    // Verbose validations
    pub(in crate::svc::vast::val) fn validate_charge_size_verbose(
        &self,
        kfs: &RSet<UItemId>,
        ctx: SvcCtx,
        ...
        for (&charge_uid, &cont_uid) in self.charge_size.difference(kfs) {
            charges.push(ValChargeSizeChargeInfo {
                ...
                charge_size: ctx.u_data.items.get(charge_uid).get_r_item_base().unwrap().charge_size,
                allowed_size: ctx
                    .u_data
                    .items
                    .get(cont_uid)
