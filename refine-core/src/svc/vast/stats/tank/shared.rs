...
pub(super) fn get_tanking_efficiency(resists: StatResistsLayer, incoming_dps: DpsProfile) -> Option<PValue> {
    ...
    let absorbed = incoming_dps.em * resists.em.into_pvalue()
        + incoming_dps.thermal * resists.thermal.into_pvalue()
        + incoming_dps.kinetic * resists.kinetic.into_pvalue()
        + incoming_dps.explosive * resists.explosive.into_pvalue();
