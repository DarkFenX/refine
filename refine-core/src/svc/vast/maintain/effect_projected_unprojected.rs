...
impl Vast {
    pub(in crate::svc) fn effect_projected(
        ...
        if let Some(projector_fit_uid) = projector_item.get_fit_uid() {
            let projector_fit_data = self.get_fit_data_mut(projector_fit_uid);
            let projector_espec = EffectSpec::new(projector_uid, effect.rid);
            if effect.projectee_filter.is_some()
                && let Some(effect_data) = projector_item.get_r_item_base().unwrap().effects.get(&effect.rid)
                && let Some(item_list_rid) = effect_data.projectee_filter_rid
            {
                projector_fit_data
                    .projectee_filter
        ...
        {
            let projectee_fit_data = self.get_fit_data_mut(projectee_fit_uid);
...
fn is_offense_blockable(projector_item: &UItem, r_effect: &REffect) -> bool {
    if r_effect.is_blockable_offensive_modifier {
        return true;
    };
