...
impl CtxModifier {
    ...
    pub(in crate::svc::calc) fn with_item(raw: RawModifier, item_uid: UItemId) -> Self {
        Self {
            raw,
            ctx: ModContext::Item(item_uid),
        }
    }
    pub(in crate::svc::calc) fn with_fit(raw: RawModifier, fit_uid: UFitId) -> Self {
        Self {
            raw,
            ctx: ModContext::Fit(fit_uid),
        }
    }
    pub(in crate::svc::calc) fn with_fit_item(raw: RawModifier, fit_uid: UFitId, item_uid: UItemId) -> Self {
        Self {
            raw,
