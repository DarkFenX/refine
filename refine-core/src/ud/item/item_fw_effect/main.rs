use crate::{
    ...
    ud::{
        UFitId,
        item::{UEffectModeOverrideIter, UEffectUpdates, UItemBase, bool_to_state_active, state_to_bool},
    },
    util::RSet,
};
...
impl UFwEffect {
    pub(crate) fn new(
        ...
        type_aid: AItemId,
        fit_uid: UFitId,
        fw_effect_state: bool,
        r_data: &RData,
    ) -> Self {
        Self {
            base: UItemBase::new(item_id, type_aid, bool_to_state_active(fw_effect_state), r_data),
            fit_uid,
        }
    }
...
impl UFwEffect {
    ...
    pub(crate) fn set_fw_effect_state(&mut self, state: bool) {
