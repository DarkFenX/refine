...
impl SolarSystem {
    pub fn get_fit(&self, fit_id: &SolFitId) -> Result<SolFitInfo> {
    ...
    pub fn add_fit(&mut self) -> Result<SolFitInfo> {
        let fit_id = self.fits.add_fit()?;
        self.svcs.add_fit(&fit_id);
        self.get_fit(&fit_id)
    }
    pub fn set_fit_fleet(&mut self, fit_id: &SolFitId, fleet_id_opt: Option<SolFleetId>) -> Result<()> {
        let fit = self.fits.get_fit(fit_id)?;
    ...
    pub fn remove_fit(&mut self, fit_id: &SolFitId) -> Result<()> {
