...
impl VastFitData {
    // Modules
    pub(in crate::sol::svc) fn get_stat_high_slots(&self, ctx: &SvcCtx, calc: &mut Calc, fit: &UadFit) -> StatSlot {
        let used = fit.mods_high.len() as Count;
        let total = get_attr_as_count(ctx, calc, fit.ship, &ac::attrs::HI_SLOTS);
        StatSlot { used, total }
    }
    pub(in crate::sol::svc) fn get_stat_mid_slots(&self, ctx: &SvcCtx, calc: &mut Calc, fit: &UadFit) -> StatSlot {
        let used = fit.mods_mid.len() as Count;
        let total = get_attr_as_count(ctx, calc, fit.ship, &ac::attrs::MED_SLOTS);
        StatSlot { used, total }
    }
    pub(in crate::sol::svc) fn get_stat_low_slots(&self, ctx: &SvcCtx, calc: &mut Calc, fit: &UadFit) -> StatSlot {
        let used = fit.mods_low.len() as Count;
        let total = get_attr_as_count(ctx, calc, fit.ship, &ac::attrs::LOW_SLOTS);
        StatSlot { used, total }
    }
    pub(in crate::sol::svc) fn get_stat_turret_slots(&self, ctx: &SvcCtx, calc: &mut Calc, fit: &UadFit) -> StatSlot {
        let used = self.mods_turret.len() as Count;
        let total = get_attr_as_count(ctx, calc, fit.ship, &ac::attrs::TURRET_SLOTS_LEFT);
        StatSlot { used, total }
    }
    pub(in crate::sol::svc) fn get_stat_launcher_slots(&self, ctx: &SvcCtx, calc: &mut Calc, fit: &UadFit) -> StatSlot {
        let used = self.mods_launcher.len() as Count;
        let total = get_attr_as_count(ctx, calc, fit.ship, &ac::attrs::LAUNCHER_SLOTS_LEFT);
        StatSlot { used, total }
    }
    // Rigs
    pub(in crate::sol::svc) fn get_stat_rig_slots(&self, ctx: &SvcCtx, calc: &mut Calc, fit: &UadFit) -> StatSlot {
        let used = fit.rigs.len() as Count;
        let total = get_attr_as_count(ctx, calc, fit.ship, &ac::attrs::UPGRADE_SLOTS_LEFT);
        StatSlot { used, total }
    }
    // Service
    pub(in crate::sol::svc) fn get_stat_service_slots(&self, ctx: &SvcCtx, calc: &mut Calc, fit: &UadFit) -> StatSlot {
