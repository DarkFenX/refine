...
impl SolSvcs {
    ...
    pub(in crate::sol::svc) fn calc_item_loaded(&mut self, sol_view: &SolView, item: &SolItem) {
        ...
        self.calc.std.reg_affectee(sol_view, item);
    ...
    pub(in crate::sol::svc) fn calc_item_unloaded(&mut self, sol_view: &SolView, item: &SolItem) {
        ...
        // Notify core calc services
        self.handle_location_owner_change(sol_view, item);
        self.calc.std.unreg_affectee(sol_view, item);
    ...
    pub(in crate::sol::svc) fn calc_attr_value_changed(
        ...
        if !ctx_modifiers.is_empty() {
            let mut affectees = Vec::new();
            for ctx_modifier in ctx_modifiers.iter() {
                self.calc.std.fill_affectees(&mut affectees, sol_view, &ctx_modifier);
    ...
    fn reg_raw_mod(
        ...
        match raw_modifier.kind {
            ...
            SolModifierKind::System => match item {
                SolItem::SwEffect(_) => {
                    self.calc.std.reg_sw_system_mod(util_cmods, sol_view, *raw_modifier);
                    for ctx_modifier in util_cmods.iter() {
                        self.force_mod_affectee_attr_recalc(util_items, sol_view, &ctx_modifier);
                    }
                }
            ...
            SolModifierKind::Buff => {
                let registered = match item {
                    SolItem::SwEffect(_) => {
                        let registered = self.calc.std.reg_sw_buff_mod(util_cmods, sol_view, *raw_modifier);
                        for ctx_modifier in util_cmods.iter() {
                            self.force_mod_affectee_attr_recalc(util_items, sol_view, &ctx_modifier);
                        }
                        registered
    ...
    fn unreg_raw_mod(
        ...
        match raw_modifier.kind {
            ...
            SolModifierKind::System => match item {
                SolItem::SwEffect(_) => {
                    self.calc.std.unreg_sw_system_mod(util_cmods, sol_view, *raw_modifier);
                    for ctx_modifier in util_cmods.iter() {
                        self.force_mod_affectee_attr_recalc(util_items, sol_view, &ctx_modifier);
                    }
                }
            ...
            SolModifierKind::Buff => {
                match item {
                    SolItem::SwEffect(_) => {
                        self.calc.std.unreg_sw_buff_mod(util_cmods, sol_view, raw_modifier);
                        for ctx_modifier in util_cmods.iter() {
                            self.force_mod_affectee_attr_recalc(util_items, sol_view, &ctx_modifier);
                        }
                    }
    ...
    fn force_mod_affectee_attr_recalc(
        ...
        self.calc.std.fill_affectees(affectees, sol_view, modifier);
        for projectee_item_id in affectees.iter() {
