...
impl SolDependencyRegister {
    pub(in crate::sol) fn debug_consistency_check(&self, sol_view: &SolView) -> DebugResult {
        for (src_attr_spec, tgt_attr_specs) in self.data.iter() {
            check_item(sol_view, &src_attr_spec.item_id)?;
            check_attr(sol_view, &src_attr_spec.attr_id)?;
            for tgt_attr_spec in tgt_attr_specs {
                check_item(sol_view, &tgt_attr_spec.item_id)?;
                check_attr(sol_view, &tgt_attr_spec.attr_id)?;
            }
        }
