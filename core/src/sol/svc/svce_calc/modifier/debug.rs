...
pub(in crate::sol::svc) fn check_raw_modifier(sol_view: &SolView, modifier: &SolRawModifier) -> DebugResult {
    check_item(sol_view, &modifier.affector_item_id)?;
    check_effect(sol_view, &modifier.effect_id)?;
    Ok(())
}

pub(in crate::sol::svc) fn check_ctx_modifier(sol_view: &SolView, modifier: &SolCtxModifier) -> DebugResult {
    match modifier.ctx {
