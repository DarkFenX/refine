...
impl SolSvc {
    pub(in crate::sol) fn process_effects(&mut self, uad: &SolUad, item: &SolItem, item_state: SolItemState) {
        if !item.is_loaded() {
            return;
        }
        let mut to_start = Vec::new();
        let mut to_stop = Vec::new();
        let online_should_run = resolve_online_effect_status(uad, item, item_state);
        for effect_id in item.get_effect_datas().unwrap().keys() {
            let effect = match uad.src.get_a_effect(effect_id) {
                Some(e) => e,
                None => continue,
            };
                ...
                for (proj_item_id, range) in projs {
                    ...
                    for effect in to_start.iter() {
                        if is_effect_projectable(effect) {
                            self.notify_effect_projected(uad, item, effect, proj_item, *range);
                        }
                    }
                }
            }
            ...
            if let Some(proj_items) = item.iter_projectee_items() {
                for proj_item_id in proj_items {
                    ...
                    for effect in to_stop.iter() {
                        if is_effect_projectable(effect) {
                            self.notify_effect_unprojected(uad, item, effect, proj_item);
                        }
                    }
                }
            }
