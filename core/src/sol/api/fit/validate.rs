...
impl SolarSystem {
    pub(in crate::sol) fn internal_validate_fit_fast(&mut self, fit_key: FitKey, options: &ValOptions) -> bool {
        self.svc
            .vast
            .validate_fit_fast(&self.uad, &mut self.svc.calc, fit_key, options)
    }
    pub(in crate::sol) fn internal_validate_fit_verbose(&mut self, fit_key: FitKey, options: &ValOptions) -> ValResult {
        self.svc
            .vast
