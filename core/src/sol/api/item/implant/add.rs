...
impl SolarSystem {
    pub(in crate::sol) fn internal_add_implant(&mut self, fit_key: FitKey, type_id: ItemTypeId) -> ItemKey {
        let uad_fit = self.uad.fits.get_mut(fit_key);
        let item_id = self.uad.items.alloc_id();
        let uad_implant = UadImplant::new(&self.uad.src, item_id, type_id, fit_key, true);
...
impl<'a> FitMut<'a> {
    pub fn add_implant(&'a mut self, type_id: ItemTypeId) -> ImplantMut<'a> {
