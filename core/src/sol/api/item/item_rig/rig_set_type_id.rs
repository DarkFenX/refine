...
impl SolarSystem {
    pub(in crate::sol::api) fn internal_set_rig_a_item_id(&mut self, item_key: ItemKey, a_item_id: ad::AItemId) {
        ...
        SolarSystem::unload_rig(&self.uad, &mut self.svc, &mut self.reffs, item_key, uad_item);
        self.uad
            .items
            .get_mut(item_key)
            .get_rig_mut()
        ...
        let uad_item = self.uad.items.get(item_key);
        SolarSystem::load_rig(&self.uad, &mut self.svc, &mut self.reffs, item_key, uad_item);
    }
}
