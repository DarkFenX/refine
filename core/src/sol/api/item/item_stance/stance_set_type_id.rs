...
impl SolarSystem {
    pub(in crate::sol::api) fn internal_set_stance_a_item_id(
        &mut self,
        item_key: ItemKey,
        ...
        let uad_item = self.uad.items.get(item_key);
        SolarSystem::util_add_stance(&self.uad, &mut self.svc, item_key, uad_item, reuse_eupdates);
    }
}
...
impl<'a> StanceMut<'a> {
    ...
    pub fn set_type_id(&mut self, type_id: ItemTypeId) {
        ...
        self.sol
            .internal_set_stance_a_item_id(self.key, type_id, &mut reuse_eupdates)
    }
}
