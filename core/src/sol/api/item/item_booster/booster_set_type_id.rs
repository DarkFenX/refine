...
impl SolarSystem {
    pub(in crate::sol::api) fn internal_set_booster_a_item_id(&mut self, item_key: ItemKey, a_item_id: ad::AItemId) {
        ...
        SolarSystem::unload_booster(&self.uad, &mut self.svc, &mut self.reffs, item_key, uad_item);
        self.uad
            .items
            .get_mut(item_key)
            .get_booster_mut()
        ...
        let uad_item = self.uad.items.get(item_key);
        SolarSystem::load_booster(&self.uad, &mut self.svc, &mut self.reffs, item_key, uad_item);
    }
}
