...
impl SolarSystem {
    pub(in crate::sol::api) fn internal_add_booster(&mut self, fit_key: FitKey, type_id: ItemTypeId) -> ItemKey {
        let uad_fit = self.uad.fits.get_mut(fit_key);
        let item_id = self.uad.items.alloc_id();
        let uad_booster = UadBooster::new(&self.uad.src, item_id, type_id, fit_key, true);
