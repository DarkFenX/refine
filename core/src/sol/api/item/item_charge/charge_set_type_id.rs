...
impl SolarSystem {
    pub(in crate::sol::api) fn internal_set_charge_a_item_id(&mut self, item_key: ItemKey, a_item_id: ad::AItemId) {
        ...
        SolarSystem::util_remove_charge_with_projs(&self.uad, &mut self.svc, &mut self.reffs, item_key, uad_item);
        self.uad
            .items
            .get_mut(item_key)
            .get_charge_mut()
        ...
        let uad_item = self.uad.items.get(item_key);
        SolarSystem::util_add_charge_with_projs(&self.uad, &mut self.svc, &mut self.reffs, item_key, uad_item);
    }
}
