...
pub struct Ability<'a> {
    ...
    pub(in crate::sol::api) fighter_key: UItemKey,
    pub(in crate::sol::api) abil_id: AAbilId,
}
impl<'a> Ability<'a> {
    pub(in crate::sol::api) fn new(sol: &'a SolarSystem, fighter_key: UItemKey, abil_id: AAbilId) -> Self {
        Self {
            sol,
            fighter_key,
            abil_id,
...
impl<'a> AbilityMut<'a> {
    pub(in crate::sol::api) fn new(sol: &'a mut SolarSystem, fighter_key: UItemKey, abil_id: AAbilId) -> Self {
        Self {
            sol,
            fighter_key,
            abil_id,
