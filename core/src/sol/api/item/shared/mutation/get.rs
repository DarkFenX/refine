...
impl SolarSystem {
    pub(in crate::sol::api) fn api_get_item_mutation(&self, item_key: UadItemKey) -> Option<Mutation<'_>> {
    ...
    pub(in crate::sol::api) fn api_get_item_mutation_mut(&mut self, item_key: UadItemKey) -> Option<MutationMut<'_>> {
        item_has_mutation_cache(self, item_key).map(|v| match v {
...
fn item_has_mutation_cache(sol: &SolarSystem, item_key: UadItemKey) -> Option<bool> {
    sol.uad
        .items
        .get(item_key)
        .get_mutation_data()
