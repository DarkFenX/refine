use crate::{
    ad,
    sol::{ItemKey, ItemTypeId, SolarSystem, api::ServiceMut},
};
...
impl SolarSystem {
    pub(in crate::sol::api) fn internal_set_service_a_item_id(&mut self, item_key: ItemKey, a_item_id: ad::AItemId) {
        ...
        SolarSystem::unload_service(&self.uad, &mut self.svc, &mut self.reffs, item_key, uad_item);
        self.uad
            .items
            .get_mut(item_key)
            .get_service_mut()
        ...
        let uad_item = self.uad.items.get(item_key);
        SolarSystem::load_service(&self.uad, &mut self.svc, &mut self.reffs, item_key, uad_item);
    }
}
