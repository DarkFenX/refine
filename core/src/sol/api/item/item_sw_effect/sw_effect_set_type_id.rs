use crate::{
    ad,
    sol::{ItemKey, ItemTypeId, SolarSystem, api::SwEffectMut},
};
...
impl SolarSystem {
    pub(in crate::sol::api) fn internal_set_sw_effect_a_item_id(&mut self, item_key: ItemKey, a_item_id: ad::AItemId) {
        ...
        SolarSystem::unload_sw_effect(&self.uad, &mut self.svc, &mut self.reffs, item_key, uad_item);
        self.uad
            .items
            .get_mut(item_key)
            .get_sw_effect_mut()
        ...
        let uad_item = self.uad.items.get(item_key);
        SolarSystem::load_sw_effect(&self.uad, &mut self.svc, &mut self.reffs, item_key, uad_item);
    }
}
