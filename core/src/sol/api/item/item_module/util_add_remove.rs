...
impl SolarSystem {
    ...
    pub(in crate::sol::api) fn util_add_module_with_charge_act(
        ...
        let charge_key = SolarSystem::util_add_module(u_data, svc, item_key, reuse_eupdates);
        // Process charge
    ...
    pub(in crate::sol::api) fn util_remove_module_with_charge_act(
        ...
        // Process module itself
        let charge_key = SolarSystem::util_remove_module(u_data, svc, item_key, reuse_eupdates);
        // Process charge
        if let Some(charge_key) = charge_key
            && let Some(charge_activated) = reuse_eupdates.charge
        {
