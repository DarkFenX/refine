...
impl SolarSystem {
    ...
    pub fn debug_consistency_check(&self) -> bool {
        ...
        if self.uad.debug_consistency_check().is_err() {
            return false;
        }
        // Check solar system helper data containers
        if self.proj_tracker.debug_consistency_check(&self.uad).is_err() {
            return false;
        }
        // Check services
        if self.svc.debug_consistency_check(&self.uad).is_err() {
            return false;
        }
        true
    }
