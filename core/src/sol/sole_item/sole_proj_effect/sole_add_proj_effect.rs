...
impl SolarSystem {
    pub fn add_proj_effect(&mut self, type_id: ItemTypeId, state: bool) -> ProjEffectInfo {
        ...
        self.get_proj_effect_info_internal(item_key).unwrap()
    }
    pub(in crate::sol) fn add_proj_effect_internal(&mut self, type_id: ItemTypeId, state: bool) -> ItemKey {
        let item_id = self.uad.items.alloc_id();
        let proj_effect = UadProjEffect::new(&self.uad.src, item_id, type_id, state);
        let item = UadItem::ProjEffect(proj_effect);
