...
impl SolarSystem {
    pub fn set_subsystem_state(&mut self, item_id: &ItemId, state: bool) -> Result<(), SetSubsystemStateError> {
        ...
        Ok(self.set_subsystem_state_internal(item_key, state)?)
    }
    pub(in crate::sol) fn set_subsystem_state_internal(
        &mut self,
        item_key: ItemKey,
        state: bool,
    ) -> Result<(), ItemKindMatchError> {
        let subsystem = self.uad.items.get_mut(item_key).get_subsystem_mut()?;
