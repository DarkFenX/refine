...
impl SolarSystem {
    pub fn get_service_info(&self, item_id: &ItemId) -> Result<ServiceInfo, GetServiceInfoError> {
        let item_key = self.uad.items.key_by_id_err(item_id)?;
        Ok(self.get_service_info_internal(item_key)?)
    }
    pub(in crate::sol) fn get_service_info_internal(
        &self,
        item_key: ItemKey,
    ) -> Result<ServiceInfo, ItemKindMatchError> {
