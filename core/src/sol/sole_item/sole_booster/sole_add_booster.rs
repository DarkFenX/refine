...
impl SolarSystem {
    pub fn add_booster(
        ...
        Ok(self.get_booster_info_internal(item_key).unwrap())
    }
    pub(in crate::sol) fn add_booster_internal(
        ...
        let booster = UadBooster::new(&self.uad.src, item_id, type_id, fit_key, state);
        let item = UadItem::Booster(booster);
