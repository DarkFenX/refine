...
impl Src {
    ...
    pub fn new(
        ...
        let rah_duration_attr_rid = r_data
            ...
            .and_then(|effect_rid| r_data.effects.get(effect_rid).unwrap().duration_attr_rid);
        let online_effect = r_data
            .effect_consts
            .online
            .map(|effect_rid| r_data.effects.get(effect_rid).unwrap().clone());
        Ok(Self {
    ...
    pub(crate) fn get_item_list_by_rid(&self, item_list_rid: RItemListId) -> &RItemList {
        self.r_data.item_lists.get(item_list_rid).unwrap()
    }
    ...
    pub(crate) fn get_attr_by_rid(&self, attr_rid: RAttrId) -> &RAttr {
        self.r_data.attrs.get(attr_rid).unwrap()
    }
    ...
    pub(crate) fn get_effect_by_rid(&self, effect_rid: REffectId) -> &RcEffect {
        self.r_data.effects.get(effect_rid).unwrap()
    }
    ...
    pub(crate) fn get_buff_by_rid(&self, buff_rid: RBuffId) -> &RBuff {
        self.r_data.buffs.get(buff_rid).unwrap()
    }
