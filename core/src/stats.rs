//! Stats-related exports
pub use crate::{
    misc::DmgKinds,
    svc::vast::{
        StatCapRegenOptions, StatCapSim, StatCapSimStagger, StatCapSrcKinds, StatDmg, StatDmgApplied, StatDmgBreacher,
        StatDmgItemKinds, StatJamApplied, StatLayerEhp, StatLayerErps, StatLayerErpsRegen, StatLayerHp, StatLayerRps,
        StatLayerRpsRegen, StatMining, StatMiningItemKinds, StatNeutItemKinds, StatOutRepItemKinds, StatRes,
        StatSensors, StatSensorsKind, StatSlot, StatTank, StatTankRegen, StatTimeOptions, StatTimeOptionsBurst,
        StatTimeOptionsSim,
    },
};
