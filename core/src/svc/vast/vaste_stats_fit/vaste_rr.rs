...
impl VastFitData {
    ...
    pub(in crate::svc) fn get_stat_orr_armor(&self, ctx: SvcCtx, calc: &mut Calc) -> AttrVal {
        get_orrps(ctx, calc, self.orr_armor.iter())
    }
    pub(in crate::svc) fn get_stat_orr_struct(&self, ctx: SvcCtx, calc: &mut Calc) -> AttrVal {
        get_orrps(ctx, calc, self.orr_struct.iter())
    }
...
fn get_orrps<'a>(
    ...
    for (&espec, rep_getter) in data {
        if let Some(reps_per_cycle) = rep_getter(ctx, calc, espec, None) {
            let cycle_time = efuncs::get_espec_cycle_time(ctx, calc, espec).unwrap();
            if cycle_time != OF(0.0) {
                rps += reps_per_cycle / cycle_time;
            }
        }
    }
    rps
