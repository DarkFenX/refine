...
impl VastFitData {
    // Fast validations
    pub(in crate::svc::vast) fn validate_charge_volume_fast(&mut self, kfs: &RSet<UadItemKey>) -> bool {
        match kfs.is_empty() {
        ...
        for (&charge_key, &cont_key) in self.charge_volume.difference(kfs) {
            charges.insert(
                ...
                ValChargeVolumeChargeInfo {
                    ...
                    charge_volume: ctx.uad.items.get(charge_key).get_r_axt().unwrap().volume,
                    max_volume: ctx.uad.items.get(cont_key).get_r_axt().unwrap().capacity,
                },
            );
        }
