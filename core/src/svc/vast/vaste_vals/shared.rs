...
pub(super) fn is_oattr_flag_set(ctx: SvcCtx, calc: &mut Calc, item_key: UItemId, attr_key: Option<RAttrKey>) -> bool {
    match attr_key {
        Some(attr_key) => is_attr_flag_set(ctx, calc, item_key, attr_key),
        None => false,
    }
}
pub(super) fn is_attr_flag_set(ctx: SvcCtx, calc: &mut Calc, item_key: UItemId, attr_key: RAttrKey) -> bool {
