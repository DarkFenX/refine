...
use crate::{
    ...
    svc::{
        ...
        vast::{StatSensorsKind, Vast, aggr::aggr_proj_first, stats::item_checks::check_drone_fighter_ship},
    },
    ud::UItemId,
};
    ...
    pub(in crate::svc) fn get_stat_item_incoming_jam(
        ...
        for (&projector_item_uid, projector_data) in incoming_ecms.iter() {
            ...
            for (&effect_rid, ospec) in projector_data.iter() {
                ...
                // }
                // // Jam chance
                // let jam_chance = UnitInterval::from_pvalue_clamped(projector_ecm_str /
                // sensors.strength); projectee_unjam_chance *= Value::ONE -
                // jam_chance.into_value(); // Jam uptime
                // // Theoretically, it is possible to have overlapping cycles with some items (e.g.
                // if // ECM burst projectors had super short cycle). This stat
                // deliberately gives up on // that by restricting cycle uptime to
                // 100% because: // - EVE has no such known scenarios;
                // // - handling it properly would be much more complex;
                // // - uptime is an approximate stat by its nature (since value depends on how item
                // cycles would be //   distributed, and the lib does not expose
                // controls to that). let cycle_uptime =
                //     UnitInterval::from_pvalue_clamped(projector_ecm.duration /
                // projector_ecm_output.duration); projectee_unjam_uptime *=
                // Value::ONE - jam_chance.into_value() * cycle_uptime.into_value();
            }
        }
        let jam = StatInJam {
