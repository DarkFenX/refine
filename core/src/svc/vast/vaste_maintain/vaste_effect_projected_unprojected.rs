...
impl Vast {
    pub(in crate::svc) fn effect_projected(
        ...
        if r_effect.get_category() == ac::effcats::TARGET {
            if !r_effect.get_stopped_effect_keys().is_empty()
                && let Some(projectee_fit_key) = projectee_item.get_fit_key()
            {
                ...
                let stopper = EffectSpec::new(projector_key, r_effect.get_key());
                for stop_a_effect_id in r_effect.get_stopped_effect_keys().iter() {
            ...
            if let Some(projector_fit_key) = projector_item.get_fit_key() {
                ...
                let projector_espec = EffectSpec::new(projector_key, r_effect.get_key());
