...
impl Vast {
    pub(in crate::svc) fn item_loaded(&mut self, u_data: &UData, item_key: UItemId, item: &UItem) {
            ...
            && !a_srqs.is_empty()
        {
            ...
            for (&skill_a_item_id, &required_a_lvl) in a_srqs.iter() {
                ...
                if match current_lvl {
                    Some(current_lvl) => current_lvl.get_inner() < required_a_lvl.into_inner(),
                    None => true,
                } {
        ...
        match item {
            UItem::Booster(booster) => {
                ...
                item_kind_add(fit_data, item_key, item_axt.kind, ItemKind::Booster);
            ...
            UItem::Character(character) => {
                ...
                item_kind_add(fit_data, item_key, item_axt.kind, ItemKind::Character);
            }
            UItem::Charge(charge) => {
            ...
            UItem::Drone(drone) => {
                ...
                item_kind_add(fit_data, item_key, item_axt.kind, ItemKind::Drone);
            ...
            UItem::Fighter(fighter) => {
                ...
                item_kind_add(fit_data, item_key, item_axt.kind, ItemKind::Fighter);
            ...
            UItem::Implant(implant) => {
                ...
                item_kind_add(fit_data, item_key, item_axt.kind, ItemKind::Implant);
            ...
            UItem::Rig(rig) => {
                ...
                item_kind_add(fit_data, item_key, item_axt.kind, ItemKind::Rig);
            ...
            UItem::Service(service) => {
                ...
                item_kind_add(fit_data, item_key, item_axt.kind, ItemKind::Service);
                if let Some(ship_limit) = &item_axt.ship_limit {
                    fit_data.ship_limited_items.insert(item_key, ship_limit.clone());
                }
            ...
            UItem::Ship(ship) => {
                ...
                item_kind_add(fit_data, item_key, item_axt.kind, ItemKind::Ship);
            ...
            UItem::Skill(skill) => {
                ...
                item_kind_add(fit_data, item_key, item_axt.kind, ItemKind::Skill);
            }
            UItem::Stance(stance) => {
                let item_axt = stance.get_axt().unwrap();
                item_kind_add(fit_data, item_key, item_axt.kind, ItemKind::Stance);
                if let Some(ship_limit) = &item_axt.ship_limit {
                    fit_data.ship_limited_items.insert(item_key, ship_limit.clone());
                }
            }
            UItem::Subsystem(subsystem) => {
                ...
                item_kind_add(fit_data, item_key, item_axt.kind, ItemKind::Subsystem);
    ...
    pub(in crate::svc) fn item_unloaded(&mut self, item_key: &UItemId, item: &UItem) {
        ...
        match item {
            UItem::Booster(booster) => {
                let item_axt = booster.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Booster);
            ...
            UItem::Character(character) => {
                let item_axt = character.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Character);
            }
            UItem::Charge(charge) => {
                let item_axt = charge.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Charge);
            ...
            UItem::Drone(drone) => {
                let item_axt = drone.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Drone);
            ...
            UItem::Fighter(fighter) => {
                let item_axt = fighter.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Fighter);
            ...
            UItem::Implant(implant) => {
                let item_axt = implant.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Implant);
            ...
            UItem::Rig(rig) => {
                let item_axt = rig.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Rig);
            ...
            UItem::Service(service) => {
                let item_axt = service.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Service);
                if item_axt.ship_limit.is_some() {
                    fit_data.ship_limited_items.remove(item_key);
                }
                if let Some(a_item_grp_id) = service.get_val_fitted_group_id() {
            ...
            UItem::Ship(ship) => {
                let item_axt = ship.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Ship);
            ...
            UItem::Skill(skill) => {
                let item_axt = skill.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Skill);
            }
            UItem::Stance(stance) => {
                let item_axt = stance.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Stance);
                if item_axt.ship_limit.is_some() {
                    fit_data.ship_limited_items.remove(item_key);
                }
            }
            UItem::Subsystem(subsystem) => {
                let item_axt = subsystem.get_axt().unwrap();
                item_kind_remove(fit_data, item_key, item_axt.kind, ItemKind::Subsystem);
