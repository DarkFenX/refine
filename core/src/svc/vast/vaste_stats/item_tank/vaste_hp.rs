use crate::{
    ...
    svc::{
        SvcCtx,
        aggr::{aggr_local_clip_amount_data, aggr_proj_clip_amount_data},
        calc::Calc,
...
fn get_local_ancil_hp(
    ...
    for (&item_key, item_data) in ancil_data.iter() {
        ...
        for (&effect_key, ospec) in item_data.iter() {
            ...
            if let Some(effect_clip_data) = aggr_local_clip_amount_data(ctx, calc, item_key, effect, cseq, ospec) {
...
fn get_remote_ancil_hp(
    ...
    for (&projector_item_key, projector_data) in incoming_ancils.iter() {
        ...
        for (&effect_key, ospec) in projector_data.iter() {
            ...
            let effect = ctx.u_data.src.get_effect(effect_key);
            if let Some(effect_clip_data) = aggr_proj_clip_amount_data(
                ctx,
                calc,
                projector_item_key,
                effect,
