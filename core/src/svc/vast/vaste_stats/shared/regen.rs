use crate::num::{PValue, UnitInterval, Value};

pub(in crate::svc::vast::vaste_stats) fn calc_regen(max: PValue, rech_time: PValue, cap_perc: UnitInterval) -> PValue {
    let cap_perc = cap_perc.into_pvalue();
    let result = PValue::TEN * max / rech_time * PValue::from_value_unchecked(cap_perc.sqrt() - cap_perc);
    match result.is_finite() {
        true => result,
        false => PValue::ZERO,
    }
}

pub(in crate::svc::vast::vaste_stats) fn regenerate(
    c0: PValue,
    c_max: PValue,
    tau: PValue,
    t0: PValue,
    t1: PValue,
) -> PValue {
    (Value::ONE + ((c0 / c_max).sqrt() - PValue::ONE) * ((t0 - t1) / tau).exp()).pow2() * c_max
}
