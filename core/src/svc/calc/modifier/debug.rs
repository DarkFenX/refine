...
pub(in crate::svc) fn check_rmod(u_data: &UData, rmod: &RawModifier) -> DebugResult {
    ...
    check_attr_rid(u_data, rmod.affectee_attr_key)?;
...
pub(in crate::svc) fn check_cmod(u_data: &UData, cmod: &CtxModifier) -> DebugResult {
    match cmod.ctx {
        ...
        // non-loaded ship)
        Context::None => (),
        Context::Item(item_key) => check_item_uid(u_data, item_key, false)?,
        Context::Fit(fit_key) => check_fit_uid(u_data, fit_key)?,
