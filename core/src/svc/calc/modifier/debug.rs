...
pub(in crate::svc) fn check_raw_modifier(uad: &Uad, modifier: &RawModifier) -> DebugResult {
    check_item_key(uad, modifier.affector_espec.item_key, true)?;
    check_a_effect_id(uad, &modifier.affector_espec.a_effect_id)?;
    Ok(())
}

pub(in crate::svc) fn check_ctx_modifier(uad: &Uad, modifier: &CtxModifier) -> DebugResult {
    match modifier.ctx {
