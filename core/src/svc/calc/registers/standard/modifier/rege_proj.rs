...
impl StandardRegister {
    ...
    pub(in crate::svc::calc) fn project_effect(
        ...
        for rmod in rmods.into_iter() {
            ...
            if let Some(cmod) = match rmod.kind {
                ModifierKind::System => self.proj_system_mod(rmod, projectee_item_key, projectee_item),
                ModifierKind::Targeted => self.proj_target_mod(rmod, projectee_item_key, projectee_item),
                ModifierKind::Buff => self.proj_buff_mod(rmod, projectee_item_key, projectee_item),
                _ => None,
            } {
                cmods.push(cmod);
            }
    ...
    pub(in crate::svc::calc) fn query_projected_effect(
        ...
        for rmod in rmods.into_iter() {
            ...
            if let Some(cmod) = match rmod.kind {
                ModifierKind::System => self.query_system_mod(rmod, projectee_item_key, projectee_item),
                ModifierKind::Targeted => self.query_target_mod(rmod, projectee_item_key, projectee_item),
                ModifierKind::Buff => self.query_buff_mod(rmod, projectee_item_key, projectee_item),
                _ => None,
            } {
                cmods.push(cmod);
            }
    ...
    pub(in crate::svc::calc) fn unproject_effect(
        ...
        for rmod in rmods.into_iter() {
            ...
            if let Some(cmod) = match rmod.kind {
                ModifierKind::System => self.unproj_system_mod(rmod, projectee_item_key, projectee_item),
                ModifierKind::Targeted => self.unproj_target_mod(rmod, projectee_item_key, projectee_item),
                ModifierKind::Buff => self.unproj_buff_mod(rmod, projectee_item_key, projectee_item),
                _ => None,
            } {
                cmods.push(cmod);
            }
    ...
    pub(super) fn reg_loc_root_for_proj(&mut self, projectee_item_key: ItemKey, projectee_item: &UadItem) {
        ...
        if let Some(rmods) = self.rmods_proj_inactive.remove_key(&projectee_item_key) {
            for rmod in rmods {
                ...
                match rmod.kind {
                    ModifierKind::System => self.reg_loc_root_for_proj_system(rmod, projectee_item_key, projectee_item),
                    ModifierKind::Targeted => {
                        self.reg_loc_root_for_proj_target(rmod, projectee_item_key, projectee_item)
                    }
                    ModifierKind::Buff => self.reg_loc_root_for_proj_buff(rmod, projectee_item_key, projectee_item),
                    _ => (),
                }
            }
        }
    ...
    pub(super) fn unreg_loc_root_for_proj(&mut self, projectee_item_key: ItemKey, projectee_item: &UadItem) {
        ...
        if let Some(rmods) = self.rmods_proj_active.remove_key(&projectee_item_key) {
            for rmod in rmods {
                ...
                match rmod.kind {
                    ModifierKind::System => {
                        self.unreg_loc_root_for_proj_system(rmod, projectee_item_key, projectee_item)
                    }
                    ModifierKind::Targeted => {
                        self.unreg_loc_root_for_proj_target(rmod, projectee_item_key, projectee_item)
                    }
                    ModifierKind::Buff => self.unreg_loc_root_for_proj_buff(rmod, projectee_item_key, projectee_item),
                    _ => (),
                }
            }
        }
    ...
    pub(super) fn unreg_inactive_proj_rmod(
        ...
        self.rmods_proj_inactive.remove_entry(projectee_item_key, rmod);
        None
    }
}
