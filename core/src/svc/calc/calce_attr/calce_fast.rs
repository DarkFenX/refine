...
impl Calc {
    ...
    pub(crate) fn get_item_attr_val_full(
        ...
        if let Some(cval) = item_attr_data.values.get(attr_id) {
        ...
        let mut cval = self.calc_item_attr_val(ctx, item_key, attr_id);
        let item_attr_data = self.attrs.get_item_attr_data_mut(&item_key).unwrap();
        item_attr_data.values.insert(*attr_id, cval);
    ...
    pub(in crate::svc::calc) fn get_item_attr_val_no_pp(
        ...
        let cval = self.calc_item_attr_val(ctx, item_key, attr_id);
        self.attrs
    ...
    fn calc_item_attr_val(&mut self, ctx: SvcCtx, item_key: UItemKey, attr_id: &AAttrId) -> CalcAttrVal {
        ...
        let base_val = match attr_id {
            ...
            &ac::attrs::SECURITY_MODIFIER => {
                ...
                match self.get_item_attr_val_full(ctx, item_key, &security_attr_id) {
                    ...
                    Err(_) => get_base_attr_value(item, attr),
                }
            }
            // Normal attributes
            _ => get_base_attr_value(item, attr),
        };
        // Get base value;
        let mut accumulator = ModAccumFast::new();
        for modification in self.iter_modifications(ctx, &item_key, item, attr_id) {
            accumulator.add_val(
                modification.val,
        ...
        let mut dogma_val = accumulator.apply_dogma_mods(base_val, attr.is_hig());
        // Lower value limit
        ...
        let extra_val = accumulator.apply_extra_mods(dogma_val, attr.is_hig());
        CalcAttrVal {
            base: base_val,
            dogma: dogma_val,
            extra: extra_val,
