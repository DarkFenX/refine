    ...
    match precalc {
        ...
        CycleSeq::LoopLimSin(inner) => {
            if time >= OF(0.0) {
                ...
                let full_repeats = AttrVal::from(get_count_full_repeats(time, full_time, full_tail_time));
                total_amount +=
                ...
                while time >= OF(0.0) {
                    ...
                    let p1_repeats = inner.p1_repeat_count.min(get_count_full_repeats(
                        time,
                        inner.p1_data.time,
    ...
    T: Default + Copy + std::ops::AddAssign<T> + std::ops::Mul<AttrVal, Output = T>,
{
    ...
    let full_repeats = repeat_limit.min(get_count_full_repeats(*time, data.time, data.tail_time));
    ...
    T: Default + Copy + std::ops::AddAssign<T> + std::ops::Mul<AttrVal, Output = T>,
{
    ...
    let full_repeats = AttrVal::from(get_count_full_repeats(*time, data.time, data.tail_time));
...
pub(super) fn get_count_full_repeats(time: AttrVal, cycle_time: AttrVal, cycle_tail_time: AttrVal) -> Count {
