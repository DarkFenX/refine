...
pub(super) fn conv_mutas(erg_data: &Data) -> Vec<ert::Muta> {
    let mut composed = HashMap::new();
    for item_data in erg_data.muta_items.iter() {
        let muta = composed
            .entry(item_data.muta_id)
            .or_insert_with(|| ert::Muta::new(item_data.muta_id));
    ...
    for attr_data in erg_data.muta_attrs.iter() {
        ...
        if let Some(muta) = composed.get_mut(&attr_data.muta_id) {
            muta.attr_mods.insert(
                attr_data.attr_id,
                ert::MutaAttrRange::new(attr_data.min_attr_mult, attr_data.max_attr_mult),
            );
        }
    }
