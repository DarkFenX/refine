    ...
    pub(in crate::api) fn internal_set_module_charge(
        ...
        if let Some(old_charge_key) = u_module.get_charge_key() {
            ...
            if !old_u_charge.get_projs().is_empty() {
                ...
                let old_u_charge = self.u_data.items.get_mut(old_charge_key).dc_charge_mut().unwrap();
        ...
        if !module_projs.is_empty() {
            let u_charge = self.u_data.items.get_mut(new_charge_key).dc_charge_mut().unwrap();
            ...
            let new_u_charge = self.u_data.items.get(new_charge_key).dc_charge().unwrap();
