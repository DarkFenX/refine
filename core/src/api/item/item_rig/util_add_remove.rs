...
impl SolarSystem {
    pub(in crate::api) fn util_add_rig(
        ...
        let u_item = u_data.items.get_mut(rig_uid);
        u_item.update_reffs(reuse_eupdates, &u_data.src);
        SolarSystem::util_add_item(u_data, svc, rig_uid, reuse_eupdates);
    }
    pub(in crate::api) fn util_remove_rig(
        ...
        SolarSystem::util_remove_item(u_data, svc, rig_uid, reuse_eupdates);
    }
}
