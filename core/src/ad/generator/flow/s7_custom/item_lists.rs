...
use crate::{
    ad::{ADataGenerator, AItemList, AItemListItemIds},
    nd::N_ITEM_LIST_MAP,
};

impl ADataGenerator {
    pub(super) fn customize_item_lists(&mut self) {
        for n_item_list in N_ITEM_LIST_MAP.values() {
            if let Some(item_filter) = n_item_list.adg_item_filter_fn {
                let a_item_list = match self.a_data.item_lists.data.entry(n_item_list.aid) {
