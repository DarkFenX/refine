...
use crate::{
    ac,
    ad::{
        AAttrVal, ABuffId, AData, AEffect, AEffectBuff, AEffectBuffDuration, AEffectBuffFull, AEffectBuffScope,
...
fn process_buffs(
    ...
    let valid_buffs = e_sc_buff_data
        .buffs
        .iter()
        .filter(|v| a_data.buffs.contains_key(&ABuffId::Eve(v.id)))
        .collect_vec();
