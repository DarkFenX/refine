...
use crate::{
    ad::{AData, AEffectBuffScope, AItemCatId},
    util::RSet,
};
...
pub(in crate::ad::generator::flow::s8_conv_post) fn fill_buff_item_lists(a_data: &mut AData) {
    ...
    // Calculator module works with assumption that count of item lists for non-ships won't exceed
    // certain count. When that limit is broken, it might make sense to:
