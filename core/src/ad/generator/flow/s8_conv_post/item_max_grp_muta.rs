...
fn get_grp_mutations(a_data: &AData) -> RMapRSet<AItemGrpId, AItemGrpId> {
    ...
    for a_muta in a_data.mutas.values() {
        for (base_item_aid, mutated_item_aid) in a_muta.item_map.iter() {
            let base_grp_aid = match a_data.items.get(base_item_aid) {
                Some(base_item) => base_item.grp_id,
                None => continue,
            };
            let mutated_grp_aid = match a_data.items.get(mutated_item_aid) {
