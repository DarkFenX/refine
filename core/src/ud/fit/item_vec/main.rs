...
impl UItemVec {
    ...
    pub(crate) fn iter_keys(&self) -> impl Iterator<Item = &UItemId> {
        self.data.iter().filter_map(|v| v.as_ref())
    }
    pub(crate) fn iter_keys_from(&self, start: Idx) -> impl Iterator<Item = &UItemId> {
        let start = Idx::min(start, self.data.len());
        self.data[start..].iter().filter_map(|v| v.as_ref())
    }
    pub(crate) fn get(&self, index: Idx) -> Option<UItemId> {
    ...
    pub(crate) fn append(&mut self, val: UItemId) -> Idx {
        let pos = self.data.len();
        self.data.push(Some(val));
        self.item_count += 1;
        pos
    }
    pub(crate) fn equip(&mut self, val: UItemId) -> Idx {
    ...
    pub(crate) fn insert(&mut self, index: Idx, val: UItemId) -> bool {
    ...
    pub(crate) fn place(&mut self, index: Idx, val: UItemId) {
        if index >= self.data.len() {
    ...
    pub(crate) fn remove(&mut self, &val: &UItemId) -> Option<Idx> {
