...
impl UBooster {
    pub(crate) fn new(item_id: ItemId, type_aid: AItemId, fit_uid: UFitId, booster_state: bool, src: &Src) -> Self {
        Self {
            base: UItemBase::new(item_id, type_aid, bool_to_state_offline(booster_state), src),
            fit_uid,
        }
    }
    // Item base methods
    ...
    pub(crate) fn get_slot(&self) -> Option<SlotIndex> {
        match self.get_axt() {
            Some(axt) => axt.booster_slot,
            None => None,
        }
    }
}
impl LibNamed for UBooster {
    fn lib_get_name() -> &'static str {
        "UBooster"
    }
}
impl std::fmt::Display for UBooster {
