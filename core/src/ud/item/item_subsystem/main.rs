...
impl USubsystem {
    pub(crate) fn new(item_id: ItemId, type_aid: AItemId, fit_uid: UFitId, subsystem_state: bool, src: &Src) -> Self {
        Self {
            base: UItemBase::new(item_id, type_aid, bool_to_state_offline(subsystem_state), src),
            fit_uid,
        }
    }
    // Item base methods
    ...
    pub(in crate::ud::item) fn update_a_data(&mut self, src: &Src) {
