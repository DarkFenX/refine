...
impl UAutocharge {
    pub(crate) fn new(
        ...
        Self {
            base: UItemBase::new(item_id, a_item_id, base_a_state, src, reuse_eupdates),
            fit_key,
            cont_key,
            cont_a_effect_id,
            projs: Projs::new(),
    ...
    pub(crate) fn set_a_state(&mut self, state: ad::AState, reuse_eupdates: &mut UEffectUpdates, src: &Src) {
    ...
    pub(in crate::ud::item) fn update_a_data(&mut self, _reuse_eupdates: &mut UEffectUpdates, _src: &Src) {
    ...
    pub(crate) fn get_cont_key(&self) -> UItemKey {
        self.cont_key
    }
