...
impl UProjs {
    ...
    pub(crate) fn add(&mut self, item_key: UItemId, proj_data: Option<UProjData>) {
        self.data.insert(item_key, proj_data);
    }
    pub(crate) fn remove(&mut self, item_key: &UItemId) -> Option<Option<UProjData>> {
        self.data.remove(item_key)
    }
    pub(crate) fn contains(&self, item_key: &UItemId) -> bool {
        self.data.contains_key(item_key)
    }
    pub(crate) fn get(&self, item_key: &UItemId) -> Option<Option<UProjData>> {
        self.data.get(item_key).copied()
    }
    pub(crate) fn get_proj_data_mut(&mut self, item_key: &UItemId) -> Option<&mut UProjData> {
