...
impl UFwEffect {
    pub(crate) fn new(item_id: ItemId, type_id: AItemId, fit_key: UFitKey, fw_effect_state: bool, src: &Src) -> Self {
        Self {
            base: UItemBase::new(item_id, type_id, bool_to_state_active(fw_effect_state), src),
            fit_key,
        }
    }
    // Item base methods
