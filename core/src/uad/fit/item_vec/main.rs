use crate::{def::Idx, uad::UadItemKey};

#[derive(Clone)]
pub(crate) struct UadItemVec {
    pub(super) data: Vec<Option<UadItemKey>>,
    pub(super) item_count: usize,
}
impl UadItemVec {
    pub(in crate::uad::fit) fn new() -> Self {
        Self {
            data: Vec::new(),
            item_count: 0,
        }
    }
    pub(crate) fn iter_all(&self) -> impl ExactSizeIterator<Item = &Option<UadItemKey>> {
        self.data.iter()
    }
    pub(crate) fn iter_keys(&self) -> impl Iterator<Item = &UadItemKey> {
        self.data.iter().filter_map(|v| v.as_ref())
    }
    pub(crate) fn iter_keys_from(&self, start: Idx) -> impl Iterator<Item = &UadItemKey> {
        let start = Idx::min(start, self.data.len());
        self.data[start..].iter().filter_map(|v| v.as_ref())
    }
    pub(crate) fn get(&self, index: Idx) -> Option<UadItemKey> {
    ...
    pub(crate) fn inner(&self) -> &Vec<Option<UadItemKey>> {
        &self.data
    }
    // Modification methods
    pub(crate) fn append(&mut self, val: UadItemKey) -> Idx {
        let pos = self.data.len();
        self.data.push(Some(val));
        self.item_count += 1;
        pos
    }
    pub(crate) fn equip(&mut self, val: UadItemKey) -> Idx {
    ...
    pub(crate) fn insert(&mut self, index: Idx, val: UadItemKey) -> bool {
    ...
    pub(crate) fn place(&mut self, index: Idx, val: UadItemKey) {
        if index >= self.data.len() {
    ...
    pub(crate) fn free(&mut self, &val: &UadItemKey) {
    ...
    pub(crate) fn remove(&mut self, &val: &UadItemKey) -> Option<Idx> {
