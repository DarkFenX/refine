use crate::def::{Idx, ItemKey};

#[derive(Clone)]
pub(crate) struct ItemVec {
...
impl ItemVec {
    ...
    pub(crate) fn iter_all(&self) -> impl ExactSizeIterator<Item = &Option<ItemKey>> {
        self.data.iter()
    }
    pub(crate) fn iter_keys(&self) -> impl Iterator<Item = &ItemKey> {
        self.data.iter().filter_map(|v| v.as_ref())
    }
    pub(crate) fn iter_keys_from(&self, start: Idx) -> impl Iterator<Item = &ItemKey> {
        let start = Idx::min(start, self.data.len());
        self.data[start..].iter().filter_map(|v| v.as_ref())
    }
    pub(crate) fn get(&self, index: Idx) -> Option<ItemKey> {
    ...
    pub(crate) fn inner(&self) -> &Vec<Option<ItemKey>> {
        &self.data
    }
    // Modification methods
    pub(crate) fn append(&mut self, val: ItemKey) -> Idx {
        let pos = self.data.len();
        self.data.push(Some(val));
        self.item_count += 1;
        pos
    }
    pub(crate) fn equip(&mut self, val: ItemKey) -> Idx {
    ...
    pub(crate) fn insert(&mut self, index: Idx, val: ItemKey) -> bool {
    ...
    pub(crate) fn place(&mut self, index: Idx, val: ItemKey) {
        if index >= self.data.len() {
    ...
    pub(crate) fn free(&mut self, &val: &ItemKey) {
    ...
    pub(crate) fn remove(&mut self, &val: &ItemKey) -> Option<Idx> {
