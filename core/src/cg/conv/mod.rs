...
pub(super) fn convert(cg_data: &Data, supp: &Support, cd_cont: &mut ch::Data) {
    cd_cont.items = item::conv_items(cg_data, supp, &mut cd_cont.cg_warns);
    cd_cont.attrs = attr::conv_attrs(cg_data);
    cd_cont.mutas = muta::conv_mutas(cg_data);
    cd_cont.effects = effect::conv_effects(cg_data, &mut cd_cont.cg_warns);
    cd_cont.buffs = buff::conv_buffs(cg_data, &mut cd_cont.cg_warns);
}
