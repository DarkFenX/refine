use crate::{
    ...
    nd::{NEffect, NEffectDmgKind, NEffectHc, eff::shared::proj_mult::get_smartbomb_proj_mult},
    rd::REffect,
    svc::{
        SvcCtx,
        calc::Calc,
...
fn get_dmg_opc(
    ...
    if let Some(projectee_key) = projectee_key {
        ...
        let mult = get_smartbomb_proj_mult(ctx, calc, projector_key, projector_effect, projectee_key, proj_data);
        dmg_em *= mult;
        dmg_therm *= mult;
        dmg_kin *= mult;
        dmg_expl *= mult;
