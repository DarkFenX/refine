...
fn get_resolved_spool(ctx: SvcCtx, calc: &mut Calc, espec: EffectSpec, spool: Option<Spool>) -> Option<ResolvedSpool> {
    ...
    let spool_step = calc
        .get_item_attr_val_extra(ctx, espec.item_key, &ac::attrs::REP_MULT_BONUS_PER_CYCLE)
        .unwrap_or(OF(0.0));
    let spool_max = calc
        .get_item_attr_val_extra(ctx, espec.item_key, &ac::attrs::REP_MULT_BONUS_MAX)
        .unwrap_or(OF(0.0));
    let cycle_time = efuncs::get_espec_cycle_time(ctx, calc, espec).unwrap_or(OF(0.0));
