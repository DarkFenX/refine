...
fn get_dmg_opc(
    ...
    Some(OutputDmgBreacher {
        absolute_max: abs_max,
        relative_max: rel_max,
        // Breachers deal damage every tick, so count of instances depends on server tick rate
        tick_count: trunc_unerr(duration_s / SERVER_TICK_S) as Count,
    })
}
