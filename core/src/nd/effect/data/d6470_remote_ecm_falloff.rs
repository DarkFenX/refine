use crate::{
    ...
    ed::EEffectId,
    misc::{Ecm, EffectSpec},
    nd::{NEffect, NEffectHc, effect::data::shared::proj_mult::get_full_noapp_proj_mult},
    rd::REffect,
    svc::{SvcCtx, calc::Calc, eff_funcs},
    ud::UItemKey,
};

const E_EFFECT_ID: EEffectId = ec::effects::REMOTE_ECM_FALLOFF;
const A_EFFECT_ID: AEffectId = ac::effects::REMOTE_ECM_FALLOFF;
...
fn internal_get_ecm_opc(
    ...
    let duration_s =
        calc.get_item_oattr_afb_oextra(ctx, projector_key, projector_effect.duration_attr_key, OF(0.0))? / OF(1000.0);
