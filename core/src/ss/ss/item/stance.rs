...
impl SolarSystem {
    ...
    pub fn set_fit_stance(&mut self, fit_id: SsFitId, a_item_id: ReeInt, state: bool) -> Result<SsStanceInfo> {
    ...
    fn get_fit_stance_mut(&mut self, fit_id: &SsFitId) -> Result<&mut SsStance> {
        let item_id = self.get_fit_stance_id(fit_id)?;
