...
def get_value(client, consts, stackable):
    ...
    eve_item_tgt = client.mk_eve_item(attrs={eve_tgt_attr.id: 100}, eff_ids=[eve_effect.id])
    client.create_sources()
    api_ss = client.create_ss()
    api_fit = api_ss.create_fit()
    api_fit.add_high_mod(module_id=eve_item_src1.id)
    api_fit.add_high_mod(module_id=eve_item_src2.id)
    api_fit.add_high_mod(module_id=eve_item_src3.id)
    api_item_tgt = api_fit.add_high_mod(module_id=eve_item_tgt.id)
