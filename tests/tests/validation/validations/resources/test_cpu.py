...
def test_equal(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (150, 150)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
...
def test_modified_use(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (75, 125)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_modified_max(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (150, 120)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is False
    assert api_val.details.cpu.used == 150
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (150, 180)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_mutation_use(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (120, 125)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (129.6, 125)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is False
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (134.4, 125)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is False
...
def test_rounding(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (5.24, 5.23)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is False
...
def test_sum_rounding(client, consts):
    ...
    for i in range(1, 21):
        ...
        # Verification
        api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
        assert api_stats.cpu.one() == (round(i / 10, 1), 0.15)
        api_val = api_fit.validate(options=ValOptions(cpu=True))
        assert api_val.passed is False
...
def test_no_ship(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (5, None)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is False
...
def test_not_loaded_user(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (0, 125)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_non_positive(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (140, 125)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is False
...
def test_no_value_max(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=True))
    assert api_stats.cpu.one() == (150, 0)
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is False
    assert api_val.details.cpu.used == 150
...
def test_no_attr_max(client, consts):
    ...
    api_val = api_fit.validate(options=ValOptions(cpu=True))
    assert api_val.passed is False
    assert api_val.details.cpu.used == 50
    assert api_val.details.cpu.max == 0
    assert api_val.details.cpu.users == {api_module.id: 50}
...
def test_stat_not_requested(client, consts):
    ...
    api_fit.add_module(type_id=eve_module_id, state=consts.ApiModuleState.online)
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(cpu=False))
    with check_no_field():
        api_stats.cpu  # ruff:ignore[useless-expression]
