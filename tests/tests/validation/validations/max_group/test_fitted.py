...
def test_known_failures(client, consts):
    ...
    api_module2 = api_fit.add_module(type_id=eve_module_id, state=consts.ApiModuleState.offline)
    # Verification
    api_val = api_fit.validate(options=ValOptions(max_group_fitted=[api_module1.id]))
    assert api_val.passed is False
    assert api_val.details.max_group_fitted == {eve_grp_id: [2, {api_module2.id: 1}]}
    api_val = api_fit.validate(options=ValOptions(max_group_fitted=[api_module2.id]))
    assert api_val.passed is False
    assert api_val.details.max_group_fitted == {eve_grp_id: [2, {api_module1.id: 1}]}
    api_val = api_fit.validate(options=ValOptions(max_group_fitted=[api_module1.id, api_module2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    api_val = api_fit.validate(options=ValOptions(max_group_fitted=[api_module1.id, api_other.id, api_module2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_not_loaded(client, consts):
    client.mk_eve_attr(id_=consts.EveAttr.max_group_fitted)
