...
def test_fail_single(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_support_fighters=True))
    assert api_stats.launched_support_fighters.one() == (1, 0)
    api_val = api_fit.validate(options=ValOptions(launched_support_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_support_fighter_count.used == 1
    assert api_val.details.launched_support_fighter_count.max == 0
    assert api_val.details.launched_support_fighter_count.users == [api_fighter.id]
...
def test_fail_multiple_ship(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_support_fighters=True))
    assert api_stats.launched_support_fighters.one() == (2, 1)
    api_val = api_fit.validate(options=ValOptions(launched_support_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_support_fighter_count.used == 2
    assert api_val.details.launched_support_fighter_count.max == 1
    assert api_val.details.launched_support_fighter_count.users == sorted([api_fighter1.id, api_fighter2.id])
...
def test_equal(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_support_fighters=True))
    assert api_stats.launched_support_fighters.one() == (1, 1)
    api_val = api_fit.validate(options=ValOptions(launched_support_fighter_count=True))
...
def test_known_failures(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_support_fighters=True))
    assert api_stats.launched_support_fighters.one() == (2, 1)
    api_val = api_fit.validate(options=ValOptions(launched_support_fighter_count=[api_fighter1.id]))
    assert api_val.passed is False
    assert api_val.details.launched_support_fighter_count.used == 2
    assert api_val.details.launched_support_fighter_count.max == 1
    assert api_val.details.launched_support_fighter_count.users == [api_fighter2.id]
    api_val = api_fit.validate(options=ValOptions(launched_support_fighter_count=[api_fighter2.id]))
    assert api_val.passed is False
    assert api_val.details.launched_support_fighter_count.used == 2
    assert api_val.details.launched_support_fighter_count.max == 1
    assert api_val.details.launched_support_fighter_count.users == [api_fighter1.id]
    api_val = api_fit.validate(options=ValOptions(launched_support_fighter_count=[api_fighter1.id, api_fighter2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    api_val = api_fit.validate(options=ValOptions(
        launched_support_fighter_count=[api_fighter1.id, api_other.id, api_fighter2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_fractional_fighter_type(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_support_fighters=True))
    assert api_stats.launched_support_fighters.one() == (3, 2)
    api_val = api_fit.validate(options=ValOptions(launched_support_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_support_fighter_count.used == 3
    assert api_val.details.launched_support_fighter_count.max == 2
    assert api_val.details.launched_support_fighter_count.users == sorted([
...
def test_no_ship(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_support_fighters=True))
    assert api_stats.launched_support_fighters.one() == (1, None)
    api_val = api_fit.validate(options=ValOptions(launched_support_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_support_fighter_count.used == 1
    assert api_val.details.launched_support_fighter_count.max is None
    assert api_val.details.launched_support_fighter_count.users == [api_fighter.id]
...
def test_not_loaded_user(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_support_fighters=True))
    assert api_stats.launched_support_fighters.one() == (0, 0)
    api_val = api_fit.validate(options=ValOptions(launched_support_fighter_count=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
