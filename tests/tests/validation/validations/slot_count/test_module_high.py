...
def test_fail_multiple_ship(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(high_slots=True))
    assert api_stats.high_slots.one() == (3, 1)
    api_val = api_fit.validate(options=ValOptions(high_slot_count=True))
    assert api_val.passed is False
...
def test_holes(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(high_slots=True))
    assert api_stats.high_slots.one() == (6, 3)
    api_val = api_fit.validate(options=ValOptions(high_slot_count=True))
    assert api_val.passed is False
...
def test_equal(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(high_slots=True))
    assert api_stats.high_slots.one() == (1, 1)
    api_val = api_fit.validate(options=ValOptions(high_slot_count=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_no_ship(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(high_slots=True))
    assert api_stats.high_slots.one() == (1, None)
    api_val = api_fit.validate(options=ValOptions(high_slot_count=True))
    assert api_val.passed is False
    assert api_val.details.high_slot_count.used == 1
...
def test_criterion_rack(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(high_slots=True))
    assert api_stats.high_slots.one() == (0, 0)
    api_val = api_fit.validate(options=ValOptions(high_slot_count=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_stat_not_requested(client, consts):
    ...
    api_fit.add_module(type_id=eve_module_id, rack=consts.ApiRack.high, state=consts.ApiModuleState.offline)
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(high_slots=False))
    with check_no_field():
        api_stats.high_slots  # ruff:ignore[useless-expression]
