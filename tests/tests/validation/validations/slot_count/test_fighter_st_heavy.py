...
def test_fail_single(client, consts):
    ...
    assert api_stats.launched_st_heavy_fighters.one() == (1, 0)
    api_val = api_fit.validate(options=ValOptions(launched_st_heavy_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_st_heavy_fighter_count.used == 1
    assert api_val.details.launched_st_heavy_fighter_count.max == 0
    assert api_val.details.launched_st_heavy_fighter_count.users == [api_fighter.id]
...
def test_fail_multiple_ship(client, consts):
    ...
    assert api_val.details.launched_st_heavy_fighter_count.users == sorted([api_fighter1.id, api_fighter2.id])
...
def test_equal(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_heavy_fighters=True))
    assert api_stats.launched_st_heavy_fighters.one() == (1, 1)
    api_val = api_fit.validate(options=ValOptions(launched_st_heavy_fighter_count=True))
...
def test_fractional_fighter_type(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_heavy_fighters=True))
    assert api_stats.launched_st_heavy_fighters.one() == (3, 2)
    api_val = api_fit.validate(options=ValOptions(launched_st_heavy_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_st_heavy_fighter_count.used == 3
    assert api_val.details.launched_st_heavy_fighter_count.max == 2
    assert api_val.details.launched_st_heavy_fighter_count.users == sorted([
...
def test_no_ship(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_heavy_fighters=True))
    assert api_stats.launched_st_heavy_fighters.one() == (1, None)
    api_val = api_fit.validate(options=ValOptions(launched_st_heavy_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_st_heavy_fighter_count.used == 1
    assert api_val.details.launched_st_heavy_fighter_count.max is None
    assert api_val.details.launched_st_heavy_fighter_count.users == [api_fighter.id]
...
def test_not_loaded_user(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_heavy_fighters=True))
    assert api_stats.launched_st_heavy_fighters.one() == (0, 0)
    api_val = api_fit.validate(options=ValOptions(launched_st_heavy_fighter_count=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_stat_not_requested(client, consts):
    ...
    api_fit.add_fighter(type_id=eve_fighter_id, state=consts.ApiMinionState.in_space)
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_heavy_fighters=False))
    with check_no_field():
        api_stats.launched_st_heavy_fighters  # ruff:ignore[useless-expression]
