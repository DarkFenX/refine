...
def test_known_failures(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_fighters=True))
    assert api_stats.launched_fighters.one() == (2, 1)
    api_val = api_fit.validate(options=ValOptions(launched_fighter_count=[api_fighter1.id]))
    assert api_val.passed is False
    assert api_val.details.launched_fighter_count.used == 2
    assert api_val.details.launched_fighter_count.max == 1
    assert api_val.details.launched_fighter_count.users == [api_fighter2.id]
    api_val = api_fit.validate(options=ValOptions(launched_fighter_count=[api_fighter2.id]))
    assert api_val.passed is False
    assert api_val.details.launched_fighter_count.used == 2
    assert api_val.details.launched_fighter_count.max == 1
    assert api_val.details.launched_fighter_count.users == [api_fighter1.id]
    api_val = api_fit.validate(options=ValOptions(launched_fighter_count=[api_fighter1.id, api_fighter2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    api_val = api_fit.validate(options=ValOptions(
        launched_fighter_count=[api_fighter1.id, api_other.id, api_fighter2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_criterion_item_kind(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_fighters=True))
    assert api_stats.launched_fighters.one() == (1, 0)
    api_val = api_fit.validate(options=ValOptions(launched_fighter_count=[api_fighter.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_stat_not_requested(client, consts):
    ...
    api_fit.add_fighter(type_id=eve_fighter_id, state=consts.ApiMinionState.in_space)
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_fighters=False))
    with check_no_field():
        api_stats.launched_fighters  # ruff:ignore[useless-expression]
