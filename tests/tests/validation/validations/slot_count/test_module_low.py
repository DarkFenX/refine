...
def test_fail_single(client, consts):
    ...
    api_module = api_fit.add_module(type_id=eve_module_id, rack=consts.ApiRack.low, state=consts.ApiModuleState.offline)
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (1, 0)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=True))
    assert api_val.passed is False
    assert api_val.details.low_slot_count.used == 1
    assert api_val.details.low_slot_count.max == 0
...
def test_fail_multiple_ship(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (3, 1)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=True))
    assert api_val.passed is False
    assert api_val.details.low_slot_count.used == 3
    assert api_val.details.low_slot_count.max == 1
    assert api_val.details.low_slot_count.users == sorted([api_module2.id, api_module3.id])
...
def test_holes(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (6, 3)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=True))
    assert api_val.passed is False
    assert api_val.details.low_slot_count.used == 6
    assert api_val.details.low_slot_count.max == 3
    assert api_val.details.low_slot_count.users == [api_module1.id]
    # Action
    api_fit.add_module(
        type_id=eve_module_id,
        rack=consts.ApiRack.low,
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (7, 3)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=True))
    assert api_val.passed is False
    assert api_val.details.low_slot_count.used == 7
    assert api_val.details.low_slot_count.max == 3
    assert api_val.details.low_slot_count.users == [api_module1.id]
    # Action
    api_module1.remove()
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (3, 3)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=True))
...
def test_equal(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (1, 1)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_known_failures(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (1, 3)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=[api_module1.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    # Action
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (3, 3)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=[api_module2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    # Action
    ...
    assert api_val.details.low_slot_count.used == 7
    assert api_val.details.low_slot_count.max == 3
    assert api_val.details.low_slot_count.users == [api_module3.id]
    api_val = api_fit.validate(options=ValOptions(low_slot_count=[api_module3.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    # Action
    ...
    assert api_val.details.low_slot_count.used == 7
    assert api_val.details.low_slot_count.max == 3
    assert api_val.details.low_slot_count.users == [api_module4.id]
    api_val = api_fit.validate(options=ValOptions(low_slot_count=[api_module4.id]))
    assert api_val.passed is False
    assert api_val.details.low_slot_count.used == 7
    assert api_val.details.low_slot_count.max == 3
    assert api_val.details.low_slot_count.users == [api_module3.id]
    api_val = api_fit.validate(options=ValOptions(low_slot_count=[api_module3.id, api_module4.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    api_val = api_fit.validate(options=ValOptions(low_slot_count=[api_module3.id, api_other.id, api_module4.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    # Action
...
def test_no_ship(client, consts):
    client.mk_eve_attr(id_=consts.EveAttr.low_slots)
    eve_module_id = client.mk_eve_item()
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (1, None)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=True))
    assert api_val.passed is False
    assert api_val.details.low_slot_count.used == 1
    assert api_val.details.low_slot_count.max is None
    assert api_val.details.low_slot_count.users == [api_module.id]
...
def test_criterion_rack(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (0, 0)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_criterion_item_kind(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (1, 0)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=[api_module.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_stat_not_requested(client, consts):
    ...
    api_fit.add_module(type_id=eve_module_id, rack=consts.ApiRack.low, state=consts.ApiModuleState.offline)
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=False))
    with check_no_field():
        api_stats.low_slots  # ruff:ignore[useless-expression]
