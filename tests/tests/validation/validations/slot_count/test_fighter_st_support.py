...
def test_fail_single(client, consts):
    eve_ftr_type_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_sq_is_st_support)
    eve_max_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_st_support_slots)
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_support_fighters=True))
    assert api_stats.launched_st_support_fighters.one() == (1, 0)
    api_val = api_fit.validate(options=ValOptions(launched_st_support_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_st_support_fighter_count.used == 1
    assert api_val.details.launched_st_support_fighter_count.max == 0
    assert api_val.details.launched_st_support_fighter_count.users == [api_fighter.id]
...
def test_fail_multiple_ship(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_support_fighters=True))
    assert api_stats.launched_st_support_fighters.one() == (2, 1)
    api_val = api_fit.validate(options=ValOptions(launched_st_support_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_st_support_fighter_count.used == 2
    assert api_val.details.launched_st_support_fighter_count.max == 1
    assert api_val.details.launched_st_support_fighter_count.users == sorted([api_fighter1.id, api_fighter2.id])
...
def test_equal(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_support_fighters=True))
    assert api_stats.launched_st_support_fighters.one() == (1, 1)
    api_val = api_fit.validate(options=ValOptions(launched_st_support_fighter_count=True))
...
def test_known_failures(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_support_fighters=True))
    assert api_stats.launched_st_support_fighters.one() == (2, 1)
    api_val = api_fit.validate(options=ValOptions(launched_st_support_fighter_count=[api_fighter1.id]))
    assert api_val.passed is False
    assert api_val.details.launched_st_support_fighter_count.used == 2
    assert api_val.details.launched_st_support_fighter_count.max == 1
    assert api_val.details.launched_st_support_fighter_count.users == [api_fighter2.id]
    api_val = api_fit.validate(options=ValOptions(launched_st_support_fighter_count=[api_fighter2.id]))
    assert api_val.passed is False
    assert api_val.details.launched_st_support_fighter_count.used == 2
    assert api_val.details.launched_st_support_fighter_count.max == 1
    assert api_val.details.launched_st_support_fighter_count.users == [api_fighter1.id]
    api_val = api_fit.validate(options=ValOptions(launched_st_support_fighter_count=[api_fighter1.id, api_fighter2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    api_val = api_fit.validate(options=ValOptions(
        launched_st_support_fighter_count=[api_fighter1.id, api_other.id, api_fighter2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_fractional_fighter_type(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_support_fighters=True))
    assert api_stats.launched_st_support_fighters.one() == (3, 2)
    api_val = api_fit.validate(options=ValOptions(launched_st_support_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_st_support_fighter_count.used == 3
    assert api_val.details.launched_st_support_fighter_count.max == 2
    assert api_val.details.launched_st_support_fighter_count.users == sorted([
...
def test_no_ship(client, consts):
    eve_ftr_type_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_sq_is_st_support)
    client.mk_eve_attr(id_=consts.EveAttr.ftr_st_support_slots)
    eve_fighter_id = client.mk_eve_item(attrs={eve_ftr_type_attr_id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_support_fighters=True))
    assert api_stats.launched_st_support_fighters.one() == (1, None)
    api_val = api_fit.validate(options=ValOptions(launched_st_support_fighter_count=True))
    assert api_val.passed is False
    assert api_val.details.launched_st_support_fighter_count.used == 1
    assert api_val.details.launched_st_support_fighter_count.max is None
    assert api_val.details.launched_st_support_fighter_count.users == [api_fighter.id]
...
def test_no_attr_max(client, consts):
    eve_ftr_type_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_sq_is_st_support)
    eve_max_attr_id = consts.EveAttr.ftr_st_support_slots
    eve_fighter_id = client.mk_eve_item(attrs={eve_ftr_type_attr_id: 1})
...
def test_no_attr_fighter_type(client, consts):
    eve_ftr_type_attr_id = consts.EveAttr.ftr_sq_is_st_support
    eve_max_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_st_support_slots)
    eve_fighter_id = client.mk_eve_item(attrs={eve_ftr_type_attr_id: 1})
    eve_ship_id = client.mk_eve_ship(attrs={eve_max_attr_id: 0})
    client.create_sources()
...
def test_not_loaded_user(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_support_fighters=True))
    assert api_stats.launched_st_support_fighters.one() == (0, 0)
    api_val = api_fit.validate(options=ValOptions(launched_st_support_fighter_count=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_criterion_fighter_type(client, consts):
    eve_ftr_type_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_sq_is_st_support)
    eve_other_ftr_type_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_sq_is_st_light)
    eve_max_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_st_support_slots)
...
def test_stat_not_requested(client, consts):
    ...
    api_fit.add_fighter(type_id=eve_fighter_id, state=consts.ApiMinionState.in_space)
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(launched_st_support_fighters=False))
    with check_no_field():
        api_stats.launched_st_support_fighters  # ruff:ignore[useless-expression]
