...
def test_fail_single(client, consts):
    ...
    assert api_stats.mid_slots.one() == (1, 0)
...
def test_fail_multiple_ship(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(mid_slots=True))
    assert api_stats.mid_slots.one() == (3, 1)
    api_val = api_fit.validate(options=ValOptions(mid_slot_count=True))
    assert api_val.passed is False
...
def test_holes(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(mid_slots=True))
    assert api_stats.mid_slots.one() == (6, 3)
    api_val = api_fit.validate(options=ValOptions(mid_slot_count=True))
    assert api_val.passed is False
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(mid_slots=True))
    assert api_stats.mid_slots.one() == (7, 3)
    api_val = api_fit.validate(options=ValOptions(mid_slot_count=True))
    assert api_val.passed is False
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(mid_slots=True))
    assert api_stats.mid_slots.one() == (3, 3)
    api_val = api_fit.validate(options=ValOptions(mid_slot_count=True))
...
def test_equal(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(mid_slots=True))
    assert api_stats.mid_slots.one() == (1, 1)
    api_val = api_fit.validate(options=ValOptions(mid_slot_count=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_known_failures(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(mid_slots=True))
    assert api_stats.mid_slots.one() == (1, 3)
    ...
    api_val = api_fit.validate(options=ValOptions(mid_slot_count=[api_module4.id]))
    assert api_val.passed is False
    assert api_val.details.mid_slot_count.used == 7
    assert api_val.details.mid_slot_count.max == 3
    assert api_val.details.mid_slot_count.users == [api_module3.id]
...
def test_no_ship(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(mid_slots=True))
    assert api_stats.mid_slots.one() == (1, None)
    api_val = api_fit.validate(options=ValOptions(mid_slot_count=True))
    assert api_val.passed is False
    assert api_val.details.mid_slot_count.used == 1
...
def test_criterion_rack(client, consts):
    ...
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(mid_slots=True))
    assert api_stats.mid_slots.one() == (0, 0)
    api_val = api_fit.validate(options=ValOptions(mid_slot_count=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_stat_not_requested(client, consts):
    ...
    api_fit.add_module(type_id=eve_module_id, rack=consts.ApiRack.mid, state=consts.ApiModuleState.offline)
    # Verification
    api_stats = api_fit.get_stats(options=FitStatsOptions(mid_slots=False))
    with check_no_field():
        api_stats.mid_slots  # ruff:ignore[useless-expression]
