...
def test_known_failures(client, consts):
    ...
    # Verification
    api_val = api_src_fit.validate(options=ValOptions(assist_immunity=[api_src_item1.id]))
    assert api_val.passed is False
    assert api_val.details.assist_immunity == {api_src_item2.id: [api_tgt_item.id]}
    api_val = api_src_fit.validate(options=ValOptions(assist_immunity=[api_src_item2.id]))
    assert api_val.passed is False
    assert api_val.details.assist_immunity == {api_src_item1.id: [api_tgt_item.id]}
    api_val = api_src_fit.validate(options=ValOptions(assist_immunity=[api_src_item1.id, api_src_item2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    api_val = api_src_fit.validate(options=ValOptions(
        assist_immunity=[api_src_item1.id, api_src_other.id, api_src_item2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
