...
def test_known_failures(client, consts):
    ...
    # Verification
    api_val = api_fit.validate(options=ValOptions(charge_size=[api_module2.charge.id]))
    assert api_val.passed is False
    assert api_val.details.charge_size == {api_module3.charge.id: (api_module3.id, 3, 2)}
    api_val = api_fit.validate(options=ValOptions(charge_size=[api_module3.charge.id]))
    assert api_val.passed is False
    assert api_val.details.charge_size == {api_module2.charge.id: (api_module2.id, 3, 2)}
    api_val = api_fit.validate(options=ValOptions(charge_size=[api_module2.charge.id, api_module3.charge.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    api_val = api_fit.validate(options=ValOptions(
        charge_size=[api_module2.charge.id, api_other.id, api_module3.charge.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
...
def test_no_attr(client, consts):
    ...
    api_fit = api_sol.create_fit()
    api_fit.add_module(type_id=eve_module_id, charge_type_id=eve_charge_id)
    # Verification - validation is ignored if attribute does not exist
    api_val = api_fit.validate(options=ValOptions(charge_size=True))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
