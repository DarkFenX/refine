...
def test_not_requested(client, consts):
    ...
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
    # Verification
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(sig_radius=False))
    with check_no_field():
        api_fit_stats.sig_radius  # ruff:ignore[useless-expression]
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(sig_radius=False))
    with check_no_field():
        api_ship_stats.sig_radius  # ruff:ignore[useless-expression]
