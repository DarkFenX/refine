...
def test_time(client, consts):
    ...
    api_fleet_dmg_stats = api_fleet.get_stats(options=FleetStatsOptions(
        dmg=[StatsOptionFitDmg(time_options=StatTimeSim(time=77))])).dmg.one()
    ...
    assert api_fit_dmg_stats.volley == [approx(7250), 0, 0, 0]
    api_charge_dmg_stats = api_module.charge.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(time_options=StatTimeSim(time=77))])).dmg.one()
    assert api_charge_dmg_stats.dps == [approx(94.155844), 0, 0, 0]
    assert api_charge_dmg_stats.volley == [approx(7250), 0, 0, 0]
    # Verification - just after second hit
    api_fleet_dmg_stats = api_fleet.get_stats(options=FleetStatsOptions(
        dmg=[StatsOptionFitDmg(time_options=StatTimeSim(time=78))])).dmg.one()
    ...
    assert api_fit_dmg_stats.volley == [approx(7250), 0, 0, 0]
    api_charge_dmg_stats = api_module.charge.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(time_options=StatTimeSim(time=78))])).dmg.one()
    assert api_charge_dmg_stats.dps == [approx(185.897436), 0, 0, 0]
    assert api_charge_dmg_stats.volley == [approx(7250), 0, 0, 0]
    # Verification - before 5th hit
    api_fleet_dmg_stats = api_fleet.get_stats(options=FleetStatsOptions(
        dmg=[StatsOptionFitDmg(time_options=StatTimeSim(time=309))])).dmg.one()
    ...
    assert api_fit_dmg_stats.volley == [approx(7250), 0, 0, 0]
    api_charge_dmg_stats = api_module.charge.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(time_options=StatTimeSim(time=309))])).dmg.one()
    ...
    # top of it.
    api_fleet_dmg_stats = api_fleet.get_stats(options=FleetStatsOptions(
        dmg=[StatsOptionFitDmg(time_options=StatTimeSim(time=311))])).dmg.one()
    ...
    assert api_fit_dmg_stats.volley == [approx(7250), 0, 0, 0]
    api_charge_dmg_stats = api_module.charge.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(time_options=StatTimeSim(time=311))])).dmg.one()
