...
from tests.stats.dmg import make_eve_dd_reaper, make_eve_drone, make_eve_ship, setup_dmg_basics
...
def test_range(client, consts):
    ...
    eve_module_id = make_eve_dd_reaper(
        ...
        dmg_interval=1000,
        dmg_duration=10000,
        range_optimal=150000,
        range_radius=5000,
        dmg_radius=3500)
    eve_src_ship_id = make_eve_ship(client=client, basic_info=eve_basic_info, radius=8900)
    ...
    # Verification - target within attacking ship radius still affected by the slash. Tested on
    # Singularity on 2026-07-26 by relative-warping Leviathan onto bookmark in the center of a frig
    # and DD'ing
    ...
    assert api_fleet_dmg_stats.dps == [0, 0, 0, approx(1718.75)]
    assert api_fleet_dmg_stats.volley == [0, 0, 0, approx(412500)]
    api_fit_dmg_stats = api_src_fit.get_stats(options=FitStatsOptions(
        dmg=[StatsOptionFitDmg(projectee_item_id=api_tgt_ship.id)])).dmg.one()
    assert api_fit_dmg_stats.dps == [0, 0, 0, approx(1718.75)]
    assert api_fit_dmg_stats.volley == [0, 0, 0, approx(412500)]
    ...
    assert api_module_proj_dmg_stats.dps == [0, 0, 0, approx(859.375)]
    assert api_module_proj_dmg_stats.volley == [0, 0, 0, approx(206250)]
    api_module_nonproj_dmg_stats = api_src_module_nonproj.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(projectee_item_id=api_tgt_ship.id)])).dmg.one()
    assert api_module_nonproj_dmg_stats.dps == [0, 0, 0, approx(859.375)]
    assert api_module_nonproj_dmg_stats.volley == [0, 0, 0, approx(206250)]
    # Action
    api_tgt_ship.change_ship(coordinates=(0, 166899, 0))
    # Verification - within surface-to-surface range of optimal + damage radius. Damage radius is
    # added to slash range, most likely like a semi-sphere. Tested on Singularity on 2026-07-26 by
    # putting target at ~104500 meters surface-to-surface range, and then moving target out of
    ...
    assert api_module_nonproj_dmg_stats.volley == [0, 0, 0, approx(206250)]
    # Action
    api_tgt_ship.change_ship(coordinates=(0, 166901, 0))
    # Verification - slightly out of range
    api_fleet_dmg_stats = api_fleet.get_stats(options=FleetStatsOptions(
        dmg=[StatsOptionFitDmg(projectee_item_id=api_tgt_ship.id)])).dmg.one()
...
def test_application(client, consts):
    ...
    api_fleet_dmg_stats = api_fleet.get_stats(options=FleetStatsOptions(
        dmg=[StatsOptionFitDmg(projectee_item_id=api_tgt_ship.id)])).dmg.one()
    assert api_fleet_dmg_stats.dps == [0, 0, 0, approx(145.357143)]
    assert api_fleet_dmg_stats.volley == [0, 0, 0, approx(34885.714286)]
    api_fit_dmg_stats = api_src_fit.get_stats(options=FitStatsOptions(
        dmg=[StatsOptionFitDmg(projectee_item_id=api_tgt_ship.id)])).dmg.one()
    assert api_fit_dmg_stats.dps == [0, 0, 0, approx(145.357143)]
    assert api_fit_dmg_stats.volley == [0, 0, 0, approx(34885.714286)]
    api_module_proj_dmg_stats = api_src_module_proj.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(projectee_item_id=api_tgt_ship.id)])).dmg.one()
    assert api_module_proj_dmg_stats.dps == [0, 0, 0, approx(72.678571)]
    assert api_module_proj_dmg_stats.volley == [0, 0, 0, approx(17442.857143)]
    api_module_nonproj_dmg_stats = api_src_module_nonproj.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(projectee_item_id=api_tgt_ship.id)])).dmg.one()
    assert api_module_nonproj_dmg_stats.dps == [0, 0, 0, approx(72.678571)]
    assert api_module_nonproj_dmg_stats.volley == [0, 0, 0, approx(17442.857143)]
    # Action
...
def test_npc_prop_mode(client, consts):
    ...
    assert api_module_proj_dmg_stats.dps == [0, 0, 0, approx(24.553571)]
    assert api_module_proj_dmg_stats.volley == [0, 0, 0, approx(5892.857143)]
    api_module_nonproj_dmg_stats = api_src_module_nonproj.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(projectee_item_id=api_tgt_drone.id)])).dmg.one()
    assert api_module_nonproj_dmg_stats.dps == [0, 0, 0, approx(24.553571)]
    assert api_module_nonproj_dmg_stats.volley == [0, 0, 0, approx(5892.857143)]
    # Action
    api_tgt_drone.change_drone(npc_prop=consts.ApiNpcProp.chase)
    # Verification
    api_module_proj_dmg_stats = api_src_module_proj.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(projectee_item_id=api_tgt_drone.id)])).dmg.one()
    assert api_module_proj_dmg_stats.dps == [0, 0, 0, approx(122.767857)]
    assert api_module_proj_dmg_stats.volley == [0, 0, 0, approx(29464.285714)]
    api_module_nonproj_dmg_stats = api_src_module_nonproj.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(projectee_item_id=api_tgt_drone.id)])).dmg.one()
    assert api_module_nonproj_dmg_stats.dps == [0, 0, 0, approx(122.767857)]
    assert api_module_nonproj_dmg_stats.volley == [0, 0, 0, approx(29464.285714)]
    # Action
...
def test_dd_attr_range_optimal_absent(client, consts):
    ...
    assert api_module_nonproj_dmg_stats.volley == [0, 0, 0, approx(206250)]
    # Action
    api_tgt_ship.change_ship(coordinates=(0, 16899, 0))
    # Verification
    api_module_proj_dmg_stats = api_src_module_proj.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(projectee_item_id=api_tgt_ship.id)])).dmg.one()
    assert api_module_proj_dmg_stats.dps == [0, 0, 0, approx(859.375)]
    ...
    assert api_module_nonproj_dmg_stats.volley == [0, 0, 0, approx(206250)]
    # Action
    api_tgt_ship.change_ship(coordinates=(0, 16901, 0))
    # Verification
    api_module_proj_dmg_stats = api_src_module_proj.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(projectee_item_id=api_tgt_ship.id)])).dmg.one()
    assert api_module_proj_dmg_stats.dps == [0, 0, 0, 0]
...
def test_dd_attr_range_radius_absent(client, consts):
    # No radius range defined - it is considered equal to be 0
    eve_basic_info = setup_dmg_basics(client=client, consts=consts)
    eve_module_id = make_eve_dd_reaper(
        client=client,
        basic_info=eve_basic_info,
    ...
    assert api_module_nonproj_dmg_stats.volley == [0, 0, 0, approx(206250)]
    # Action
    api_tgt_ship.change_ship(coordinates=(0, 161899, 0))
    # Verification
    api_module_proj_dmg_stats = api_src_module_proj.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(projectee_item_id=api_tgt_ship.id)])).dmg.one()
    assert api_module_proj_dmg_stats.dps == [0, 0, 0, approx(859.375)]
    ...
    assert api_module_nonproj_dmg_stats.volley == [0, 0, 0, approx(206250)]
    # Action
    api_tgt_ship.change_ship(coordinates=(0, 161901, 0))
    # Verification
    api_module_proj_dmg_stats = api_src_module_proj.get_stats(options=ItemStatsOptions(
        dmg=[StatsOptionItemDmg(projectee_item_id=api_tgt_ship.id)])).dmg.one()
    assert api_module_proj_dmg_stats.dps == [0, 0, 0, 0]
