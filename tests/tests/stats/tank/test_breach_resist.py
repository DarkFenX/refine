...
def test_ship_modified(client, consts):
    eve_resist_attr_id = client.mk_eve_attr(id_=consts.EveAttr.breacher_pod_dmg_resist)
    ...
    eve_rig_id = client.mk_eve_item(attrs={eve_mod_attr_id: -90}, eff_ids=[eve_mod_effect_id])
...
def test_ship_no_value(client, consts):
    client.mk_eve_attr(id_=consts.EveAttr.breacher_pod_dmg_resist, def_val=1)
...
def test_ship_absent(client, consts):
    client.mk_eve_attr(id_=consts.EveAttr.breacher_pod_dmg_resist)
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    # Verification
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(breach_resist=True))
    assert api_fit_stats.breach_resist is None
...
def test_incorrect_item_kind(client, consts):
    ...
    # Verification
    api_drone_stats = api_drone.get_stats(options=ItemStatsOptions(breach_resist=True))
    assert api_drone_stats.breach_resist is None
...
def test_ship_not_loaded(client, consts):
    ...
    assert api_fit_stats.breach_resist is None
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(breach_resist=True))
    assert api_ship_stats.breach_resist is None
...
def test_not_requested(client, consts):
    ...
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
    # Verification
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(breach_resist=False))
    with check_no_field():
        api_fit_stats.breach_resist  # ruff:ignore[useless-expression]
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(breach_resist=False))
    with check_no_field():
        api_ship_stats.breach_resist  # ruff:ignore[useless-expression]
