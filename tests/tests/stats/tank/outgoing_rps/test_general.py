...
def test_not_requested(client, consts):
    ...
    with check_no_field():
        api_fleet_stats.outgoing_rps  # ruff:ignore[useless-expression]
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(outgoing_rps=False))
    with check_no_field():
        api_fit_stats.outgoing_rps  # ruff:ignore[useless-expression]
    api_module_stats = api_module.get_stats(options=ItemStatsOptions(outgoing_rps=False))
    with check_no_field():
        api_module_stats.outgoing_rps  # ruff:ignore[useless-expression]
