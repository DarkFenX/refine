...
def test_ship_tether_modified(client, consts):
    ...
    eve_mod = client.mk_eve_effect_mod(
        ...
        affectee_attr_id=eve_tether_attr_id)
...
def test_ship_tether_values(client, consts):
    eve_tether_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_tethering)
    eve_warp_attr_id = client.mk_eve_attr(id_=consts.EveAttr.warp_scramble_status)
    eve_ship1_id = client.mk_eve_ship(attrs={eve_warp_attr_id: 0, eve_tether_attr_id: -100})
    eve_ship2_id = client.mk_eve_ship(attrs={eve_warp_attr_id: 0, eve_tether_attr_id: 0})
    eve_ship3_id = client.mk_eve_ship(attrs={eve_warp_attr_id: 0, eve_tether_attr_id: 100})
...
def test_ship_tether_no_attr(client, consts):
    eve_tether_attr_id = consts.EveAttr.disallow_tethering
    eve_warp_attr_id = client.mk_eve_attr(id_=consts.EveAttr.warp_scramble_status)
    eve_ship_id = client.mk_eve_ship(attrs={eve_tether_attr_id: 1, eve_warp_attr_id: 0})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
...
def test_ship_warp_values(client, consts):
    eve_tether_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_tethering)
    eve_warp_attr_id = client.mk_eve_attr(id_=consts.EveAttr.warp_scramble_status)
    eve_ship1_id = client.mk_eve_ship(attrs={eve_tether_attr_id: 0, eve_warp_attr_id: -100})
    eve_ship2_id = client.mk_eve_ship(attrs={eve_tether_attr_id: 0, eve_warp_attr_id: 0})
    eve_ship3_id = client.mk_eve_ship(attrs={eve_tether_attr_id: 0, eve_warp_attr_id: 100})
    eve_ship4_id = client.mk_eve_ship(attrs={eve_tether_attr_id: 0})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship1_id)
...
def test_ship_warp_no_attr(client, consts):
    ...
    eve_ship_id = client.mk_eve_ship(attrs={eve_tether_attr_id: 0, eve_warp_attr_id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
...
def test_ship_absent(client, consts):
    client.mk_eve_attr(id_=consts.EveAttr.disallow_tethering)
    ...
    # Verification
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(can_tether=True))
    assert api_fit_stats.can_tether is None
...
def test_ship_not_loaded(client, consts):
    ...
    assert api_fit_stats.can_tether is None
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(can_tether=True))
    assert api_ship_stats.can_tether is None


def test_struct(client, consts):
    eve_tether_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_tethering)
    eve_warp_attr_id = client.mk_eve_attr(id_=consts.EveAttr.warp_scramble_status)
    eve_struct_id = client.mk_eve_struct(attrs={eve_tether_attr_id: 1, eve_warp_attr_id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
...
def test_incorrect_item_kind(client, consts):
    eve_tether_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_tethering)
    eve_warp_attr_id = client.mk_eve_attr(id_=consts.EveAttr.warp_scramble_status)
    eve_fighter_id = client.mk_eve_fighter(attrs={eve_tether_attr_id: 100, eve_warp_attr_id: 100})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_fighter = api_fit.add_fighter(type_id=eve_fighter_id)
    # Verification
    api_drone_stats = api_fighter.get_stats(options=ItemStatsOptions(can_tether=True))
    assert api_drone_stats.can_tether is None


def test_not_requested(client, consts):
    eve_tether_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_tethering)
    eve_warp_attr_id = client.mk_eve_attr(id_=consts.EveAttr.warp_scramble_status)
    eve_ship_id = client.mk_eve_ship(attrs={eve_tether_attr_id: 100, eve_warp_attr_id: 100})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
    # Verification
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(can_tether=False))
    with check_no_field():
        api_fit_stats.can_tether  # ruff:ignore[useless-expression]
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(can_tether=False))
    with check_no_field():
        api_ship_stats.can_tether  # ruff:ignore[useless-expression]
