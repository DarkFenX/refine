...
def test_chargedness_local(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(hp=True, rps=[
        ...
        StatsOptionRps(time=StatTimeSim(optional_reloads=consts.ApiOptionalReload.on_empty)),
        StatsOptionRps(time=StatTimeSim(optional_reloads=consts.ApiOptionalReload.disabled))]))
...
def test_chargedness_remote(client, consts):
    ...
    api_src_stats = api_src_fit.get_stats(options=FitStatsOptions(outgoing_rps=[
        StatsOptionFitOutRps(time=StatTimeBurst()),
        StatsOptionFitOutRps(time=StatTimeSim(optional_reloads=consts.ApiOptionalReload.on_empty)),
        StatsOptionFitOutRps(time=StatTimeSim(optional_reloads=consts.ApiOptionalReload.disabled))]))
