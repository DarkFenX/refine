...
def test_root_add_afor_afee_proj_remove_state_proj_fit(client, consts):
    ...
    assert api_affectee_struct.update().attrs[eve_affectee_attr_id].modified == approx(-2)
    api_affector_module.change_module(add_proj_item_ids=[api_affectee_struct.id])
    assert api_affectee_struct.update().attrs[eve_affectee_attr_id].modified == approx(1)
...
def test_root_add_afee_afor_proj_state_remove_afor_afee(client, consts):
    ...
    assert api_affectee_ship.update().attrs[eve_affectee_attr_id].modified == approx(-2)
    api_affector_fit = api_sol.create_fit()
    api_affector_module = api_affector_fit.add_module(
        type_id=eve_affector_module_id,
...
def test_child_add_afor_afee_proj_remove_state_proj_fit(client, consts):
    ...
    assert api_affectee_drone.update().attrs[eve_affectee_attr_id].modified == approx(-2)
    api_affector_module.change_module(add_proj_item_ids=[api_affectee_drone.id])
    assert api_affectee_drone.update().attrs[eve_affectee_attr_id].modified == approx(1)
    api_affector_module.change_module(state=consts.ApiModuleState.online)
    assert api_affectee_drone.update().attrs[eve_affectee_attr_id].modified == approx(-2)
    api_affector_module.change_module(rm_proj_item_ids=[api_affectee_drone.id])
    assert api_affectee_drone.update().attrs[eve_affectee_attr_id].modified == approx(-2)
    api_affectee_fit.remove()
...
def test_child_add_afee_afor_proj_state_remove_afor_afee(client, consts):
    ...
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(-2)
    api_affector_fit = api_sol.create_fit()
    api_affector_module = api_affector_fit.add_module(
        type_id=eve_affector_module_id,
        state=consts.ApiModuleState.online)
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(-2)
    api_affector_module.change_module(add_proj_item_ids=[api_affectee_fighter.id])
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(-2)
    api_affector_module.change_module(state=consts.ApiModuleState.active)
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(1)
    api_affector_module.remove()
    assert api_affectee_fighter.update().attrs[eve_affectee_attr_id].modified == approx(-2)
