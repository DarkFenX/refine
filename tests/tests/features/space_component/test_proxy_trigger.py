"""
Proximity triggers/traps were initially used by the Dr Who event, and in a few following events. As
of 2025-03-28, there seems to be no items which actively use it, so there seems to be no normal way
of testing it. However, thanks to FC Kestrel, we know that as of 2025-03-28 traps do not apply to
drones.

Proximity triggers have an item list attached, which, judging by the name, defines which entities
can trigger those traps. Possibly, this is the list which is used to define which items buff applies
to as well. The library uses this list to filter modifications, i.e. anything which can't trigger
the trap can't receive its buffs.
"""
...
def test_affectee_filter_item_list(client, consts):
    ...
    eve_beacon_id = client.mk_eve_item(pt_buffs=({eve_buff_id: 10}, eve_item_list_id))
...
def test_affectee_filter_unspecified(client, consts):
    ...
    eve_buff_id = client.mk_eve_buff(
        ...
        op=consts.EveBuffOp.post_percent,
        item_mods=[client.mk_eve_buff_mod(attr_id=eve_attr_id)])
    eve_beacon_id = client.mk_eve_item(pt_buffs={eve_buff_id: 10})
    eve_ship_id = client.mk_eve_ship(attrs={eve_attr_id: 200})
    eve_struct_id = client.mk_eve_struct(attrs={eve_attr_id: 200})
    eve_drone_id = client.mk_eve_drone(attrs={eve_attr_id: 200})
    client.create_sources()
