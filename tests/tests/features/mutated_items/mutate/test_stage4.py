...
def test_overwrite(client, consts):
    ...
    api_item = api_fit.add_module(type_id=eve_base_item_id, mutation=(eve_mutator1_id, {
        eve_first_attr_id: Muta.roll_to_api(val=0.7),
        eve_both_attr_id: Muta.roll_to_api(val=0.3)}))
    # Verification
    api_item.update()
    ...
    assert api_item.mutation.rolls[eve_first_attr_id] == approx(0.7)
    assert api_item.mutation.rolls[eve_both_attr_id] == approx(0.3)
    ...
    api_item.change_module(mutation=(eve_mutator2_id, {
        eve_second_attr_id: Muta.roll_to_api(val=0.6),
        eve_both_attr_id: Muta.roll_to_api(val=0.3)}))
    ...
    assert api_item.mutation.rolls[eve_first_attr_id] == approx(0.7)
    assert api_item.mutation.rolls[eve_second_attr_id] == approx(0.6)
    assert api_item.mutation.rolls[eve_both_attr_id] == approx(0.3)
...
def test_item_category(client, consts):
    ...
    api_item1 = api_fit.add_module(type_id=eve_base_item_id)
    api_item2 = api_fit.add_module(type_id=eve_base_item_id)
...
def test_projection(client, consts):
    ...
    api_affector_module = api_affector_fit.add_module(type_id=eve_base_item_id, state=consts.ApiModuleState.active)
    api_affector_drone = api_affector_fit.add_drone(type_id=eve_base_item_id, state=consts.ApiMinionState.engaging)
...
def test_drone(client, consts):
    ...
    assert len(api_item.mutation.rolls) == 2
    assert api_item.mutation.rolls[eve_roll_attr_id] == approx(0.3)
    assert api_item.mutation.rolls[eve_absolute_attr_id] == approx(0.625)
