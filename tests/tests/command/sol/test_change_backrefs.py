...
def test_fleet_change(client):
    ...
    with api_sol.batch() as api_sol_batch:
        ...
        api_fleet = api_sol_batch.create_fleet(fit_ids=[api_fit1.id])
...
def test_fleet_remove(client):
    ...
    with api_sol.batch() as api_sol_batch:
        ...
        api_sol_batch.remove_fleet(fleet_id=api_fleet.id)
    # Verification
...
def test_fit_change(client):
    ...
    with api_sol.batch() as api_sol_batch:
        ...
        api_fit = api_sol_batch.create_fit(fleet_id=api_fleet1.id)
        api_fleet2 = api_sol_batch.create_fleet()
        api_sol_batch.change_fit(fit_id=api_fit.id, fleet_id=api_fleet2.id)
    # Verification
...
def test_fit_remove(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_fleet = api_sol_batch.create_fleet(fit_ids=[api_fit.id])
        api_sol_batch.remove_fit(fit_id=api_fit.id)
    # Verification
...
def test_item_remove(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_module = api_sol_batch.add_module(fit_id=api_fit.id, type_id=eve_module_id, charge_type_id=eve_charge_id)
        api_sol_batch.remove_item(item_id=api_module.charge.id)
    # Verification
    api_module.update()
    with check_no_field():
...
def test_booster_add(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_booster = api_sol_batch.add_booster(fit_id=api_fit.id, type_id=eve_booster_id)
    # Verification
...
def test_booster_change(client):
    ...
    api_fit = api_sol.create_fit()
    with api_sol.batch() as api_sol_batch:
        api_booster = api_sol_batch.add_booster(fit_id=api_fit.id, type_id=eve_booster_id, state=True)
        api_sol_batch.change_booster(item_id=api_booster.id, state=False)
    # Verification
...
def test_character_set(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_character = api_sol_batch.set_character(fit_id=api_fit.id, type_id=eve_character_id)
    # Verification
...
def test_character_change_via_fit_id(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_character = api_sol_batch.set_character(fit_id=api_fit.id, type_id=eve_character_id, state=True)
        api_sol_batch.change_character_via_fit_id(fit_id=api_fit.id, state=False)
    # Verification
...
def test_character_change_via_item_id(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_character = api_sol_batch.set_character(fit_id=api_fit.id, type_id=eve_character_id, state=True)
        api_sol_batch.change_character_via_item_id(item_id=api_character.id, state=False)
    # Verification
    assert api_character.update().state is False
...
def test_character_unset(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_sol_batch.set_character(fit_id=api_fit.id, type_id=eve_character_id)
        api_sol_batch.unset_character(fit_id=api_fit.id)
    # Verification
    api_fit.update()
    with check_no_field():
...
def test_charge_change_after_module_add(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_module = api_sol_batch.add_module(fit_id=api_fit.id, type_id=eve_module_id, charge_type_id=eve_charge_id)
        api_sol_batch.change_charge(item_id=api_module.charge.id, state=False)
    # Verification
...
def test_charge_change_after_module_change(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_charge = api_sol_batch.change_module(item_id=api_module.id, charge_type_id=eve_charge_id)
        api_sol_batch.change_charge(item_id=api_charge.id, state=False)
    # Verification
...
def test_drone_add(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_tgt_ship = api_sol_batch.set_ship(fit_id=api_tgt_fit.id, type_id=eve_ship_id)
        api_src_fit = api_sol_batch.create_fit()
        api_src_drone = api_sol_batch.add_drone(
...
def test_drone_change(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_tgt_ship = api_sol_batch.set_ship(fit_id=api_tgt_fit.id, type_id=eve_ship_id)
        api_src_drone = api_sol_batch.add_drone(fit_id=api_src_fit.id, type_id=eve_drone_id)
        api_sol_batch.change_drone(item_id=api_src_drone.id, add_proj_item_ids=[api_tgt_ship.id])
    # Verification
    assert api_tgt_ship.id in api_src_drone.update().projs
...
def test_fighter_add(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_tgt_ship = api_sol_batch.set_ship(fit_id=api_tgt_fit.id, type_id=eve_ship_id)
        api_src_fit = api_sol_batch.create_fit()
        api_src_fighter = api_sol_batch.add_fighter(
            fit_id=api_src_fit.id,
...
def test_fighter_change(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_tgt_ship = api_sol_batch.set_ship(fit_id=api_tgt_fit.id, type_id=eve_ship_id)
        api_src_fighter = api_sol_batch.add_fighter(fit_id=api_src_fit.id, type_id=eve_fighter_id)
        api_sol_batch.change_fighter(item_id=api_src_fighter.id, add_proj_item_ids=[api_tgt_ship.id])
    # Verification
    assert api_tgt_ship.id in api_src_fighter.update().projs
...
def test_fw_effect_add(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_fw_effect = api_sol_batch.add_fw_effect(fit_id=api_fit.id, type_id=eve_fw_effect_id)
    # Verification
...
def test_fw_effect_change(client):
    ...
    api_fit = api_sol.create_fit()
    with api_sol.batch() as api_sol_batch:
        api_fw_effect = api_sol_batch.add_fw_effect(fit_id=api_fit.id, type_id=eve_fw_effect_id, state=True)
        api_sol_batch.change_fw_effect(item_id=api_fw_effect.id, state=False)
    # Verification
...
def test_implant_add(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_implant = api_sol_batch.add_implant(fit_id=api_fit.id, type_id=eve_implant_id)
    # Verification
...
def test_implant_change(client):
    ...
    api_fit = api_sol.create_fit()
    with api_sol.batch() as api_sol_batch:
        api_implant = api_sol_batch.add_implant(fit_id=api_fit.id, type_id=eve_implant_id, state=True)
        api_sol_batch.change_implant(item_id=api_implant.id, state=False)
    # Verification
...
def test_module_change(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_tgt_ship = api_sol_batch.set_ship(fit_id=api_tgt_fit.id, type_id=eve_ship_id)
        api_src_module = api_sol_batch.add_module(fit_id=api_src_fit.id, type_id=eve_module_id)
        api_sol_batch.change_module(item_id=api_src_module.id, add_proj_item_ids=[api_tgt_ship.id])
    # Verification
    assert api_tgt_ship.id in api_src_module.update().projs
...
def test_proj_effect_add(client):
    ...
    api_fit = api_sol.create_fit()
    with api_sol.batch() as api_sol_batch:
        api_tgt_ship = api_sol_batch.set_ship(fit_id=api_fit.id, type_id=eve_ship_id)
        api_src_proj_effect = api_sol_batch.add_proj_effect(type_id=eve_proj_effect_id, proj_item_ids=[api_tgt_ship.id])
    # Verification
...
def test_proj_effect_change(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_tgt_ship = api_sol_batch.set_ship(fit_id=api_fit.id, type_id=eve_ship_id)
        api_src_proj_effect = api_sol_batch.add_proj_effect(type_id=eve_proj_effect_id)
        api_sol_batch.change_proj_effect(item_id=api_src_proj_effect.id, add_proj_item_ids=[api_tgt_ship.id])
    # Verification
    assert api_tgt_ship.id in api_src_proj_effect.update().proj_item_ids
...
def test_rig_add(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_rig = api_sol_batch.add_rig(fit_id=api_fit.id, type_id=eve_rig_id)
    # Verification
...
def test_rig_change(client):
    ...
    api_fit = api_sol.create_fit()
    with api_sol.batch() as api_sol_batch:
        api_rig = api_sol_batch.add_rig(fit_id=api_fit.id, type_id=eve_rig_id, state=True)
        api_sol_batch.change_rig(item_id=api_rig.id, state=False)
    # Verification
...
def test_service_add(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_service = api_sol_batch.add_service(fit_id=api_fit.id, type_id=eve_service_id)
    # Verification
...
def test_service_change(client, consts):
    ...
    api_fit = api_sol.create_fit()
    with api_sol.batch() as api_sol_batch:
        api_service = api_sol_batch.add_service(
            fit_id=api_fit.id,
            type_id=eve_service_id,
            state=consts.ApiServiceState.online)
        api_sol_batch.change_service(item_id=api_service.id, state=consts.ApiServiceState.disabled)
    # Verification
...
def test_ship_change_via_fit_id(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_ship = api_sol_batch.set_ship(fit_id=api_fit.id, type_id=eve_ship_id, state=True)
        api_sol_batch.change_ship_via_fit_id(fit_id=api_fit.id, state=False)
    # Verification
...
def test_ship_change_via_item_id(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_ship = api_sol_batch.set_ship(fit_id=api_fit.id, type_id=eve_ship_id, state=True)
        api_sol_batch.change_ship_via_item_id(item_id=api_ship.id, state=False)
    # Verification
    assert api_ship.update().state is False
...
def test_ship_unset(client):
    ...
    with api_sol.batch() as api_sol_batch:
        ...
        api_sol_batch.unset_ship(fit_id=api_fit.id)
    # Verification
    api_fit.update()
    with check_no_field():
...
def test_skill_add(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_skill = api_sol_batch.add_skill(fit_id=api_fit.id, type_id=eve_skill_id, level=3)
    # Verification
    api_skill.update()
...
def test_skill_change(client):
    ...
    api_fit = api_sol.create_fit()
    with api_sol.batch() as api_sol_batch:
        api_skill = api_sol_batch.add_skill(fit_id=api_fit.id, type_id=eve_skill_id, level=1)
        api_sol_batch.change_skill(item_id=api_skill.id, level=5)
    # Verification
...
def test_stance_set(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_stance = api_sol_batch.set_stance(fit_id=api_fit.id, type_id=eve_stance_id)
    # Verification
...
def test_stance_change_via_fit_id(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_stance = api_sol_batch.set_stance(fit_id=api_fit.id, type_id=eve_stance_id, state=True)
        api_sol_batch.change_stance_via_fit_id(fit_id=api_fit.id, state=False)
    # Verification
...
def test_stance_change_via_item_id(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_stance = api_sol_batch.set_stance(fit_id=api_fit.id, type_id=eve_stance_id, state=True)
        api_sol_batch.change_stance_via_item_id(item_id=api_stance.id, state=False)
    # Verification
    assert api_stance.update().state is False
...
def test_stance_unset(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_sol_batch.set_stance(fit_id=api_fit.id, type_id=eve_stance_id)
        api_sol_batch.unset_stance(fit_id=api_fit.id)
    # Verification
    api_fit.update()
    with check_no_field():
...
def test_subsystem_add(client):
    ...
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_subsystem = api_sol_batch.add_subsystem(fit_id=api_fit.id, type_id=eve_subsystem_id)
    # Verification
...
def test_subsystem_change(client):
    ...
    api_fit = api_sol.create_fit()
    with api_sol.batch() as api_sol_batch:
        api_subsystem = api_sol_batch.add_subsystem(fit_id=api_fit.id, type_id=eve_subsystem_id, state=True)
        api_sol_batch.change_subsystem(item_id=api_subsystem.id, state=False)
    # Verification
...
def test_sw_effect_change(client):
    ...
    api_sol = client.create_sol()
    with api_sol.batch() as api_sol_batch:
        api_sw_effect = api_sol_batch.add_sw_effect(type_id=eve_sw_effect_id, state=True)
        api_sol_batch.change_sw_effect(item_id=api_sw_effect.id, state=False)
    # Verification
