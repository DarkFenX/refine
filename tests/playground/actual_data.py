"""
Just collection of methods to test with real data from time to time.
"""

import json
import typing
from pathlib import Path
from time import time

from fw import approx
from fw.api import (
    FitStatsOptions,
    StatCapSrcKinds,
    StatsOptionCapBalance,
    StatsOptionCapSim,
    StatsOptionFitDmg,
    StatsOptionJump,
    ValOptions,
)

if typing.TYPE_CHECKING:
    from fw.api.types.item import Item

SCRIPT_DIR_PATH = Path(__file__).resolve().absolute().parent
PHOBOS_BASE_PATH = Path('~', 'Desktop', 'phobos_tq_en-us').expanduser()


# ruff:ignore[missing-type-function-argument, missing-return-type-undocumented-public-function]
def test_benchmark_attr_calc(client, consts):
    setup_eve_data(client=client, data=client._get_default_eve_data())  # ruff:ignore[private-member-access]
    api_sol = client.create_sol(sec_zone=consts.ApiSecZone.hisec)
    api_fit = api_sol.create_fit()
    with api_fit.batch() as api_fit_batch:
        api_fit_batch.set_character(type_id=1373)
        for eve_skill_id in get_skill_type_ids():
            api_fit_batch.add_skill(type_id=eve_skill_id, level=5)
        api_fit_batch.set_ship(type_id=11184)  # Crusader
        # ruff:ignore[commented-out-code]
        # api_fit_cmds.add_module(type_id=4403, rack=consts.ApiRack.low, state=consts.ApiModuleState.active)
    iterations = 1000000
    print('starting attr-calc benchmark')  # ruff:ignore[print]
    before = time()
    api_sol.benchmark(command={'type': 'attr_calc', 'fit_id': api_fit.id, 'type_id': 1306, 'iterations': iterations})
    after = time()
    print('done with attr-calc benchmark')  # ruff:ignore[print]
    delta = after - before
    ips = iterations / delta
    print(f'{iterations} iterations done in {delta:.3f} seconds, {ips:.2f} iterations per second')  # ruff:ignore[print]


# ruff:ignore[missing-type-function-argument, missing-return-type-undocumented-public-function]
def test_benchmark_stats(client, consts):
    setup_eve_data(client=client, data=client._get_default_eve_data())  # ruff:ignore[private-member-access]

    api_sol = client.create_sol(sec_zone=consts.ApiSecZone.hisec)
    api_src_fit = api_sol.create_fit()
    with api_src_fit.batch() as api_src_fit_batch:
        api_src_fit_batch.set_character(type_id=1373)
        for eve_skill_id in get_skill_type_ids():
            api_src_fit_batch.add_skill(type_id=eve_skill_id, level=5)
        api_src_fit_batch.add_implant(type_id=13231)  # TD-603
        api_src_fit_batch.add_implant(type_id=10228)  # SM-703
        api_src_fit_batch.add_implant(type_id=24663)  # Zor hyperlink
        api_src_fit_batch.add_implant(type_id=13244)  # SS-903
        api_src_fit_batch.add_implant(type_id=13219)  # LP-1003
        api_src_fit_batch.add_booster(type_id=28674)  # Synth drop
        api_src_fit_batch.add_booster(type_id=28672)  # Synth crash
        api_src_fit_batch.add_booster(type_id=45999)  # Pyro 2
        api_src_fit_batch.set_ship(type_id=32311, coordinates=(0, 0, 0), movement=(0, 0, 1))  # NTyphoon
        # T2 800mms with hail
        for _ in range(4):
            api_src_fit_batch.add_module(
                type_id=2929,
                rack=consts.ApiRack.high,
                state=consts.ApiModuleState.overload,
                charge_type_id=12779)
        # T2 torpedo launchers with EM rages
        for _ in range(4):
            api_src_fit_batch.add_module(
                type_id=2420,
                rack=consts.ApiRack.high,
                state=consts.ApiModuleState.overload,
                charge_type_id=24523)
        # Enduring 500MN
        api_src_fit_batch.add_module(type_id=5945, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)
        # T2 med cap booster with navy 800
        api_src_fit_batch.add_module(
            type_id=2024,
            rack=consts.ApiRack.mid,
            state=consts.ApiModuleState.active,
            charge_type_id=32014)
        # T2 EM hardener
        api_src_fit_batch.add_module(type_id=2301, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)
        # T2 scram
        api_src_fit_batch.add_module(type_id=448, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)
        # T2 multispec
        api_src_fit_batch.add_module(type_id=2281, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)
        # T2 DC
        api_src_fit_batch.add_module(type_id=2048, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 gyrostabs
        for _ in range(2):
            api_src_fit_batch.add_module(type_id=519, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 BCS'
        for _ in range(2):
            api_src_fit_batch.add_module(type_id=22291, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 DDAs
        for _ in range(2):
            api_src_fit_batch.add_module(type_id=4405, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 therm rig
        api_src_fit_batch.add_rig(type_id=26436)
        # T1 CDFEs
        for _ in range(2):
            api_src_fit_batch.add_rig(type_id=26088)
        # T2 ogres
        for _ in range(5):
            api_src_fit_batch.add_drone(type_id=2446, state=consts.ApiMinionState.engaging)
        # T2 ogres
        for _ in range(3):
            api_src_fit_batch.add_drone(type_id=2446, state=consts.ApiMinionState.in_bay)

    api_tgt_fit = api_sol.create_fit()
    with api_tgt_fit.batch() as api_tgt_fit_batch:
        api_tgt_fit_batch.set_character(type_id=1373)
        for eve_skill_id in get_skill_type_ids():
            api_tgt_fit_batch.add_skill(type_id=eve_skill_id, level=5)
        # Nightmare
        api_tgt_ship = api_tgt_fit_batch.set_ship(type_id=17736, coordinates=(0, 20000, 0), movement=(180, 0, 1))
        # T2 AB
        api_tgt_fit_batch.add_module(type_id=12068, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)

    iterations = 1000000
    print('starting stats benchmark')  # ruff:ignore[print]
    before = time()
    api_sol.benchmark(command={
        'type': 'stats', 'fit_id': api_src_fit.id,
        'projectee_item_id': api_tgt_ship.id, 'iterations': iterations})
    after = time()
    print('done with stats benchmark')  # ruff:ignore[print]
    delta = after - before
    ips = iterations / delta
    print(f'{iterations} iterations done in {delta:.3f} seconds, {ips:.2f} iterations per second')  # ruff:ignore[print]


# ruff:ignore[missing-type-function-argument, missing-return-type-undocumented-public-function]
def test_benchmark_try_fit_items(client, consts):
    setup_eve_data(client=client, data=client._get_default_eve_data())  # ruff:ignore[private-member-access]
    api_sol = client.create_sol(sec_zone=consts.ApiSecZone.hisec)
    api_fit = api_sol.create_fit()
    with api_fit.batch() as api_fit_batch:
        api_fit_batch.set_character(type_id=1373)
        for eve_skill_id in get_skill_type_ids():
            api_fit_batch.add_skill(type_id=eve_skill_id, level=5)
        api_fit_batch.add_implant(type_id=13231)  # TD-603
        api_fit_batch.add_implant(type_id=10228)  # SM-703
        api_fit_batch.add_implant(type_id=24663)  # Zor hyperlink
        api_fit_batch.add_implant(type_id=13244)  # SS-903
        api_fit_batch.add_implant(type_id=13219)  # LP-1003
        api_fit_batch.add_booster(type_id=28674)  # Synth drop
        api_fit_batch.add_booster(type_id=28672)  # Synth crash
        api_fit_batch.add_booster(type_id=45999)  # Pyro 2
        api_fit_batch.set_ship(type_id=32311)  # NTyphoon
        # T2 800mms with hail
        for _ in range(3):
            api_fit_batch.add_module(
                type_id=2929,
                rack=consts.ApiRack.high,
                state=consts.ApiModuleState.overload,
                charge_type_id=12779)
        # T2 torpedo launchers with thermal rages
        for _ in range(3):
            api_fit_batch.add_module(
                type_id=2420,
                rack=consts.ApiRack.high,
                state=consts.ApiModuleState.overload,
                charge_type_id=2811)
        # Enduring 500MN
        api_fit_batch.add_module(type_id=5945, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)
        # T2 med cap booster with navy 800
        api_fit_batch.add_module(
            type_id=2024,
            rack=consts.ApiRack.mid,
            state=consts.ApiModuleState.active,
            charge_type_id=32014)
        # T2 EM hardener
        api_fit_batch.add_module(type_id=2301, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)
        # T2 scram
        api_fit_batch.add_module(type_id=448, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)
        # T2 DC
        api_fit_batch.add_module(type_id=2048, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 gyrostabs
        for _ in range(2):
            api_fit_batch.add_module(type_id=519, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 BCS'
        for _ in range(2):
            api_fit_batch.add_module(type_id=22291, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 DDA
        api_fit_batch.add_module(type_id=4405, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 therm rig
        api_fit_batch.add_rig(type_id=26436)
        # T1 CDFEs
        for _ in range(2):
            api_fit_batch.add_rig(type_id=26088)
        # T2 ogres
        for _ in range(5):
            api_fit_batch.add_drone(type_id=2446, state=consts.ApiMinionState.engaging)
        # T2 ogres
        for _ in range(2):
            api_fit_batch.add_drone(type_id=2446, state=consts.ApiMinionState.in_bay)
    api_val = api_fit.validate(options=ValOptions(default=True))
    assert api_val.passed is True
    iterations = 1000
    try_fit_type_ids = get_try_fit_type_ids()
    options = ValOptions(default=True).to_dict()
    print(f'starting try-fit-items benchmark, trying {len(try_fit_type_ids)} items per iteration')  # ruff:ignore[print]
    before = time()
    api_sol.benchmark(command={
        'type': 'try_fit_items',
        'fit_id': api_fit.id,
        'type_ids': try_fit_type_ids,
        'val_options': options,
        'iterations': iterations})
    after = time()
    print('done with try-fit-items benchmark')  # ruff:ignore[print]
    delta = after - before
    iters_ps = iterations / delta
    items_ps = iters_ps * len(try_fit_type_ids)
    print(  # ruff:ignore[print]
        f'{iterations} iterations done in {delta:.3f} seconds, {iters_ps:.2f} iterations per second,'
        f' {items_ps:.2f} items per second')


# ruff:ignore[missing-type-function-argument, missing-return-type-undocumented-public-function]
def test_try_fit_items_nphoon(client, consts):
    setup_eve_data(client=client, data=client._get_default_eve_data())  # ruff:ignore[private-member-access]
    api_sol = client.create_sol(sec_zone=consts.ApiSecZone.hisec)
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_sol_batch.set_character(fit_id=api_fit.id, type_id=1373)
        for eve_skill_id in get_skill_type_ids():
            api_sol_batch.add_skill(fit_id=api_fit.id, type_id=eve_skill_id, level=5)
        api_sol_batch.add_implant(fit_id=api_fit.id, type_id=13231)  # TD-603
        api_sol_batch.add_implant(fit_id=api_fit.id, type_id=10228)  # SM-703
        api_sol_batch.add_implant(fit_id=api_fit.id, type_id=24663)  # Zor hyperlink
        api_sol_batch.add_implant(fit_id=api_fit.id, type_id=13244)  # SS-903
        api_sol_batch.add_implant(fit_id=api_fit.id, type_id=13219)  # LP-1003
        api_sol_batch.add_booster(fit_id=api_fit.id, type_id=28674)  # Synth drop
        api_sol_batch.add_booster(fit_id=api_fit.id, type_id=28672)  # Synth crash
        api_sol_batch.add_booster(fit_id=api_fit.id, type_id=45999)  # Pyro 2
        api_sol_batch.set_ship(fit_id=api_fit.id, type_id=32311)  # NTyphoon
        # T2 800mms with hail
        for _ in range(3):
            api_sol_batch.add_module(
                fit_id=api_fit.id,
                type_id=2929,
                rack=consts.ApiRack.high,
                state=consts.ApiModuleState.overload,
                charge_type_id=12779)
        # T2 torpedo launchers with thermal rages
        for _ in range(3):
            api_sol_batch.add_module(
                fit_id=api_fit.id,
                type_id=2420,
                rack=consts.ApiRack.high,
                state=consts.ApiModuleState.overload,
                charge_type_id=2811)
        # 500MN
        api_sol_batch.add_module(
            fit_id=api_fit.id,
            type_id=5945,
            rack=consts.ApiRack.mid,
            state=consts.ApiModuleState.active)
        # T2 med cap booster with navy 800
        api_sol_batch.add_module(
            fit_id=api_fit.id,
            type_id=2024,
            rack=consts.ApiRack.mid,
            state=consts.ApiModuleState.active,
            charge_type_id=32014)
        # EM hard
        api_sol_batch.add_module(
            fit_id=api_fit.id,
            type_id=2301,
            rack=consts.ApiRack.mid,
            state=consts.ApiModuleState.active)
        # Scram
        api_sol_batch.add_module(
            fit_id=api_fit.id,
            type_id=448,
            rack=consts.ApiRack.mid,
            state=consts.ApiModuleState.active)
        # DC
        api_sol_batch.add_module(
            fit_id=api_fit.id,
            type_id=2048,
            rack=consts.ApiRack.low,
            state=consts.ApiModuleState.online)
        for _ in range(2):
            # Gyro
            api_sol_batch.add_module(
                fit_id=api_fit.id,
                type_id=519,
                rack=consts.ApiRack.low,
                state=consts.ApiModuleState.online)
        for _ in range(2):
            # BCS
            api_sol_batch.add_module(
                fit_id=api_fit.id,
                type_id=22291,
                rack=consts.ApiRack.low,
                state=consts.ApiModuleState.online)
        # T2 DDA
        api_sol_batch.add_module(
            fit_id=api_fit.id,
            type_id=4405,
            rack=consts.ApiRack.low,
            state=consts.ApiModuleState.online)
        # T2 therm rig
        api_sol_batch.add_rig(fit_id=api_fit.id, type_id=26436)
        # T1 CDFEs
        for _ in range(2):
            api_sol_batch.add_rig(fit_id=api_fit.id, type_id=26088)
        # T2 ogres
        for _ in range(5):
            api_sol_batch.add_drone(fit_id=api_fit.id, type_id=2446, state=consts.ApiMinionState.engaging)
        # T2 ogres
        for _ in range(2):
            api_sol_batch.add_drone(fit_id=api_fit.id, type_id=2446, state=consts.ApiMinionState.in_bay)
    api_val = api_fit.validate(options=ValOptions(default=True))
    assert api_val.passed is True
    try_fit_type_ids = get_try_fit_type_ids()
    type_ids = api_fit.try_fit_items(type_ids=try_fit_type_ids, val_options=ValOptions(default=True))
    print('---')  # ruff:ignore[print]
    print(f'Sent {len(try_fit_type_ids)} items, received {len(type_ids)} eligible items')  # ruff:ignore[print]
    print_items(type_ids=type_ids, print_types=False)


# ruff:ignore[missing-type-function-argument, missing-return-type-undocumented-public-function]
def test_stacking(client, consts):
    setup_eve_data(client=client, data=client._get_default_eve_data())  # ruff:ignore[private-member-access]
    api_sol = client.create_sol(sec_zone=consts.ApiSecZone.hisec)
    api_fit_m = api_sol.create_fit()
    with api_fit_m.batch() as api_fit_m_batch:
        api_fit_m_batch.set_character(type_id=1373)
        for eve_skill_id in get_skill_type_ids():
            api_fit_m_batch.add_skill(type_id=eve_skill_id, level=5)
        api_male = api_fit_m_batch.set_ship(type_id=11186)  # Malediction
    for _ in range(2):
        api_fit_s = api_sol.create_fit()
        with api_fit_s.batch() as api_fit_s_batch:
            api_fit_s_batch.set_character(type_id=1373)
            for eve_skill_id in get_skill_type_ids():
                api_fit_s_batch.add_skill(type_id=eve_skill_id, level=5)
            api_fit_s_batch.set_ship(type_id=640)  # Scorpion
            for _ in range(8):
                api_fit_s_batch.add_module(
                    type_id=1964,
                    rack=consts.ApiRack.mid,
                    state=consts.ApiModuleState.active,
                    charge_type_id=29011,
                    proj_item_ids=[api_male.id])
    # Actual value captured ingame
    assert api_male.update().attrs[564].modified == approx(5264.18055777, accuracy=9)


# ruff:ignore[missing-type-function-argument, missing-return-type-undocumented-public-function]
def test_item_attrs(client):
    setup_eve_data(client=client, data=client._get_default_eve_data())  # ruff:ignore[private-member-access]
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    with api_fit.batch() as api_fit_batch:
        api_fit_batch.set_character(type_id=1373)
        for eve_skill_id in get_skill_type_ids():
            api_fit_batch.add_skill(type_id=eve_skill_id, level=5)
        api_ship = api_fit_batch.set_ship(type_id=89607)  # Odysseus
    api_attrs_before = api_ship.update().update().attrs
    api_fit.add_fw_effect(type_id=30864)  # C6 magnetar
    api_attrs_after = api_ship.update().attrs
    print_attr_diff(attrs1=api_attrs_before, attrs2=api_attrs_after)


# ruff:ignore[missing-type-function-argument, missing-return-type-undocumented-public-function]
def test_stats(client, consts):
    setup_eve_data(client=client, data=client._get_default_eve_data())  # ruff:ignore[private-member-access]
    api_sol = client.create_sol(sec_zone=consts.ApiSecZone.hisec)
    api_src_fit = api_sol.create_fit()
    with api_src_fit.batch() as api_src_fit_batch:
        api_src_fit_batch.set_character(type_id=1373)
        for eve_skill_id in get_skill_type_ids():
            api_src_fit_batch.add_skill(type_id=eve_skill_id, level=5)
        api_src_fit_batch.add_implant(type_id=13231)  # TD-603
        api_src_fit_batch.add_implant(type_id=10228)  # SM-703
        api_src_fit_batch.add_implant(type_id=24663)  # Zor hyperlink
        api_src_fit_batch.add_implant(type_id=13244)  # SS-903
        api_src_fit_batch.add_implant(type_id=13219)  # LP-1003
        api_src_fit_batch.add_booster(type_id=28674)  # Synth drop
        api_src_fit_batch.add_booster(type_id=28672)  # Synth crash
        api_src_fit_batch.add_booster(type_id=45999)  # Pyro 2
        api_src_fit_batch.set_ship(type_id=32311, coordinates=(0, 0, 0), movement=(0, 0, 1))  # NTyphoon
        # T2 800mms with hail
        for _ in range(4):
            api_src_fit_batch.add_module(
                type_id=2929,
                rack=consts.ApiRack.high,
                state=consts.ApiModuleState.overload,
                charge_type_id=12779)
        # T2 torpedo launchers with EM rages
        for _ in range(4):
            api_src_fit_batch.add_module(
                type_id=2420,
                rack=consts.ApiRack.high,
                state=consts.ApiModuleState.overload,
                charge_type_id=24523)
        # Enduring 500MN
        api_src_fit_batch.add_module(type_id=5945, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)
        # T2 med cap booster with navy 800
        api_src_fit_batch.add_module(
            type_id=2024,
            rack=consts.ApiRack.mid,
            state=consts.ApiModuleState.active,
            charge_type_id=32014)
        # T2 EM hardener
        api_src_fit_batch.add_module(type_id=2301, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)
        # T2 scram
        api_src_fit_batch.add_module(type_id=448, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)
        # T2 multispec
        api_src_fit_batch.add_module(type_id=2281, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)
        # T2 DC
        api_src_fit_batch.add_module(type_id=2048, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 gyrostabs
        for _ in range(2):
            api_src_fit_batch.add_module(type_id=519, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 BCS'
        for _ in range(2):
            api_src_fit_batch.add_module(type_id=22291, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 DDAs
        for _ in range(2):
            api_src_fit_batch.add_module(type_id=4405, rack=consts.ApiRack.low, state=consts.ApiModuleState.online)
        # T2 therm rig
        api_src_fit_batch.add_rig(type_id=26436)
        # T1 CDFEs
        for _ in range(2):
            api_src_fit_batch.add_rig(type_id=26088)
        # T2 ogres
        for _ in range(5):
            api_src_fit_batch.add_drone(type_id=2446, state=consts.ApiMinionState.engaging)
        # T2 ogres
        for _ in range(3):
            api_src_fit_batch.add_drone(type_id=2446, state=consts.ApiMinionState.in_bay)

    api_tgt_fit = api_sol.create_fit()
    with api_tgt_fit.batch() as api_tgt_fit_batch:
        api_tgt_fit_batch.set_character(type_id=1373)
        for eve_skill_id in get_skill_type_ids():
            api_tgt_fit_batch.add_skill(type_id=eve_skill_id, level=5)
        # Nightmare
        api_tgt_ship = api_tgt_fit_batch.set_ship(type_id=17736, coordinates=(0, 20000, 0), movement=(180, 0, 1))
        # T2 AB
        api_tgt_fit_batch.add_module(type_id=12068, rack=consts.ApiRack.mid, state=consts.ApiModuleState.active)

    api_src_fit_stats = api_src_fit.get_stats(options=FitStatsOptions(
        dmg=[StatsOptionFitDmg(projectee_item_id=api_tgt_ship.id)],
        cap_balance=[
            StatsOptionCapBalance(),
            StatsOptionCapBalance(src_kinds=StatCapSrcKinds(default=False, regen=True, cap_injectors=True)),
            StatsOptionCapBalance(src_kinds=StatCapSrcKinds(default=False, consumers=True))],
        cap_sim=[StatsOptionCapSim(cap_perc=1)]))
    print(api_src_fit_stats.dmg.one())  # ruff:ignore[print]
    print(api_src_fit_stats.cap_balance)  # ruff:ignore[print]
    print(api_src_fit_stats.cap_sim.one())  # ruff:ignore[print]


# ruff:ignore[missing-type-function-argument, missing-return-type-undocumented-public-function]
def test_playground(client, consts):
    setup_eve_data(client=client, data=client._get_default_eve_data())  # ruff:ignore[private-member-access]
    api_sol = client.create_sol(sec_zone=consts.ApiSecZone.hisec)
    with api_sol.batch() as api_sol_batch:
        api_fit = api_sol_batch.create_fit()
        api_sol_batch.set_character(fit_id=api_fit.id, type_id=1373)
        for eve_skill_id in get_skill_type_ids():
            api_sol_batch.add_skill(fit_id=api_fit.id, type_id=eve_skill_id, level=5)
        # Widow
        api_sol_batch.set_ship(fit_id=api_fit.id, type_id=22436)
        # Blops portal
        api_sol_batch.add_module(fit_id=api_fit.id, type_id=28652, state=consts.ApiModuleState.active)
        # Purifier passenger
        api_fit_psg = api_sol_batch.create_fit()
        api_sol_batch.set_character(fit_id=api_fit_psg.id, type_id=1373)
        for eve_skill_id in get_skill_type_ids():
            api_sol_batch.add_skill(fit_id=api_fit_psg.id, type_id=eve_skill_id, level=5)
        api_sol_batch.set_ship(fit_id=api_fit_psg.id, type_id=12038)
    # Verification
    assert api_fit.validate(options=ValOptions(cloaking_blocked=True)).passed is True
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(
        jump=[StatsOptionJump(range=5, passenger_fit_ids=[api_fit_psg.id])]))
    print(api_fit_stats.jump)  # ruff:ignore[print]


# ruff:ignore[missing-type-function-argument]
def setup_eve_data(*, client, data) -> None:
    files = [
        'fsd_built/types.json',
        'fsd_built/groups.json',
        'fsd_built/typelist.json',
        'fsd_built/dogmaattributes.json',
        'fsd_built/typedogma.json',
        'fsd_built/dogmaeffects.json',
        'fsd_lite/fighterabilities.json',
        'fsd_lite/fighterabilitiesbytype.json',
        'fsd_lite/dbuffcollections.json',
        'fsd_built/spacecomponentsbytype.json',
        'fsd_built/requiredskillsfortypes.json',
        'fsd_built/dynamicitemattributes.json']
    for file in files:
        with (PHOBOS_BASE_PATH / file).open() as f:
            # ruff:ignore[private-member-access]
            client._EveDataServer__setup_handler(url=f'/{data.alias}/{file}', data=f.read())
    client.create_source(data=data, cleanup_check=False)


def get_skill_type_ids() -> list[int]:
    with (SCRIPT_DIR_PATH / 'skill_type_ids.txt').open() as f:
        return [int(line) for line in f.readlines() if line]


def get_try_fit_type_ids() -> list[int]:
    with (SCRIPT_DIR_PATH / 'try_fit_type_ids.txt').open() as f:
        return [int(line) for line in f.readlines() if line]


def print_items(*, type_ids: list[int], print_types: bool = False) -> None:
    with (PHOBOS_BASE_PATH / 'fsd_built' / 'types.json').open() as f:
        item_id_item_name_map = {}
        item_id_group_id_map = {}
        for entry in json.load(f).values():
            item_id_item_name_map[entry['typeID']] = entry['typeName']
            item_id_group_id_map[entry['typeID']] = entry['groupID']
    with (PHOBOS_BASE_PATH / 'fsd_built' / 'groups.json').open() as f:
        group_id_group_name_map = {}
        group_id_category_id_map = {}
        for entry in json.load(f).values():
            group_id_group_name_map[entry['groupID']] = entry['groupName']
            group_id_category_id_map[entry['groupID']] = entry['categoryID']
    with (PHOBOS_BASE_PATH / 'fsd_built' / 'categories.json').open() as f:
        category_id_category_name_map = {}
        for entry in json.load(f).values():
            category_id_category_name_map[entry['categoryID']] = entry['categoryName']
    item_id_group_name_map = {
        item_id: group_id_group_name_map[group_id]
        for item_id, group_id in item_id_group_id_map.items()}
    item_id_category_name_map = {
        item_id: category_id_category_name_map[group_id_category_id_map[group_id]]
        for item_id, group_id in item_id_group_id_map.items()}

    def sorter(item_id: int) -> tuple[str, str, str]:
        return item_id_category_name_map[item_id], item_id_group_name_map[item_id], item_id_item_name_map[item_id]

    if print_types:
        for item_id in sorted(type_ids, key=sorter):
            line = (f'  {item_id_category_name_map[item_id]}'
                    f' / {item_id_group_name_map[item_id]}'
                    f' / {item_id_item_name_map[item_id]}')
            print(line)  # ruff:ignore[print]
    else:
        seen_lines = set()
        for item_id in sorted(type_ids, key=sorter):
            line = f'  {item_id_category_name_map[item_id]} / {item_id_group_name_map[item_id]}'
            if line in seen_lines:
                continue
            seen_lines.add(line)
            print(line)  # ruff:ignore[print]


def print_attrs(*, api_item: Item) -> None:
    item_id_item_name_map = {}
    with (PHOBOS_BASE_PATH / 'fsd_built' / 'types.json').open() as f:
        for entry in json.load(f).values():
            item_id_item_name_map[entry['typeID']] = entry['typeName']
    attr_id_attr_name_map = {}
    with (PHOBOS_BASE_PATH / 'fsd_built' / 'dogmaattributes.json').open() as f:
        for entry in json.load(f).values():
            attr_id_attr_name_map[entry['attributeID']] = entry['name']
    print('---')  # ruff:ignore[print]
    print(f'{item_id_item_name_map[api_item.type_id]}:')  # ruff:ignore[print]
    for attr_id in sorted(api_item.attrs, key=lambda i: attr_id_attr_name_map[i]):
        attr_name = attr_id_attr_name_map[attr_id]
        attr_val = api_item.attrs[attr_id].modified
        print(f'  {attr_name}: {attr_val}')  # ruff:ignore[print]


def print_attr_diff(*, attrs1: dict, attrs2: dict) -> None:
    item_id_item_name_map = {}
    with (PHOBOS_BASE_PATH / 'fsd_built' / 'types.json').open() as f:
        for entry in json.load(f).values():
            item_id_item_name_map[entry['typeID']] = entry['typeName']
    attr_id_attr_name_map = {}
    with (PHOBOS_BASE_PATH / 'fsd_built' / 'dogmaattributes.json').open() as f:
        for entry in json.load(f).values():
            attr_id_attr_name_map[entry['attributeID']] = entry['name']
    print('---')  # ruff:ignore[print]
    attr_ids = set(attrs1.keys()) | set(attrs2.keys())
    for attr_id in sorted(attr_ids, key=lambda i: attr_id_attr_name_map[i]):
        try:
            attr1_val = attrs1[attr_id].modified
        except KeyError:
            attr1_val = None
        try:
            attr2_val = attrs2[attr_id].modified
        except KeyError:
            attr2_val = None
        if attr1_val != attr2_val:
            attr_name = attr_id_attr_name_map[attr_id]
            print(f'  {attr_name}: {attr1_val} -> {attr2_val}')  # ruff:ignore[print]
