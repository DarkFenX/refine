...
impl FsdMerge<rc::edt::MutaItemConv> for MutaItemConvs {
    fn fsd_merge(self, id: rc::ReeInt) -> Vec<rc::edt::MutaItemConv> {
        let mut vec = Vec::new();
        for item_map in self.item_maps {
            for applicable_type in item_map.applicable_item_ids {
                vec.push(rc::edt::MutaItemConv::new(id, applicable_type, item_map.result_item_id))
            }
        }
        vec
    }
