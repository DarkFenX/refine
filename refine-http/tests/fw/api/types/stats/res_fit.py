...
class FitStats(AttrDict):
    ...
    def __init__(self, *, data: dict) -> None:
        super().__init__(data=data, hooks={
            ...
            # Fit resources
            'cpu': AttrHookDef(func=lambda d: (
                NttList(StatResource(data=e) for e in d) if d is not None else None)),
            'powergrid': AttrHookDef(func=lambda d: (
                NttList(StatResource(data=e) for e in d) if d is not None else None)),
            'calibration': AttrHookDef(func=lambda d: (
                NttList(StatResource(data=e) for e in d) if d is not None else None)),
            'drone_bay_volume': AttrHookDef(func=lambda d: (
                NttList(StatResource(data=e) for e in d) if d is not None else None)),
            'drone_bandwidth': AttrHookDef(func=lambda d: (
                NttList(StatResource(data=e) for e in d) if d is not None else None)),
            'fighter_bay_volume': AttrHookDef(func=lambda d: (
                NttList(StatResource(data=e) for e in d) if d is not None else None)),
            # Fit slots
            'high_slots': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'mid_slots': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'low_slots': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'turret_slots': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'launcher_slots': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'rig_slots': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'service_slots': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'subsystem_slots': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'launched_drones': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'launched_fighters': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'launched_light_fighters': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'launched_heavy_fighters': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'launched_support_fighters': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'launched_st_light_fighters': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'launched_st_heavy_fighters': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            'launched_st_support_fighters': AttrHookDef(func=lambda d: (
                NttList(StatSlot(data=e) for e in d) if d is not None else None)),
            # Ship tank
