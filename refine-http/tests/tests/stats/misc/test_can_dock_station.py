...
def test_ship_dock_modified(client, consts):
    ...
    eve_ship_id = client.mk_eve_ship(attrs={eve_dock_attr_id: 0})
...
def test_ship_warp(client, consts):
    ...
    eve_ship_id = client.mk_eve_ship(attrs={eve_warp_attr_id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
...
def test_ship_dock_values(client, consts):
    eve_dock_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_docking)
    eve_ship1_id = client.mk_eve_ship(attrs={eve_dock_attr_id: -100})
    eve_ship2_id = client.mk_eve_ship(attrs={eve_dock_attr_id: 0})
    eve_ship3_id = client.mk_eve_ship(attrs={eve_dock_attr_id: 100})
    eve_ship4_id = client.mk_eve_ship()
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
...
def test_ship_dock_no_attr(client, consts):
    ...
    eve_ship_id = client.mk_eve_ship(attrs={eve_dock_attr_id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
...
def test_struct(client, consts):
    eve_dock_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_docking)
    eve_struct_id = client.mk_eve_struct(attrs={eve_dock_attr_id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
...
def test_not_requested(client, consts):
    eve_dock_attr_id = client.mk_eve_attr(id_=consts.EveAttr.disallow_docking)
    eve_ship_id = client.mk_eve_ship(attrs={eve_dock_attr_id: 100})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    api_ship = api_fit.set_ship(type_id=eve_ship_id)
    # Verification
    api_fit_stats = api_fit.get_stats(options=FitStatsOptions(can_dock_station=False))
    with check_no_field():
        api_fit_stats.can_dock_station  # ruff:ignore[useless-expression]
    api_ship_stats = api_ship.get_stats(options=ItemStatsOptions(can_dock_station=False))
    with check_no_field():
        api_ship_stats.can_dock_station  # ruff:ignore[useless-expression]
