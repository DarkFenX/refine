...
def test_fail_single(client, consts):
    eve_ftr_type_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_sq_is_st_heavy)
    eve_max_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_st_heavy_slots)
    ...
    # Verification
    api_val = api_fit.validate(options=ValOptions(unlaunchable_st_heavy_fighter=True))
    assert api_val.passed is False
    assert api_val.details.unlaunchable_st_heavy_fighter.max == 0
    assert api_val.details.unlaunchable_st_heavy_fighter.users == [api_fighter.id]
...
def test_fail_multiple_ship(client, consts):
    ...
    assert api_val.passed is False
    assert api_val.details.unlaunchable_st_heavy_fighter.max == 0
    assert api_val.details.unlaunchable_st_heavy_fighter.users == sorted([api_fighter1.id, api_fighter2.id])
...
def test_known_failures(client, consts):
    ...
    api_fighter2 = api_fit.add_fighter(type_id=eve_fighter_id, state=consts.ApiMinionState.in_bay)
    # Verification
    api_val = api_fit.validate(options=ValOptions(unlaunchable_st_heavy_fighter=[api_fighter1.id]))
    assert api_val.passed is False
    assert api_val.details.unlaunchable_st_heavy_fighter.max == 0
    assert api_val.details.unlaunchable_st_heavy_fighter.users == [api_fighter2.id]
    api_val = api_fit.validate(options=ValOptions(unlaunchable_st_heavy_fighter=[api_fighter2.id]))
    assert api_val.passed is False
    assert api_val.details.unlaunchable_st_heavy_fighter.max == 0
    assert api_val.details.unlaunchable_st_heavy_fighter.users == [api_fighter1.id]
    api_val = api_fit.validate(options=ValOptions(unlaunchable_st_heavy_fighter=[api_fighter1.id, api_fighter2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    api_val = api_fit.validate(options=ValOptions(
        unlaunchable_st_heavy_fighter=[api_fighter1.id, api_other.id, api_fighter2.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
...
def test_fractional_fighter_type(client, consts):
    ...
    assert api_val.passed is False
    assert api_val.details.unlaunchable_st_heavy_fighter.max == 0
    assert api_val.details.unlaunchable_st_heavy_fighter.users == sorted([
...
def test_no_ship(client, consts):
    eve_ftr_type_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_sq_is_st_heavy)
    client.mk_eve_attr(id_=consts.EveAttr.ftr_st_heavy_slots)
    eve_fighter_id = client.mk_eve_item(attrs={eve_ftr_type_attr_id: 1})
    client.create_sources()
    api_sol = client.create_sol()
    api_fit = api_sol.create_fit()
    ...
    api_val = api_fit.validate(options=ValOptions(unlaunchable_st_heavy_fighter=True))
    assert api_val.passed is False
    assert api_val.details.unlaunchable_st_heavy_fighter.max is None
    assert api_val.details.unlaunchable_st_heavy_fighter.users == [api_fighter.id]
...
def test_no_attr_max(client, consts):
    eve_ftr_type_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_sq_is_st_heavy)
    eve_max_attr_id = consts.EveAttr.ftr_st_heavy_slots
    eve_fighter_id = client.mk_eve_item(attrs={eve_ftr_type_attr_id: 1})
...
def test_no_attr_fighter_type(client, consts):
    eve_ftr_type_attr_id = consts.EveAttr.ftr_sq_is_st_heavy
    eve_max_attr_id = client.mk_eve_attr(id_=consts.EveAttr.ftr_st_heavy_slots)
    eve_fighter_id = client.mk_eve_item(attrs={eve_ftr_type_attr_id: 1})
    eve_ship_id = client.mk_eve_ship(attrs={eve_max_attr_id: 0})
    client.create_sources()
...
def test_criterion_item_kind(client, consts):
    ...
    api_val = api_fit.validate(options=ValOptions(unlaunchable_st_heavy_fighter=[api_fighter.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
