...
def test_known_failures(client, consts):
    ...
    api_val = api_fit.validate(options=ValOptions(low_slot_count=(True, [api_module3.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
    ...
    api_val = api_fit.validate(options=ValOptions(low_slot_count=(True, [api_module4.id])))
    assert api_val.passed is False
    assert api_val.details.low_slot_count.used == 7
    assert api_val.details.low_slot_count.max == 3
    assert api_val.details.low_slot_count.users == [api_module3.id]
    api_val = api_fit.validate(options=ValOptions(low_slot_count=(True, [api_module3.id, api_module4.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    api_val = api_fit.validate(options=ValOptions(
        low_slot_count=(True, [api_module3.id, api_other.id, api_module4.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
    # Action
...
def test_criterion_item_kind(client, consts):
    ...
    api_stats = api_fit.get_stats(options=FitStatsOptions(low_slots=True))
    assert api_stats.low_slots.one() == (1, 0)
    api_val = api_fit.validate(options=ValOptions(low_slot_count=(True, [api_module.id])))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # noqa: B018
