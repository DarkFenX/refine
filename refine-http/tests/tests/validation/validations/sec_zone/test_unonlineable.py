...
def test_known_failures(client, consts):
    ...
    api_val = api_fit.validate(options=ValOptions(
        sec_zone_unonlineable=[api_lowsec_service.id, api_nullsec_service.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
    api_val = api_fit.validate(options=ValOptions(
        sec_zone_unonlineable=[api_lowsec_service.id, api_other.id, api_nullsec_service.id, api_hisec_service.id]))
    assert api_val.passed is True
    with check_no_field():
        api_val.details  # ruff:ignore[useless-expression]
