...
def test_data_types_key_prefix(client, log):
    ...
    log.wait_log_entry(msg=warning, level='WARN', span='src-add:sync:edh')
...
def test_data_typedogma_key_prefix(client, log):
    ...
    log.wait_log_entry(msg=warning1, level='WARN', span='src-add:sync:edh')
    log.wait_log_entry(msg=warning2, level='WARN', span='src-add:sync:edh')
    api_sol = client.create_sol()
    api_item1 = api_sol.add_sw_effect(type_id=eve_item1_id)
