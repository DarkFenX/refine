...
pub(crate) async fn batch_sol(
    ...
    WithRejection(Json(payload), _): WithRejection<Json<SolBatchReqBody>, ApiError>,
) -> impl IntoResponse {
    match internal_batch_sol(state, sol_id, payload).await {
        Ok(resp) => (StatusCode::OK, Json(resp)).into_response(),
        Err(err) => err.into_response(),
    }
}

async fn internal_batch_sol(
    state: AppState,
    sol_id: String,
