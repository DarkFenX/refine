...
impl HCmdResps {
    ...
    fn get_fleet_id(&self, index: usize) -> Result<rc::FleetId, HExecError> {
        ...
        match resp {
            HCmdResp::CreatedFleetId(resp) => Ok(resp.fleet_id),
    ...
    fn get_fit_id(&self, index: usize) -> Result<rc::FitId, HExecError> {
        let resp = self.get_resp(index)?;
        match resp {
            HCmdResp::CreatedFitId(resp) => Ok(resp.fit_id),
    ...
    fn get_item_id(&self, index: usize) -> Result<rc::ItemId, HExecError> {
        let resp = self.get_resp(index)?;
        match resp {
            HCmdResp::CreatedItemIds(resp) => Ok(resp.item_id),
    ...
    fn get_charge_item_id(&self, index: usize) -> Result<rc::ItemId, HExecError> {
        let resp = self.get_resp(index)?;
        match resp {
            HCmdResp::CreatedItemIds(resp) if let Some(charge_item_id) = resp.charge_item_id => Ok(charge_item_id),
            HCmdResp::ChangedItemIds(resp) if let Some(charge_item_id) = resp.charge_item_id => Ok(charge_item_id),
